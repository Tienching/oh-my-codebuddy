...
- When session guidance enables `USE_OMB_EXPLORE_CMD`, prefer `omb explore` for simple read-only repository lookups during planning; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `omb explore` is unavailable.
...
6. **Apply improvements**: When reviewers approve with improvement suggestions, merge all accepted improvements into the plan file before proceeding. Final consensus output **MUST** include an **ADR** section with: **Decision**, **Drivers**, **Alternatives considered**, **Why chosen**, **Consequences**, **Follow-ups**. Specifically:
   ...
   c. Update the plan file in `.omb/plans/` with the accepted improvements (add missing details, refine steps, strengthen acceptance criteria, ADR updates, etc.)
   ...
   f. For the `$team` path, add an explicit launch-hint block with concrete `omb team` / `$team` commands and a **team verification path** (what team proves before shutdown, what Ralph verifies after handoff)
...
1. Read plan file from `.omb/plans/`
...
- For consensus/ralplan execution handoff: **Available-Agent-Types Roster**, **Follow-up Staffing Guidance** (including suggested reasoning levels by lane), explicit `omb team` / `$team` **Launch Hints**, and **Team Verification Path**
- For deliberate consensus mode: **Pre-mortem (3 scenarios)** and **Expanded Test Plan** (unit/integration/e2e/observability)

Plans are saved to `.omb/plans/`. Drafts go to `.omb/drafts/`.
</Steps>
...
- If ToolSearch finds no MCP tools or Codex is unavailable, fall back to equivalent OMB prompt agents -- never block on external tools
...
- In consensus mode, execution follow-up handoff **MUST** include an explicit available-agent-types roster plus concrete staffing / role-allocation guidance grounded in that roster, suggested reasoning levels by lane, explicit `omb team` / `$team` launch hints, and a team verification path
</Tool_Usage>
