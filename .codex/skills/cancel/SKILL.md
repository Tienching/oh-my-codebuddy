---
name: cancel
description: Cancel any active OMB mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline, team)
---
...
- By default the command inspects the current session via `state_list_active` and `state_get_status`, navigating `.omb/state/sessions/{sessionId}/…` to discover which mode is active.
- When a session id is provided or already known, that session-scoped path is authoritative. Legacy files in `.omb/state/*.json` are consulted only as a compatibility fallback if the session id is missing or empty.
- Swarm is a shared SQLite/marker mode (`.omb/state/swarm.db` / `.omb/state/swarm-active.marker`) and is not session-scoped.
...
- `.omb/state/sessions/` (empty directory cleanup after clearing sessions)
...
1. Call `state_list_active` to enumerate `.omb/state/sessions/{sessionId}/…` and discover every active session.
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omb/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected by checking for config files in `.omb/state/team/`:

```bash
# Check for active teams
ls .omb/state/team/*/config.json 2>/dev/null
```
...
```
For each team found in .omb/state/team/:
  1. Read config.json to get team_name and workers list
  2. For each worker:
     a. Write shutdown inbox to .omb/state/team/{name}/workers/{worker}/inbox.md
...
**Cleanup:**
```
  1. Strip AGENTS.md team worker overlay (<!-- OMB:TEAM:WORKER:START/END -->)
  2. Remove team state directory: rm -rf .omb/state/team/{name}/
...
1. Check `.omb/state/team/*/config.json` for active teams
