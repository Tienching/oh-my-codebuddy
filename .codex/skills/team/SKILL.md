...
- Use **CodeBuddy native subagents** for bounded, in-session parallelism where one leader thread can fan out a few independent subtasks and wait for them directly.
- Use **`omb team`** when you need durable tmux workers, shared task state, mailbox/dispatch coordination, worktrees, explicit lifecycle control, or long-running parallel execution that must survive beyond one local reasoning burst.
...
1. Invoke OMB runtime directly with `omb team ...`
...
If `omb team` is unavailable, stop with a hard error.
...
- **Canonical launch:** use plain `omb team ...` / `$team ...` for coordinated workers.
...
2. Reuse the latest relevant snapshot in `.omb/context/{slug}-*.md` when available.
3. If none exists, create `.omb/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
   - task statement
   - desired outcome
...
- include an explicit launch hint (`omb team N "<task>"` / `$team N "<task>"`) for the coordinated team run; mention a later separate Ralph follow-up only when genuinely needed
...
`omb team` currently performs:

1. Parse args (`N`, `agent-type`, task)
2. Sanitize team name from task text
3. Initialize team state:
   - `.omb/state/team/<team>/config.json`
   - `.omb/state/team/<team>/manifest.v2.json`
   - `.omb/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.omb/state/team/<team>/worker-agents.md`
...
7. Launch workers with:
   - `OMB_TEAM_WORKER=<team>/worker-<n>`
   - `OMB_TEAM_STATE_ROOT=<leader-cwd>/.omb/state`
   - `OMB_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `OMB_TEAM_WORKER_CLI` / `OMB_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- Workers may run in separate git worktrees (`omb team --worktree[=<name>]`) while sharing one team state root
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `OMB_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `OMB_TEAM_WORKER_CLI` / auto detection.
...
3. Low-complexity default from `OMB_DEFAULT_SPARK_MODEL` (legacy alias: `OMB_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `OMB_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `OMB_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, OMB allocates **per-worker reasoning effort dynamically** from the resolved worker role (`low`, `medium`, `high`).
- Explicit launch args still win: if `OMB_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
2. Monitor task and worker progress with runtime/state tools first (`omb team status <team>`, `omb team resume <team>`, mailbox/state files)
3. Wait for terminal task state before shutdown:
   - `pending=0`
   - `in_progress=0`
   - `failed=0` (or explicitly acknowledged failure path)
4. Only then run `omb team shutdown <team>`
...
1. Use `omb team ...` runtime lifecycle commands for orchestration.
2. Use `omb team api ... --json` for mailbox/task mutations.
3. Verify delivery via mailbox/state evidence (`mailbox/*.json`, task status, `omb team status`).
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.omb/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`OMB_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.omb/state/team/<team>/...` files
- Team mailbox files:
- `.omb/state/team/<team>/mailbox/leader-fixed.json`
- `.omb/state/team/<team>/mailbox/worker-<n>.json`
- `.omb/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.omb/state/team/<team>/manifest.v2.json`
- `.omb/state/team/<team>/tasks/task-<id>.json`
- `.omb/state/team/<team>/workers/worker-<n>/identity.json`
- `.omb/state/team/<team>/workers/worker-<n>/inbox.md`
- `.omb/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.omb/state/team/<team>/workers/worker-<n>/status.json`
- `.omb/state/team-leader-nudge.json`


## Team Mutation Interop (CLI-first)

Use `omb team api` for machine-readable mutation/reads instead of legacy `team_*` MCP tools.
...
- Send ACK to `leader-fixed` mailbox via `omb team api send-message --json`
- Claim/transition/release task lifecycle via `omb team api <operation> --json`
...
Use only after checking `omb team status <team>` and mailbox/state evidence:

1. Capture pane tail to confirm current worker state:
   - `tmux capture-pane -t %<worker-pane> -p -S -120`
   - If a larger-tail read or bounded summary would help, prefer explicit opt-in inspection via `omb sparkshell --tmux-pane %<worker-pane> --tail-lines 400` before improvising extra tmux commands.
...
2. `.omb/state/team/<team>/mailbox/leader-fixed.json` exists
3. Worker skill loaded and `omb team api send-message --json` called
4. Task-id mismatch not blocking worker flow

### Worker logs `omb team api ... ENOENT` (or legacy `team_send_message ENOENT` / `team_update_task ENOENT`)

Meaning:
- Team state path no longer exists while worker is still running.
- Typical cause: leader/manual flow ran `omb team shutdown <team>` (or removed `.omb/state/team/<team>`) before worker finished.

Checks:

1. `omb team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.omb/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .omb/state/team/<team>`) happened during execution
...
- Do not kill HUD pane (`omb hud --watch`) unless intentionally restarting HUD
