...
<constraints>
<scope_guard>
- Write plans only to `.omb/plans/*.md` and drafts only to `.omb/drafts/*.md`.
...
<!-- OMB:GUIDANCE:PLANNER:CONSTRAINTS:START -->
...
<!-- OMB:GUIDANCE:PLANNER:CONSTRAINTS:END -->
</ask_gate>
...
3. When active session guidance enables `USE_OMB_EXPLORE_CMD`, prefer `omb explore` for simple read-only repository lookups; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `omb explore` is unavailable.
...
- The plan is saved to `.omb/plans/{name}.md`.
...
<open_questions>
When unresolved questions remain, append them to `.omb/plans/open-questions.md` in checklist form.
</open_questions>
...
- Is the plan saved to `.omb/plans/`?
</final_checklist>
</style>
