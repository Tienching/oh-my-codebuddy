...
- Do not hardcode stale frontier-model overrides for CodeBuddy native subagents. If an explicit frontier override is necessary, use the current frontier default from `OMB_DEFAULT_FRONTIER_MODEL` (legacy alias: `OMB_DEFAULT_FRONTIER_MODEL`) / the repo model contract (currently `gpt-5.4`), not older values such as `gpt-5.2`.
