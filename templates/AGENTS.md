...
- Within a single CodeBuddy session or team pane, use CodeBuddy native subagents for independent, bounded parallel subtasks when that improves throughput.
...
| Keyword(s) | Skill | Action |
|-------------|-------|--------|
| "ralph", "don't stop", "must complete", "keep going" | `$ralph` | Runtime-only: read `~/.codex/skills/ralph/SKILL.md`, execute persistence loop only inside OMB CLI/runtime |
| "autopilot", "build me", "I want a" | `$autopilot` | Runtime-only: read `~/.codex/skills/autopilot/SKILL.md`, execute autonomous pipeline only inside OMB CLI/runtime |
| "ultrawork", "ulw", "parallel" | `$ultrawork` | Runtime-only: read `~/.codex/skills/ultrawork/SKILL.md`, execute parallel agents only inside OMB CLI/runtime |
| "ultraqa" | `$ultraqa` | Runtime-only: read `~/.codex/skills/ralph/SKILL.md`, run persistent completion and verification loop only inside OMB CLI/runtime (UltraQA compatibility alias) |
...
| "team", "swarm", "coordinated team", "coordinated swarm" | `$team` | Runtime-only: read `~/.codex/skills/team/SKILL.md`, start tmux-based team orchestration only inside OMB CLI/runtime (swarm compatibility alias) |
| "ecomode", "eco", "budget" | `$ecomode` | Runtime-only: read `~/.codex/skills/ultrawork/SKILL.md`, execute cost-aware parallel workflow only inside OMB CLI/runtime (ecomode compatibility alias) |
...
- Planning is complete only after both `.omb/plans/prd-*.md` and `.omb/plans/test-spec-*.md` exist (legacy `.omx/plans/...` paths remain acceptable compatibility aliases).
...
Model precedence:
1. Explicit model in `OMB_TEAM_WORKER_LAUNCH_ARGS` (legacy alias: `OMX_TEAM_WORKER_LAUNCH_ARGS`)
2. Inherited leader `--model`
3. Low-complexity default model from `OMB_DEFAULT_SPARK_MODEL` (legacy aliases: `OMX_DEFAULT_SPARK_MODEL`, `OMX_SPARK_MODEL`)

Normalize model flags to one canonical `--model <value>` entry.
Do not guess frontier/spark defaults from model-family recency; use `OMB_DEFAULT_FRONTIER_MODEL` / `OMB_DEFAULT_SPARK_MODEL` first, with `OMX_DEFAULT_FRONTIER_MODEL` / `OMX_DEFAULT_SPARK_MODEL` as legacy aliases.
</team_model_resolution>
...
Command routing:
- When `USE_OMB_EXPLORE_CMD` (legacy alias: `USE_OMX_EXPLORE_CMD`) enables advisory routing, strongly prefer `omb explore` (`omx explore` compatibility alias) as the default surface for simple read-only repository lookup tasks (files, symbols, patterns, relationships).
- For simple file/symbol lookups, use `omb explore` first; `omx explore` remains the compatibility alias.

When to use what:
- Use `omb explore --prompt ...` (or `omx explore --prompt ...`) for simple read-only lookups.
- Use `omb sparkshell` / `omx sparkshell` for noisy read-only shell commands, bounded verification runs, repo-wide listing/search, or tmux-pane summaries; `omb sparkshell --tmux-pane ...` is explicit opt-in.
- Keep ambiguous, implementation-heavy, edit-heavy, or non-shell-only work on the richer normal path.
- `omb explore` / `omx explore` is a shell-only, allowlisted, read-only path; do not rely on it for edits, tests, diagnostics, MCP/web access, or complex shell composition.
- If `omb explore` / `omx explore` or `omb sparkshell` / `omx sparkshell` is incomplete or ambiguous, retry narrower and gracefully fall back to the normal path.
...
- Persist verdict JSON in `.omb/state/{scope}/ralph-progress.json` (legacy `.omx/state/...` paths remain acceptable compatibility aliases).
...
<state_management>
OMB persists runtime state under `.omb/` (legacy `.omx/` compatibility paths may also exist):
...
Run `omb setup` to install all components. Run `omb doctor` to verify installation (`omx` remains a compatibility alias).
