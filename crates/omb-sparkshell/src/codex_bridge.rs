...
pub fn resolve_model() -> String {
    env::var("OMB_SPARKSHELL_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("OMB_DEFAULT_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
        .or_else(|| {
            env::var("OMB_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_fallback_model() -> String {
    env::var("OMB_SPARKSHELL_FALLBACK_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("OMB_DEFAULT_FRONTIER_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn read_summary_timeout_ms() -> u64 {
    env::var("OMB_SPARKSHELL_SUMMARY_TIMEOUT_MS")
        .ok()
...
mod tests {
    ...
    fn model_resolution_prefers_sparkshell_override() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMB_SPARKSHELL_MODEL", "spark-a");
            env::set_var("OMB_DEFAULT_SPARK_MODEL", "spark-b");
        }
        assert_eq!(resolve_model(), "spark-a");
        unsafe {
            env::remove_var("OMB_SPARKSHELL_MODEL");
            env::remove_var("OMB_DEFAULT_SPARK_MODEL");
            env::remove_var("OMB_SPARK_MODEL");
        }
    }
    ...
    fn fallback_model_resolution_prefers_override_then_default_frontier() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OMB_SPARKSHELL_FALLBACK_MODEL");
            env::remove_var("OMB_DEFAULT_FRONTIER_MODEL");
        }
        assert_eq!(resolve_fallback_model(), DEFAULT_FRONTIER_MODEL);

        unsafe {
            env::set_var("OMB_DEFAULT_FRONTIER_MODEL", "frontier-a");
        }
        assert_eq!(resolve_fallback_model(), "frontier-a");

        unsafe {
            env::set_var("OMB_SPARKSHELL_FALLBACK_MODEL", "frontier-b");
        }
    ...
    fn timeout_defaults_when_unset() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OMB_SPARKSHELL_SUMMARY_TIMEOUT_MS");
        }
    ...
    fn model_resolution_ignores_blank_override_and_uses_secondary_env() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMB_SPARKSHELL_MODEL", "   ");
            env::set_var("OMB_DEFAULT_SPARK_MODEL", "spark-b");
        }
    ...
    fn timeout_defaults_for_zero_and_invalid_values() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMB_SPARKSHELL_SUMMARY_TIMEOUT_MS", "0");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);

        unsafe {
            env::set_var("OMB_SPARKSHELL_SUMMARY_TIMEOUT_MS", "bogus");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);
