...
use omb_mux::build_capture_pane_args;
...
fn run(args: Vec<String>) -> Result<(), SparkshellError> {
    ...
    match summarize_output(&execution_argv, &output) {
        ...
        Err(error) => {
            write_raw_output(&output.stdout, &output.stderr)?;
            eprintln!("omb sparkshell: summary unavailable ({error})");
        }
    }
...
fn usage_text() -> String {
    format!(
        concat!(
            ...
            "   or: omb-sparkshell --tmux-pane <pane-id> [--tail-lines <{min}-{max}>]\n",
            "\n",
