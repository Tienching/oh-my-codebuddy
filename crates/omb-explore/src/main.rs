...
const CODEX_BIN_ENV: &str = "OMB_EXPLORE_CODEX_BIN";
const HARNESS_ROOT_ENV: &str = "OMB_EXPLORE_ROOT";
...
fn usage() -> &'static str {
    "Usage: omb-explore --cwd <dir> --prompt <text> --prompt-file <explore-prompt.md> --model-spark <model> --model-fallback <model>"
}
...
fn temp_output_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    env::temp_dir().join(format!("omb-explore-{}-{}.md", std::process::id(), nanos))
}
...
fn build_direct_wrapper(self_exe: &Path, command: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "#!/bin/sh\nprintf '%s\\n' {} >&2\nexit 127\n",
        shell_quote(&format!(
            "omb explore allowlisted host command `{command}` is unavailable on this host"
        )),
    ))
}
...
fn validate_direct_command(command_name: &str, args: &[String]) -> Result<(), String> {
    if !ALLOWED_DIRECT_COMMANDS.contains(&command_name) {
        return Err(format!(
            "command `{command_name}` is not on the omb explore allowlist"
        ));
    }
    ...
    match command_name {
        "rg" => {
            ...
            {
                return Err("ripgrep `--pre` is not allowed in omb explore".to_string());
            }
            if args.iter().any(|arg| arg == "-") {
                return Err("ripgrep stdin (`-`) is not allowed in omb explore".to_string());
            }
        }
        "grep" => {
            if args.iter().any(|arg| arg == "-") {
                return Err("grep stdin (`-`) is not allowed in omb explore".to_string());
            }
            if non_option_operands(args).len() < 2 {
                return Err(
                    "grep requires a pattern and at least one file/path in omb explore".to_string(),
                );
            }
        }
        "find" => {
            if args.iter().any(|arg| {
                ...
                return Err(
                    "find actions that execute, delete, or write files are not allowed in omb explore"
                        .to_string(),
                );
            }
        }
        "cat" => {
            ...
            if operands.is_empty() {
                return Err("cat requires at least one file/path in omb explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("cat stdin (`-`) is not allowed in omb explore".to_string());
            }
        }
        "head" | "wc" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err(format!(
                    "{command_name} requires at least one file/path in omb explore"
                ));
            }
            if operands.contains(&"-") {
                return Err(format!(
                    "{command_name} stdin (`-`) is not allowed in omb explore"
                ));
            }
        }
        "tail" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err("tail requires at least one file/path in omb explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("tail stdin (`-`) is not allowed in omb explore".to_string());
            }
            if args.iter().any(|arg| {
                matches!(arg.as_str(), "-f" | "-F" | "--retry") || arg.starts_with("--follow")
            }) {
                return Err("tail follow/retry modes are not allowed in omb explore".to_string());
            }
        }
        _ => {}
    }
