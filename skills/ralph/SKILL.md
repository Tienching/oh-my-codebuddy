...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.omb/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_OMB_EXPLORE_CMD`, prefer `omb explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.omb/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `oh-my-codebuddy:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
Use the `omb_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.omb/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.omb/plans/prd-{slug}.md`
   - Progress ledger: `.omb/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.omb/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.omb/plans/prd-{slug}.md`, create `.omb/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- If `.omb/prd.json` exists and canonical PRD is absent, migrate one-way into `.omb/plans/prd-{slug}.md`.
- If `.omb/progress.txt` exists and canonical progress ledger is absent, import one-way into `.omb/state/{scope}/ralph-progress.json`.
