...
- **team-exec**: Team execution via CodeBuddy CLI workers. Always the OMB execution backend.
- **ralph-verify**: Ralph verification loop with configurable iteration count.

## State Management

Pipeline state persists via the ModeState system at `.omb/state/pipeline-state.json`.
...
- **team**: Pipeline delegates execution to team mode (CodeBuddy CLI workers)
