...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.omb/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.omb/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.omb/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.omb/plans/autopilot-spec.md`
...
6. **Phase 5 - Cleanup**: Clear all mode state via OMB MCP tools on successful completion
...
Use `omb_state` MCP tools for autopilot lifecycle state.
