...
OMB_TEAM_WORKER_CLI_MAP=codebuddy,claude omb team 2:executor "split doc/code tasks"
...
- **`omb team ...` CLI** — Primary method for interactive or automated team orchestration. Use this when you want direct tmux-pane visibility or a scriptable launch path.
- **Team state files** — Inspect `.omb/state/team/<team>/` when you need status, task, or mailbox evidence after launch.
...
- tmux/session cleanup: Use the documented `omb team` shutdown / cleanup flow when you need to stop worker panes or clean up an interrupted run.

### Automation example

```
1. omb team 1:executor "fix bugs"
2. omb team status <team-name>
3. omb team shutdown <team-name>
