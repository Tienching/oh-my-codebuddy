---
name: worker
description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OMB teams
---

# Worker Skill

This skill is for a CodeBuddy session that was started as an OMB Team worker (a tmux pane spawned by `$team`).

## Identity

You MUST be running with `OMB_TEAM_WORKER` set. It looks like:

`<team-name>/worker-<n>`

Example: `alpha/worker-2`

## Load Worker Skill Path (CodeBuddy)
...
2. `${CODEX_HOME:-~/.codex}/skills/worker/SKILL.md`
3. `<leader_cwd>/.codebuddy/skills/worker/SKILL.md`
4. `<leader_cwd>/.codex/skills/worker/SKILL.md`
5. `<leader_cwd>/skills/worker/SKILL.md` (repo fallback)

## Startup Protocol (ACK)

1. Parse `OMB_TEAM_WORKER` into:
   - `teamName` (before the `/`)
...
Use CLI interop:
- `omb team api send-message --input <json> --json` with `{team_name, from_worker, to_worker:"leader-fixed", body}`
...
6. Claim the task (do NOT start work without a claim) using claim-safe lifecycle CLI interop (`omb team api claim-task --json`).
7. Do the work.
8. Complete/fail the task via lifecycle transition CLI interop (`omb team api transition-task-status --json`) from `in_progress` to `completed` or `failed`.
   - Do NOT directly write lifecycle fields (`status`, `owner`, `result`, `error`) in task files.
9. Use `omb team api release-task-claim --json` only for rollback/requeue to `pending` (not for completion).
...
Use CLI interop:
- `omb team api mailbox-list --json` to read
- `omb team api mailbox-mark-delivered --json` to acknowledge delivery
...
- Prefer inbox/mailbox/task state and `omb team api ... --json` operations.
...
If the lead sends a shutdown request, follow the shutdown inbox instructions exactly, write your shutdown ack file, then exit the CodeBuddy session.
