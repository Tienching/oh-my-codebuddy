...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what OMB may decide without confirmation.
</Purpose>
...
- **Autoresearch (`--autoresearch`)**: same interview rigor as Standard, but specialized for `omb autoresearch` launch readiness and `.omb/specs/` mission/sandbox artifact handoff

If no flag is provided, use **Standard**.

<Mode_Flags>
- **`--autoresearch`**: switch the interview into autoresearch-intake mode for `omb autoresearch` handoff. In this mode, the interview should converge on a launch-ready research mission, write canonical artifacts under `.omb/specs/`, and preserve the explicit `refine further` vs `launch` boundary for downstream CLI intake.
</Mode_Flags>
</Depth_Profiles>
...
- When session guidance enables `USE_OMB_EXPLORE_CMD`, prefer `omb explore` for simple read-only brownfield fact gathering; keep prompts narrow and concrete, and keep ambiguous or non-shell-only investigation on the richer normal path and fall back normally if `omb explore` is unavailable.
- Always run a preflight context intake before the first interview question
- Reduce user effort: ask only the highest-leverage unresolved question, and never ask the user for codebase facts that can be discovered directly
- For brownfield work, prefer evidence-backed confirmation questions such as "I found X in Y. Should this change follow that pattern?"
- In CodeBuddy CLI, prefer `request_user_input` when available; if unavailable, fall back to concise plain-text one-question turns
...
2. Attempt to load the latest relevant context snapshot from `.omb/context/{slug}-*.md`.
...
4. Save snapshot to `.omb/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".omb/context/<slug>-<timestamp>.md"
  }
}
```
...
1. Write interview transcript summary to:
   - `.omb/interviews/{slug}-{timestamp}.md`  
     (kept for ralph PRD compatibility)
2. Write execution-ready spec to:
   - `.omb/specs/deep-interview-{slug}.md`
...
- Decision Boundaries (what OMB may decide without confirmation)
- Constraints
...
When the clarified task is specifically about `omb autoresearch`, or the skill is invoked with `--autoresearch`, keep the interview domain-specific and emit launch-consumable artifacts without skipping clarification.
...
- **Canonical artifact path:** `.omb/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.omb/specs/autoresearch-{slug}/mission.md`, `.omb/specs/autoresearch-{slug}/sandbox.md`, and `.omb/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.omb/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.omb/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- **Structured result contract:** `result.json` should point to the draft + mission/sandbox artifacts and carry the finalized `topic`, `evaluatorCommand`, `keepPolicy`, `slug`, `launchReady`, and `blockedReasons` fields so `omb autoresearch` can consume it directly
...
- **Input Artifact:** `.omb/specs/deep-interview-{slug}.md` (optionally accompanied by the transcript/context snapshot for traceability)
...
- **Expected Output:** Canonical planning artifacts under `.omb/plans/`, especially `prd-*.md` and `test-spec-*.md`
...
### 2. **`$autopilot`**
- **Input Artifact:** `.omb/specs/deep-interview-{slug}.md`
...
- Read/write context snapshots under `.omb/context/`
- Save transcript/spec artifacts under `.omb/interviews/` and `.omb/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.omb/context/{slug}-{timestamp}.md`
...
- [ ] Transcript written to `.omb/interviews/{slug}-{timestamp}.md`
- [ ] Spec written to `.omb/specs/deep-interview-{slug}.md`
