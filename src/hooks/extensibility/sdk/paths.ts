...
export function hookPluginRootDir(cwd: string, pluginName: string): string {
  return join(cwd, '.omb', 'state', 'hooks', 'plugins', sanitizeHookPluginName(pluginName));
}
...
export function ombRootStateFilePath(cwd: string, fileName: string): string {
  return join(cwd, '.omb', 'state', fileName);
}
