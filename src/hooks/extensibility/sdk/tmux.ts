...
const INJECTION_MARKER = '[OMB_TMUX_INJECT]';
...
async function sendTmuxKeys(
  ...
  const marker = process.env.OMB_HOOK_PLUGIN_LOOP_MARKER || '[OMB_HOOK_PLUGIN]';
  ...
  const cooldownMs = typeof options.cooldownMs === 'number'
    ...
    : asPositiveNumber(process.env.OMB_HOOK_PLUGIN_COOLDOWN_MS, DEFAULT_COOLDOWN_MS);
  const dedupeWindowMs = asPositiveNumber(process.env.OMB_HOOK_PLUGIN_DEDUPE_MS, DEFAULT_DEDUPE_WINDOW_MS);
