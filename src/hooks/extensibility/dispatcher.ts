...
const RESULT_PREFIX = "__OMB_PLUGIN_RESULT__ ";
const RUNNER_SIGKILL_GRACE_MS = 250;

function hooksLogPath(cwd: string): string {
	const day = new Date().toISOString().slice(0, 10);
	return join(cwd, ".omb", "logs", `hooks-${day}.jsonl`);
}
...
async function appendHooksLog(
	...
	await appendFile(
		...
		console.warn("[omb] warning: failed to append hook dispatch log entry", {
			cwd,
...
function isTeamWorker(env: NodeJS.ProcessEnv): boolean {
	return (
		typeof env.OMB_TEAM_WORKER === "string" && env.OMB_TEAM_WORKER.trim() !== ""
	);
}
