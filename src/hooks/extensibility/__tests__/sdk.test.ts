...
describe('createHookPluginSdk', () => {
  ...
  describe('tmux.sendKeys', () => {
    ...
    it('returns loop_guard_input_marker when text contains loop marker', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omb-sdk-'));
      const originalMarker = process.env.OMB_HOOK_PLUGIN_LOOP_MARKER;
      try {
        process.env.OMB_HOOK_PLUGIN_LOOP_MARKER = '[TESTMARK]';
        const sdk = createHookPluginSdk({
          cwd,
          pluginName: 'test',
          event: makeEvent(),
        ...
        if (originalMarker === undefined) {
          delete process.env.OMB_HOOK_PLUGIN_LOOP_MARKER;
        } else {
          process.env.OMB_HOOK_PLUGIN_LOOP_MARKER = originalMarker;
        }
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('prefers non-HUD codex pane when targeting a tmux session', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omb-sdk-'));
      const fakeBinDir = await mkdtemp(join(tmpdir(), 'omb-sdk-bin-'));
...
if [[ "$cmd" == "list-panes" ]]; then
  printf "%%2	1	node /pkg/dist/cli/omb.js hud --watch
  ...
  describe('omx', () => {
    it('exposes only the explicit read-only omx readers', async () => {
      ...
      try {
        ...
        assert.deepEqual(Object.keys(sdk.omb).sort(), ['hud', 'notifyFallback', 'session', 'updateCheck']);
        assert.equal(typeof sdk.omb.session.read, 'function');
        assert.equal(typeof sdk.omb.hud.read, 'function');
        assert.equal(typeof sdk.omb.notifyFallback.read, 'function');
        assert.equal(typeof sdk.omb.updateCheck.read, 'function');
        assert.equal('pluginState' in sdk, false);
        assert.equal('readJson' in sdk.omb, false);
        assert.equal('list' in sdk.omb, false);
        assert.equal('exists' in sdk.omb, false);
        assert.equal('write' in sdk.omb.session, false);
        assert.equal('delete' in sdk.omb.session, false);
        assert.equal('write' in sdk.omb.hud, false);
        assert.equal('delete' in sdk.omb.hud, false);
        assert.equal('write' in sdk.omb.notifyFallback, false);
        assert.equal('delete' in sdk.omb.notifyFallback, false);
        assert.equal('write' in sdk.omb.updateCheck, false);
        assert.equal('delete' in sdk.omb.updateCheck, false);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('reads session state from .omb/state/session.json', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        const state = await sdk.omb.session.read();
    ...
    it('returns null for invalid session state without session_id', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omb.session.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('reads hud, notifyFallback, and updateCheck state from root-scoped omx files', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.deepEqual(await sdk.omb.hud.read(), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        assert.deepEqual(await sdk.omb.notifyFallback.read(), {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        assert.deepEqual(await sdk.omb.updateCheck.read(), {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });
      } finally {
    ...
    it('returns null for missing omx reader files', async () => {
      ...
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omb.session.read(), null);
        assert.equal(await sdk.omb.hud.read(), null);
        assert.equal(await sdk.omb.notifyFallback.read(), null);
        assert.equal(await sdk.omb.updateCheck.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
...
describe('clearHookPluginState', () => {
  it('removes data.json and tmux.json for plugin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-clear-'));
    try {
      const pluginDir = join(cwd, '.omb', 'state', 'hooks', 'plugins', 'my-plugin');
