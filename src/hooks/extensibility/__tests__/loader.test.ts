...
describe('hooksDir', () => {
  it('returns .omb/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.omb', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .omb/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.omb', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-load-'));
    try {
