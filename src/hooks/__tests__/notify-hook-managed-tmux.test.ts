...
describe('notify-hook managed tmux windows fallback', () => {
  ...
  afterEach(() => {
    ...
    if (originalTeamWorker !== undefined) process.env.OMB_TEAM_WORKER = originalTeamWorker;
    else delete process.env.OMB_TEAM_WORKER;
  });

  it('does not rely on ps ancestry checks on native Windows', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-managed-tmux-win32-'));
    try {
      const stateDir = join(cwd, '.omb', 'state');
      const sessionId = 'omb-test-session';
