...
describe('notify-hook Ralph session resume', () => {
  it('resumes a matching prior Ralph into the current OMB session and rebinds the pane', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-resume-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
      const currentSessionDir = join(stateDir, 'sessions', currentOmbSessionId);
      const priorSessionDir = join(stateDir, 'sessions', priorOmbSessionId);
      const currentPaneId = '%99';
      await writeJson(join(stateDir, 'session.json'), { session_id: currentOmbSessionId });
      await writeJson(join(priorSessionDir, 'ralph-state.json'), {
        ...
        owner_omb_session_id: priorOmbSessionId,
      ...
      assert.equal(currentState.owner_omb_session_id, currentOmbSessionId);
  ...
  it('rebinds the current pane for an already-active current-session Ralph state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-current-pane-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('preserves current-session legacy owner_codex_thread_id until owner_codex_session_id is available', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-current-legacy-owner-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const currentSessionDir = join(stateDir, 'sessions', currentOmbSessionId);
  ...
  it('adopts a same-thread Ralph across OMB session turnover even when Codex session id is absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-thread-turnover-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const priorOmbSessionId = 'sess-prior';
      const currentOmbSessionId = 'sess-current';
      ...
      await writeJson(join(stateDir, 'session.json'), { session_id: priorOmbSessionId });
      await writeJson(join(priorSessionDir, 'ralph-state.json'), {
        active: true,
        iteration: 4,
        max_iterations: 10,
  ...
  it('does not auto-resume without a unique matching prior Ralph owner', async (t) => {
    ...
    for (const scenario of scenarios) {
      await t.test(scenario.name, async () => {
        const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-no-resume-'));
        try {
          const stateDir = join(wd, '.omb', 'state');
          const currentOmbSessionId = 'sess-current';
          const currentSessionDir = join(stateDir, 'sessions', currentOmbSessionId);
          ...
          for (const priorSessionId of scenario.priorSessions) {
            await writeJson(join(stateDir, 'sessions', priorSessionId, 'ralph-state.json'), {
              ...
              current_phase: 'executing',
              started_at: '2026-02-22T00:00:00.000Z',
              owner_omb_session_id: priorSessionId,
              owner_codex_session_id: 'codex-session-1',
            });
          }
  ...
  it('resumes a legacy prior Ralph that only tracks owner_codex_thread_id', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-thread-resume-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('does not fall back to owner_codex_thread_id when owner_codex_session_id is present and mismatched', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-session-precedence-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('does not auto-resume a legacy Ralph when both source and payload thread ids are missing', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-empty-thread-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('does not auto-resume over an inactive current-session Ralph file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-inactive-current-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('does not auto-resume over an unreadable current-session Ralph file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-unreadable-current-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('serializes concurrent resume attempts so only one transfer occurs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-concurrent-resume-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
  ...
  it('rolls back the target state when transfer fails after writing the current session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-notify-ralph-transfer-rollback-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
      const currentOmbSessionId = 'sess-current';
      const priorOmbSessionId = 'sess-prior';
