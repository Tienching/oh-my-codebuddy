...
const RUNTIME_START = "<!-- OMB:RUNTIME:START -->";
const RUNTIME_END = "<!-- OMB:RUNTIME:END -->";
const WORKER_START = "<!-- OMB:TEAM:WORKER:START -->";
...
describe("generateOverlay", () => {
  ...
  it("generates overlay with no state files (empty but valid)", async () => {
    ...
    assert.ok(overlay.includes("<!-- OMB:RUNTIME:START -->"));
    assert.ok(overlay.includes("<!-- OMB:RUNTIME:END -->"));
  ...
  it("adds advisory explore routing guidance by default and hides it only on explicit opt-out", async () => {
    const previous = process.env.USE_OMB_EXPLORE_CMD;
    try {
      delete process.env.USE_OMB_EXPLORE_CMD;
      ...
      assert.match(defaultOverlay, /omb explore` FIRST before attempting full code analysis/i);

      process.env.USE_OMB_EXPLORE_CMD = "off";
      ...
      if (typeof previous === "string")
        process.env.USE_OMB_EXPLORE_CMD = previous;
      else delete process.env.USE_OMB_EXPLORE_CMD;
    }
  });

  it("generates overlay with active modes", async () => {
    const sessionId = "test-session-2";
    const sessionDir = join(tempDir, ".omb", "state", "sessions", sessionId);
  ...
  it("generates overlay with session-scoped active modes for current session", async () => {
    await mkdir(join(tempDir, ".omb", "state", "sessions", "sess1"), {
      recursive: true,
    });
    await writeFile(
      join(tempDir, ".omb", "state", "sessions", "sess1", "team-state.json"),
      JSON.stringify({
        active: true,
        iteration: 1,
        max_iterations: 5,
  ...
  it("generates overlay with project memory summary", async () => {
    await writeFile(
      join(tempDir, ".omb", "project-memory.json"),
      JSON.stringify({
  ...
  it("skips inactive modes", async () => {
    await writeFile(
      join(tempDir, ".omb", "state", "autopilot-state.json"),
  ...
  it("adds blocked ralph planning gate when PRD/test spec are missing", async () => {
    ...
    await mkdir(join(tempDir, ".omb", "plans"), { recursive: true });
  ...
  it("unlocks ralph planning gate when PRD and test spec exist", async () => {
    ...
    const plansDir = join(tempDir, ".omb", "plans");
...
describe("resolveSessionOrchestrationMode", () => {
  ...
  it("falls back to root team skill state only when no session-scoped skill state exists", async () => {
    ...
    await writeFile(
      join(tempDir, ".omb", "state", "skill-active-state.json"),
      JSON.stringify({ active: true, skill: "team" }),
    );
...
`;
  ...
  it("applyOverlay is idempotent (apply twice, no duplication)", async () => {
    ...
    const startCount = (secondApply.match(/<!-- OMB:RUNTIME:START -->/g) || [])
      .length;
  ...
  it("handles stale markers from previous session", async () => {
    ...
    const startCount = (result.match(/<!-- OMB:RUNTIME:START -->/g) || [])
      .length;
    assert.equal(startCount, 1);
  });
...
describe("session-scoped model instructions file", () => {
  ...
  before(async () => {
    tempDir = await makeTempDir();
    restoreCodexHome = setMockCodebuddyHome(join(tempDir, "home", ".codebuddy"));
  });
  after(async () => {
    restoreCodexHome?.();
    await rm(tempDir, { recursive: true, force: true });
  ...
  it("deduplicates duplicate skill references when project and user scopes both install the same skill", async () => {
    const userAgentsMd = join(tempDir, "home", ".codebuddy", "AGENTS.md");
    const projectAgentsMd = join(tempDir, "AGENTS.md");
    const userSkillDir = join(tempDir, "home", ".codebuddy", "skills", "help");
    const projectSkillDir = join(tempDir, ".codebuddy", "skills", "help");
    await mkdir(join(tempDir, "home", ".codebuddy"), { recursive: true });
    ...
    await writeFile(
      ...
      [
        "# User instructions",
        "",
        "- help: user copy (file: /tmp/home/.codebuddy/skills/help/SKILL.md)",
      ].join("\n"),
    );
    await writeFile(
      projectAgentsMd,
      [
        ...
        "",
        "- help: project copy (file: /tmp/project/.codebuddy/skills/help/SKILL.md)",
      ].join("\n"),
    );
...
describe("hasOverlay", () => {
  ...
  it("returns false when only start marker", () => {
    assert.ok(!hasOverlay("<!-- OMB:RUNTIME:START -->\nbroken"));
  });
});
