...
describe("deep-interview Ouroboros contract", () => {
	...
	it("uses OMB-native output paths", () => {
		assert.match(deepInterviewSkill, /\.omb\/interviews\//);
		assert.match(deepInterviewSkill, /\.omb\/specs\//);
	});
	...
	it("documents the autoresearch specialization contract", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.omb\/specs\/deep-interview-autoresearch-\{slug\}\.md/i,
		);
