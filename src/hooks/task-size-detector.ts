 ...
 * (mandatory enforcement). In OMB, CodeBuddy CLI does not support pre-tool hooks, so this
 * module serves as:
 ...
 *   3. Future hook readiness — will be promoted to runtime enforcement when CodeBuddy CLI
 *      adds pre-hook support
 *
 * The actual gate enforcement in OMB is advisory: AGENTS.md instructs the model to
