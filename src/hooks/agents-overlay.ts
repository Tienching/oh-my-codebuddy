/**
 * AGENTS.md Runtime Overlay for oh-my-codebuddy
 *
...
const START_MARKER = "<!-- OMB:RUNTIME:START -->";
...
async function readNotepadPriority(cwd: string): Promise<string> {
  const notePath = ombNotepadPath(cwd);
...
async function readProjectMemorySummary(cwd: string): Promise<string> {
  const memPath = ombProjectMemoryPath(cwd);
...
function minimalOmbWorkflowFallback(): string {
  return [
    ...
    "- For other OMB workflow keywords (`ralph`, `autopilot`, `ralplan`, `ultrawork`, `ultraqa`, `switch`, `ecomode`, `cancel`), prefer the matching `omb <workflow> ...` runtime command when available.",
