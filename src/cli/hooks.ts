...
Usage:
  {cmd} hooks init       Create .omb/hooks/sample-plugin.mjs scaffold
  {cmd} hooks status     Show plugin directory + discovered plugins
  {cmd} hooks validate   Validate plugin exports/signatures
  {cmd} hooks test       Dispatch synthetic turn-complete event to plugins

Notes:
  - This command is additive. Existing \`{cmd} tmux-hook\` behavior is unchanged.
  - Plugins are enabled by default. Disable with {acronym}_HOOK_PLUGINS=0.
`);
}
...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with OMB_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OMB_HOOK_PLUGINS=0)'}`,
  );
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: omb hooks init');
    return;
  }
...
async function testHooks(): Promise<void> {
  ...
  const event = buildHookEvent('turn-complete', {
    source: 'native',
    context: {
      reason: 'omb-hooks-test',
    },
    session_id: 'omb-hooks-test',
  ...
  const logPath = join(cwd, '.omb', 'logs', `hooks-${new Date().toISOString().split('T')[0]}.jsonl`);
