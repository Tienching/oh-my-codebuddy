/**
 * omb uninstall - Remove oh-my-codebuddy configuration and installed artifacts
 */
...
async function removeHooksFile(
  ...
  const { nextContent, removedCount } = removeManagedCodebuddyHooks(existing);
  const parsed = parseCodebuddyHooksConfig(existing);
...
function printSummary(summary: UninstallSummary, dryRun: boolean): void {
  ...
  if (totalActions === 0) {
    console.log(
      "  Nothing to remove. oh-my-codebuddy does not appear to be installed.",
    );
  }
}
