...
function runOmx(
  ...
  const result = spawnSync('node', [ombBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
describe('resolveSparkShellBinaryPath', () => {
  it('prefers OMB_SPARKSHELL_BIN override', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-sparkshell-override-'));
    try {
      const binary = join(cwd, 'bin', 'custom-sparkshell');
      assert.equal(
        resolveSparkShellBinaryPath({
          cwd,
          env: { OMB_SPARKSHELL_BIN: './bin/custom-sparkshell' },
          packageRoot: '/unused',
        }),
        binary,
      );
  ...
  it('falls back from packaged binary to repo-local build artifact', () => {
    ...
    const packaged = join(packageRoot, 'bin', 'native', `${process.platform}-${process.arch}`, process.platform === 'win32' ? 'omb-sparkshell.exe' : 'omb-sparkshell');
  ...
  it('checks Linux musl packaged paths before glibc and legacy paths', () => {
    assert.deepEqual(
      ...
      [
        '/repo/bin/native/linux-x64-musl/omb-sparkshell',
        '/repo/bin/native/linux-x64-glibc/omb-sparkshell',
        '/repo/bin/native/linux-x64/omb-sparkshell',
      ],
    );
  });
  ...
  it('tries Linux musl packaged binaries before glibc fallbacks', () => {
    const packageRoot = '/repo';
    const seen: string[] = [];
    const glibcPath = '/repo/bin/native/linux-x64-glibc/omb-sparkshell';
  ...
  it('hydrates a native binary when packaged and repo-local binaries are absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-sparkshell-hydrated-'));
    try {
      ...
      const stagedBinary = join(stagingDir, process.platform === 'win32' ? 'omb-sparkshell.exe' : 'omb-sparkshell');
      ...
      const archiveName = process.platform === 'win32'
        ? 'omb-sparkshell-x86_64-pc-windows-msvc.zip'
      ...
      const buildArchive = process.platform === 'win32'
        ...
        : spawnSync('tar', ['-czf', archivePath, '-C', stagingDir, 'omb-sparkshell'], { encoding: 'utf-8' });
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          ...
          assets: [{
            ...
            arch: 'x64',
            archive: archiveName,
            binary: process.platform === 'win32' ? 'omb-sparkshell.exe' : 'omb-sparkshell',
            binary_path: process.platform === 'win32' ? 'omb-sparkshell.exe' : 'omb-sparkshell',
            sha256: checksum,
  ...
  it('falls back cleanly when hydration manifest is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-sparkshell-hydration-missing-'));
    try {
...
describe('runSparkShellBinary', () => {
  it('passes argv directly to the native sidecar', () => {
    ...
    runSparkShellBinary('/fake/omb-sparkshell', ['git', 'diff --stat', 'a|b'], {
      cwd: '/tmp/example',
      env: { TEST_ENV: '1' },
    ...
    assert.deepEqual(invoked, {
      binaryPath: '/fake/omb-sparkshell',
      args: ['git', 'diff --stat', 'a|b'],
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  });

  it('merges .omb-config.json env overrides behind explicit shell env', async () => {
    const codebuddyHome = await mkdtemp(join(tmpdir(), 'omb-sparkshell-config-env-'));
    await writeFile(join(codebuddyHome, '.omb-config.json'), JSON.stringify({
      env: {
    ...
    try {
      ...
      runSparkShellBinary('/fake/omb-sparkshell', ['git', 'status'], {
        cwd: codebuddyHome,
        env: {
          CODEBUDDY_HOME: codebuddyHome,
          OMB_DEFAULT_FRONTIER_MODEL: 'frontier-shell',
        },
      ...
      assert.equal(invokedEnv?.OMB_DEFAULT_FRONTIER_MODEL, 'frontier-shell');
      assert.equal(invokedEnv?.OMB_DEFAULT_SPARK_MODEL, 'spark-local');
    } finally {
  ...
  it('detects GLIBC symbol version failures from the native loader', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: "omb-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found\n",
        status: 1,
        signal: null,
      }),
      true,
  ...
  it('ignores non-compatibility stderr failures', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: 'omb sparkshell: summary unavailable (tmux failed)\n',
        status: 1,
        signal: null,
      }),
      false,
...
describe('omb sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    ...
    try {
      ...
      assert.match(result.stdout, /omb explore\s+Default read-only exploration entrypoint \(may adaptively use sparkshell backend\)/);
      assert.match(result.stdout, /omb sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /omb sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    ...
    try {
      ...
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: omb sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: omb sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('preserves child stdout, stderr, and exit code through the JS bridge', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-sparkshell-bridge-'));
    try {
      const binDir = join(cwd, 'bin');
      const stubPath = join(binDir, process.platform === 'win32' ? 'omb-sparkshell.cmd' : 'omb-sparkshell');
  ...
  it('falls back to raw execution when the packaged native binary is GLIBC-incompatible', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-sparkshell-glibc-fallback-'));
    try {
      const testDir = dirname(fileURLToPath(import.meta.url));
      const repoRoot = join(testDir, '..', '..', '..');
      ...
      const binDir = join(cacheDir, packageJson.version, `${process.platform}-${process.arch}`, 'omb-sparkshell');
      await mkdir(binDir, { recursive: true });
      const stubPath = join(binDir, process.platform === 'win32' ? 'omb-sparkshell.exe' : 'omb-sparkshell');
      await writeFile(
        stubPath,
        process.platform === 'win32'
          ? '@echo off\r\n>&2 echo omb-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39\' not found\r\nexit /b 1\r\n'
