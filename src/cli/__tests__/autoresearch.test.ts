...
async function initRepo(): Promise<string> {
  const raw = await mkdtemp(join(tmpdir(), 'omb-autoresearch-test-'));
...
describe('omb autoresearch', () => {
  ...
  it('resolves guided deep-interview artifacts by seeded slug even when file mtimes predate launch timestamp', async () => {
    ...
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-deep-interview-mtime-bin-'));
    try {
...
mkdir -p "$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch"
cat >"$OMB_TEST_REPO_ROOT/.omb/specs/deep-interview-autoresearch-test-launch.md" <<'EOF'
...
- launch
EOF
cat >"$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/mission.md" <<'EOF'
# Mission

Investigate flaky onboarding behavior
EOF
cat >"$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/sandbox.md" <<'EOF'
---
evaluator:
...
cat >"$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/result.json" <<'EOF'
{
  ...
  "draftArtifactPath": "${repo}/.omb/specs/deep-interview-autoresearch-test-launch.md",
  "missionArtifactPath": "${repo}/.omb/specs/autoresearch-test-launch/mission.md",
  "sandboxArtifactPath": "${repo}/.omb/specs/autoresearch-test-launch/sandbox.md",
  "launchReady": true,
  "blockedReasons": []
}
EOF
touch -t 202603180000 "$OMB_TEST_REPO_ROOT/.omb/specs/deep-interview-autoresearch-test-launch.md"
touch -t 202603180000 "$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/mission.md"
touch -t 202603180000 "$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/sandbox.md"
touch -t 202603180000 "$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch/result.json"
`,
        'utf-8',
      );
  ...
  it('launches interactive deep-interview intake, materializes mission files, and then prefers split-pane handoff', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-deep-interview-bin-'));
    try {
...
mkdir -p "$OMB_TEST_REPO_ROOT/.omb/specs/deep-int"
mkdir -p "$OMB_TEST_REPO_ROOT/.omb/specs/autoresearch-test-launch"
cat >"$OMB_TEST_REPO_ROOT/.omb/specs/deep-interview-autoresearch-test-launch.md" <<'EOF'
# Deep Interview Autoresearch Draft — test-launch
...
`,
      ...
      const codebuddyArgs = await readFile(codexLog, 'utf-8');
      const tmuxOutput = await readFile(tmuxLog, 'utf-8');
      assert.match(codebuddyArgs, /\$deep-interview --autoresearch/);
  ...
  it('uses split-window launch for explicit run inside tmux while preserving the interview pane', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-run-split-bin-'));
    try {
...
`,
  ...
  it('falls back to foreground execution when tmux split-window fails', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-run-fallback-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const logsRoot = join(repo, '.omb', 'logs', 'autoresearch');
  ...
  it('rejects mission directories outside a git repo', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-outside-git-'));
    try {
  ...
  it('rejects autoresearch launch when root ralph mode is already active', async () => {
    ...
    try {
      ...
      await mkdir(join(repo, '.omb', 'state'), { recursive: true });
      await writeFile(
        join(repo, '.omb', 'state', 'ralph-state.json'),
        JSON.stringify({
          active: true,
          mode: 'ralph',
          iteration: 0,
      ...
      const worktreesRoot = join(repo, '.omb', 'worktrees');
...
`,
  ...
  it('stops after repeated noop turns', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-noop-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const state = JSON.parse(await readFile(join(repo, '.omb', 'state', 'autoresearch-state.json'), 'utf-8')) as {
        active: boolean;
      };
