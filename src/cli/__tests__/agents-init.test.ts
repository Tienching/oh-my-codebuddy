...
describe('omb agents-init', () => {
  it('creates a managed root AGENTS.md plus direct-child AGENTS.md files while skipping ignored directories', async () => {
    ...
    try {
      ...
      assert.match(rootAgents, /OMB:AGENTS-INIT:MANAGED/);
      ...
      assert.match(rootAgents, /# oh-my-codebuddy - Intelligent Multi-Agent Orchestration/);
  ...
  it('skips unmanaged AGENTS.md files by default but can adopt them with --force and a backup', async () => {
    ...
    try {
      ...
      assert.match(adopted, /OMB:AGENTS-INIT:MANAGED/);
      const backupRoot = join(wd, '.omb', 'backups', 'agents-init');
  ...
  it('protects project-root AGENTS.md during an active OMB session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-agents-init-'));
    try {
