...
function runOmx(
  ...
  const nodeWrapper = join(cwd, '.omb-test-node.sh');
  if (!existsSync(nodeWrapper)) {
    writeFileSync(nodeWrapper, '#!/bin/sh\nexec node "$@"\n');
    chmodSync(nodeWrapper, 0o755);
  }
  const r = spawnSync(nodeWrapper, [ombBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
describe('loadExplorePrompt', () => {
  it('reads prompt file content', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-prompt-'));
    try {
...
describe('resolvePackagedExploreHarnessCommand', () => {
  it('uses a packaged native binary when metadata matches the current platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-packaged-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
      await writeFile(join(binDir, 'omb-explore-harness.meta.json'), JSON.stringify({
  ...
  it('ignores packaged binaries built for a different platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-packaged-mismatch-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
...
describe('resolveExploreHarnessCommand', () => {
  it('uses env override when provided', () => {
    const resolved = resolveExploreHarnessCommand('/repo', { OMB_EXPLORE_BIN: '/tmp/omb-explore-stub' } as NodeJS.ProcessEnv);
    assert.deepEqual(resolved, { command: '/tmp/omb-explore-stub', args: [] });
  });

  it('prefers a packaged native harness binary when present', async () => {
    await withPackagedExploreHarnessLock(async () => {
      const wd = await mkdtemp(join(tmpdir(), 'omb-explore-native-'));
      try {
        const binDir = join(wd, 'bin');
        await mkdir(binDir, { recursive: true });
        await writeFile(join(wd, 'package.json'), '{}\n');
  ...
  it('uses an existing repo-built native harness before cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-target-'));
    try {
      ...
      await mkdir(join(wd, 'crates', 'omb-explore'), { recursive: true });
      await writeFile(join(wd, 'crates', 'omb-explore', 'Cargo.toml'), '[package]\nname="omb-explore-harness"\nversion="0.0.0"\n');
  ...
  it('builds cargo fallback command otherwise', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-fallback-'));
    try {
      const crateDir = join(wd, 'crates', 'omb-explore');
      ...
      assert.ok(resolved.args.includes(join(wd, 'crates', 'omb-explore', 'Cargo.toml')));
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('hydrates a native harness for packaged installs before attempting cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-hydrated-'));
    try {
      ...
      const archivePath = join(assetRoot, 'omb-explore-harness-x86_64-unknown-linux-musl.tar.gz');
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          version: '0.8.15',
          assets: [{
            product: 'omb-explore-harness',
            version: '0.8.15',
            platform: 'linux',
            arch: 'x64',
            archive: 'omb-explore-harness-x86_64-unknown-linux-musl.tar.gz',
            binary: 'omb-explore-harness',
            binary_path: 'omb-explore-harness',
            sha256: checksum,
            size: archiveBuffer.length,
            download_url: `${server.baseUrl}/omb-explore-harness-x86_64-unknown-linux-musl.tar.gz`,
          }],
        }, null, 2));
  ...
  it('reports a clean fallback error when the native manifest is unavailable for packaged installs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-missing-manifest-'));
    try {
      await writeFile(join(wd, 'package.json'), JSON.stringify({
        version: '0.8.15',
        repository: { url: 'git+https://github.com/Tienching/oh-my-codebuddy.git' },
...
describe('buildExploreHarnessArgs', () => {
  it('includes cwd, prompt, prompt contract, and constrained model settings', () => {
    const args = buildExploreHarnessArgs('find auth', '/repo', {
      OMB_EXPLORE_SPARK_MODEL: 'spark-model',
...
describe('exploreCommand', () => {
  it('routes qualifying read-only shell commands through sparkshell instead of the direct harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-sparkshell-route-'));
    try {
  ...
  it('falls back to the explore harness when sparkshell backend is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-sparkshell-fallback-'));
    try {
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
        harnessStub,
  ...
  it('falls back to the explore harness when sparkshell is GLIBC-incompatible', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-sparkshell-glibc-'));
    try {
      const sparkshellStub = join(wd, 'sparkshell-stub.sh');
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
        sparkshellStub,
        "#!/bin/sh\necho \"omb-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version \\`GLIBC_2.39' not found\" 1>&2\nexit 1\n",
      );
      await writeFile(
        harnessStub,
        '#!/bin/sh\nprintf "%s\\n" "# Answer" "- fallback harness recovered the lookup"\n',
  ...
  it('passes prompt to harness and preserves markdown stdout', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-cmd-'));
    try {
      ...
      const originalEnv = process.env.OMB_EXPLORE_BIN;
      process.env.OMB_EXPLORE_BIN = stub;
      const originalCwd = process.cwd();
      process.chdir(wd);
      try {
        ...
        process.chdir(originalCwd);
        if (originalEnv === undefined) delete process.env.OMB_EXPLORE_BIN;
        else process.env.OMB_EXPLORE_BIN = originalEnv;
        process.stdout.write = originalStdout;
        process.stderr.write = originalStderr;
      }
  ...
  it('works end-to-end through omb explore', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-e2e-'));
    try {
      const stub = join(wd, 'explore-stub.sh');
      await writeFile(
        stub,
  ...
  it('launches an env-node codex binary while keeping model shell commands allowlisted', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-harness-e2e-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        ...
        assert.match(captured, /PATH=.*omb-explore-allowlist-/);
        assert.match(captured, /SHELL=.*omb-explore-allowlist-.*\/bin\/bash$/m);
  ...
  it('supports --prompt-file end-to-end with the harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-harness-prompt-file-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const capturePath = join(wd, 'capture.json');
        const codexStub = await writeEnvNodeCodexStub(wd, capturePath);
  ...
  it('preserves must-preserve facts in a long noisy summary fixture', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-fidelity-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '%s\n' '# Answer' '## Critical facts' '- MUST: summary mode stayed read-only' '- MUST: blocked command stayed node --version' '- MUST: next command is omb team status <team-name>' '' '## Noise'
i=0
while [ "$i" -lt 80 ]; do
  printf '%s\n' "- distractor line $i"
  i=$((i + 1))
...
`,
        ...
        assert.match(result.stdout, /MUST: next command is omb team status <team-name>/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('preserves buried critical facts in adversarial noisy output', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-adversarial-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
...
`,
  ...
  it('falls back after spark failure and preserves actionable stderr guidance', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-fallback-success-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omb explore] spark model \`%s\` unavailable or failed (exit 17). Falling back to \`gpt-5.4\`.\n' "\${OMB_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omb explore] spark stderr: spark timed out; retry with the frontier fallback\n' >&2
...
`,
        ...
        const result = await runExploreCommandForTest(wd, ['--prompt', 'validate fallback recovery'], {
          OMB_EXPLORE_BIN: harnessStub,
          OMB_EXPLORE_SPARK_MODEL: 'spark-test-model',
        });
  ...
  it('reports both failed attempts with codes and final actionable stderr end-to-end', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-explore-fallback-failure-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omb explore] spark model \`%s\` unavailable or failed (exit 23). Falling back to \`gpt-5.4\`.\n' "\${OMB_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omb explore] spark stderr: spark backend unavailable; install the fallback runtime\n' >&2
printf '[omb explore] both spark (\`%s\`) and fallback (\`gpt-5.4\`) attempts failed (codes 23 / 29). Last stderr: fallback backend unavailable; set OMB_EXPLORE_BIN to a working harness\n' "\${OMB_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
exit 1
`,
        );
        ...
        assert.match(
          result.stderr,
          /both spark \(`spark-test-model`\) and fallback \(`gpt-5\.4`\) attempts failed \(codes 23 \/ 29\)\. Last stderr: fallback backend unavailable; set OMB_EXPLORE_BIN to a working harness/,
        );
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
