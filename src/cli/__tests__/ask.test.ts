...
describe('omb ask', () => {
  it('script usage documents provider-specific long flags from CLI help', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-ask-usage-'));
    try {
  ...
  it('uses package-root advisor script path from non-package cwd and still writes artifact', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-ask-nonroot-'));
    try {
      ...
      assert.ok(artifactPath.startsWith(join(wd, '.omb', 'artifacts', 'claude-')));
  ...
  it('supports claude --print and gemini --prompt end-to-end through omb ask', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-ask-provider-flags-'));
    try {
      const fakeBin = join(wd, 'bin');
      await mkdir(fakeBin, { recursive: true });
  ...
  it('injects --agent-prompt content into final prompt while keeping Original task raw', async () => {
    ...
    try {
      const fakeBin = join(wd, 'bin');
      const codebuddyHome = join(wd, '.codex-home');
  ...
  it('fails clearly when --agent-prompt role is missing from prompts directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-ask-agent-prompt-missing-'));
    try {
      const fakeBin = join(wd, 'bin');
      const codebuddyHome = join(wd, '.codex-home');
      const promptsDir = join(codebuddyHome, 'prompts');
