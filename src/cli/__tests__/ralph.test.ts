...
const approvedHint: ApprovedExecutionLaunchHint = {
  mode: 'ralph',
  command: 'omb ralph "Execute approved issue 1072 plan"',
  task: 'Execute approved issue 1072 plan',
  sourcePath: '.omb/plans/prd-issue-1072.md',
  testSpecPaths: ['.omb/plans/test-spec-issue-1072.md'],
  deepInterviewSpecPaths: ['.omb/specs/deep-interview-issue-1072.md'],
};
...
describe('ralph deslop launch wiring', () => {
  ...
  it('documents changed-files-only deslop guidance by default', () => {
    const instructions = buildRalphAppendInstructions('fix issue 920', {
      changedFilesPath: '.omb/ralph/changed-files.txt',
      noDeslop: false,
      approvedHint: null,
    });
  ...
  it('includes approved plan and deep-interview handoff context when available', () => {
    ...
    assert.match(instructions, /approved plan: \.omb\/plans\/prd-issue-1072\.md/i);
    assert.match(instructions, /test specs: \.omb\/plans\/test-spec-issue-1072\.md/i);
    assert.match(instructions, /deep-interview specs: \.omb\/specs\/deep-interview-issue-1072\.md/i);
