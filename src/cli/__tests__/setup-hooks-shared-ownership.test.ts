...
function countManagedHooks(entries: HookRegistration[] | undefined): number {
  return hookCommands(entries).filter((command) => command.includes("codebuddy-native-hook.js")).length;
}
...
describe("omx setup/uninstall shared ownership for native hooks", () => {
  ...
  it("setup preserves user hooks while deduping stale OMX wrappers", async () => {
    ...
    try {
      ...
      assert.ok(
        hookCommands(refreshed.hooks?.UserPromptSubmit).some((command) => command.includes("codebuddy-native-hook.js")),
