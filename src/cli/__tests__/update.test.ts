...
describe('maybeCheckAndPromptUpdate', () => {
  ...
  it('runs setup refresh after a successful auto-update', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-update-'));
  ...
  it('does not refresh setup when the global update fails', async () => {
    ...
    try {
      ...
      assert.equal(setupCalls, 0);
      assert.match(logs.join('\n'), /Update failed\. Run manually: npm install -g oh-my-codebuddy@latest/);
    } finally {
      console.log = originalLog;
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('skips the update flow in degraded non-interactive environments', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-update-'));
    const originalStdinTty = process.stdin.isTTY;
    const originalStdoutTty = process.stdout.isTTY;
    let fetchCalls = 0;
