...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-guided-test-'));
...
describe('parseInitArgs', () => {
  ...
  it('sanitizes slug via slugifyMissionName', () => {
    const result = parseInitArgs(['--slug', '../../etc/cron.d/omb']);
...
describe('checkTmuxAvailable', () => {
  ...
  it('launches background tmux sessions with an absolute omb entry path even from a relative launcher', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-autoresearch-guided-bin-'));
    const startupCwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-guided-start-'));
...
`,
      ...
      assert.match(tmuxOutput, /new-session -d -s omb-autoresearch-demo/);
      assert.match(tmuxOutput, new RegExp(escapeRegExp(launcherPath)));
      assert.doesNotMatch(tmuxOutput, /dist\/cli\/omb\.js autoresearch/);
    } finally {
...
describe('autoresearch intake draft artifacts', () => {
  it('writes a canonical deep-interview autoresearch draft artifact from vague input', async () => {
    ...
    try {
      ...
      assert.match(artifact.path, /\.omb\/specs\/deep-interview-autoresearch-improve-onboarding-for-first-time-contributors\.md$/);
...
describe('runAutoresearchNoviceBridge', () => {
  it('loops through refine further before launching and writes draft + mission files', async () => {
    ...
    try {
      ...
      const draftContent = await readFile(join(repo, '.omb', 'specs', 'deep-interview-autoresearch-ux-eval.md'), 'utf-8');
      const resultContent = await readFile(join(repo, '.omb', 'specs', 'autoresearch-ux-eval', 'result.json'), 'utf-8');
      const missionContent = await readFile(join(result.missionDir, 'mission.md'), 'utf-8');
      const sandboxContent = await readFile(join(result.missionDir, 'sandbox.md'), 'utf-8');
  ...
  it('uses seeded novice inputs while still requiring confirmation-driven launch', async () => {
    ...
    try {
      ...
      const draftContent = await readFile(join(repo, '.omb', 'specs', 'deep-interview-autoresearch-seeded-topic.md'), 'utf-8');
