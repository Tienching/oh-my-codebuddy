...
function buildDescriptorHelp(descriptor: McpCliDescriptor): string {
  ...
  return [
    `Usage: omb ${descriptor.commandName} <tool-name> [--input <json>] [--json]`,
    "",
    descriptor.title,
    "",
    "Available tools:",
    ...
    "",
    "Examples:",
    `  omb ${descriptor.commandName} ${descriptor.tools[0]?.name ?? "<tool>"} --input '{}' --json`,
  ].join("\n");
}
...
async function loadStateDescriptor(): Promise<McpCliDescriptor> {
  ...
  return {
    commandName: "state",
    title: "CLI parity surface for OMB state MCP tools.",
...
async function loadTraceDescriptor(): Promise<McpCliDescriptor> {
  ...
  return {
    commandName: "trace",
    title: "CLI parity surface for OMB trace MCP tools.",
...
async function loadCodeIntelDescriptor(): Promise<McpCliDescriptor> {
  ...
  return {
    ...
    title: "CLI parity surface for OMB code-intel MCP tools.",
  ...
  switch (commandName) {
    ...
    case "notepad":
      await runDescriptorCommand(
        args,
        async () => await loadMemoryDescriptor("notepad", "notepad_", "CLI parity surface for OMB notepad MCP tools."),
      );
      return;
    case "project-memory":
      await runDescriptorCommand(
        ...
        async () => await loadMemoryDescriptor(
          "project-memory",
          "project_memory_",
          "CLI parity surface for OMB project-memory MCP tools.",
        ),
      );
      return;
    case "trace":
