...
const DEFAULT_CONFIG: TmuxHookConfig = {
  ...
  prompt_template: 'Continue from current mode state. [OMB_TMUX_INJECT]',
  marker: '[OMB_TMUX_INJECT]',
  dry_run: false,
  log_level: 'info',
...
Usage:
  {cmd} tmux-hook init       Create .omb/tmux-hook.json
  {cmd} tmux-hook status     Show config + runtime state summary
  {cmd} tmux-hook validate   Validate config and tmux target reachability
  {cmd} tmux-hook test       Run a synthetic notify-hook turn (end-to-end)
`);
}
...
function ombDir(cwd = process.cwd()): string {
  return join(cwd, '.omb');
}

function tmuxHookConfigPath(cwd = process.cwd()): string {
  return join(ombDir(cwd), 'tmux-hook.json');
}

function tmuxHookStatePath(cwd = process.cwd()): string {
  return join(ombDir(cwd), 'state', 'tmux-hook-state.json');
}
...
async function readValidatedConfig(cwd = process.cwd()): Promise<TmuxHookConfig> {
  ...
  if (!existsSync(configPath)) {
    throw new Error('tmux-hook config missing. Run: omb tmux-hook init');
  }
  ...
  if (!existsSync(configPath)) {
    ...
    if (initResult.created) {
      ...
      if (initResult.usedPlaceholderTarget) {
        console.log('Could not auto-detect a tmux target. Edit `.omb/tmux-hook.json` when ready.');
...
async function initTmuxHookConfig(opts?: { silent?: boolean; cwd?: string }): Promise<InitConfigResult> {
  ...
  const configPath = tmuxHookConfigPath(cwd);
  await mkdir(ombDir(cwd), { recursive: true });
...
async function testTmuxHook(args: string[]): Promise<void> {
  ...
  const payload = {
    ...
    'thread-id': threadId,
    'turn-id': turnId,
    'input-messages': ['omb tmux-hook test'],
  ...
  console.log('Check: .omb/logs/tmux-hook-YYYY-MM-DD.jsonl for skip/reason codes.');
}
