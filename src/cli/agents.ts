...
import { codebuddyAgentsDir, projectCodebuddyAgentsDir } from '../utils/paths.js';

export const RESERVED_NATIVE_AGENT_NAMES = new Set(['default', 'worker', 'explorer']);
const DEFAULT_AGENT_MODEL = 'gpt-5.4';
const AGENTS_USAGE = [
  'Usage:',
  '  omb agents list [--scope user|project]',
  '  omb agents add <name> [--scope user|project] [--force]',
  '  omb agents edit <name> [--scope user|project]',
  '  omb agents remove <name> [--scope user|project] [--force]',
  '',
  'Manage CodeBuddy native agent TOML files under ~/.codebuddy/agents/ or ./.codebuddy/agents/.',
  '',
  'Notes:',
...
function normalizeAgentName(name: string): string {
  ...
  if (isReservedNativeAgentName(trimmed)) {
    throw new Error(`"${trimmed}" is reserved by CodeBuddy built-in agents`);
  }
  return trimmed;
}

function resolveAgentsDir(scope: AgentScope, cwd = process.cwd()): string {
  return scope === 'project' ? projectCodebuddyAgentsDir(cwd) : codebuddyAgentsDir();
}
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.omb', 'setup-scope.json');
  ...
  return existsSync(join(cwd, '.codebuddy')) || existsSync(join(cwd, '.codex')) ? 'project' : 'user';
}
...
function scaffoldAgentToml(name: string): string {
  ...
  return [
    `# CodeBuddy native agent: ${normalized}`,
    `name = "${normalized}"`,
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: omb agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: omb agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: omb agents remove <name>');
