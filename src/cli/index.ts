...
export function resolveCodexHomeForLaunch(
  ...
  if (env.CODEBUDDY_HOME && env.CODEBUDDY_HOME.trim() !== "") {
...
function runCodexBlocking(
  ...
  if (result.error) {
    ...
    if (kind === "missing") {
      console.error(
        "[omb] failed to launch codebuddy: executable not found in PATH",
      );
    } else if (kind === "blocked") {
      console.error(
        `[omb] failed to launch codebuddy: executable is present but blocked in the current environment (${errno.code || "blocked"})`,
      );
    } else {
...
export async function launchWithHud(args: string[]): Promise<void> {
  if (isNativeWindows()) {
    ...
    if (result.error) {
      ...
      if (kind === "missing") {
        console.warn(
          "[omb] warning: tmux was not found on native Windows. Continuing without tmux/HUD.\n" +
        ...
        console.warn(
          `[omb] warning: tmux probe failed on native Windows (${errno.code || errno.message}). Continuing without tmux/HUD.`,
        );
      }
    } else if (result.status !== 0 && !isTmuxAvailable()) {
      const stderr = (result.stderr || "").trim();
      console.warn(
        `[omb] warning: tmux reported an error on native Windows${stderr ? ` (${stderr})` : ""}. Continuing without tmux/HUD.`,
      );
    }
  }
  ...
  try {
    ...
    if (repaired) {
      console.log("[omb] Repaired managed config.toml compatibility issue.");
    }
  } catch {
  ...
  try {
    ...
    console.error(
      `[omb] preLaunch warning: ${err instanceof Error ? err.message : err}`,
    );
  }
...
export async function execWithOverlay(args: string[]): Promise<void> {
  ...
  try {
    ...
    const codexEnvBase = codexHomeOverride
      ? {
          ...process.env,
          CODEBUDDY_HOME: codexHomeOverride,
    ...
    const codexEnv = notifyTempContractRaw
      ? {
          ...codexEnvBase,
          [OMB_NOTIFY_TEMP_CONTRACT_ENV]: notifyTempContractRaw,
        }
      : codexEnvBase;
...
function parseLegacyConfigOverride(
  rawValue: string,
):
  ...
  if (key === MODEL_INSTRUCTIONS_FILE_KEY) {
    return { kind: "system-prompt-file", value };
  }
  if (key === REASONING_KEY && REASONING_MODE_SET.has(value)) {
    return { kind: "reasoning", value: value as ReasoningMode };
  }
  return null;
}
...
export function normalizeCodexLaunchArgs(args: string[]): string[] {
  ...
  for (let index = 0; index < launchPolicyParsed.remainingArgs.length; index += 1) {
    const arg = launchPolicyParsed.remainingArgs[index];

    if (
      arg === MADMAX_FLAG ||
      arg === YOLO_FLAG ||
      arg === CODEBUDDY_LEGACY_BYPASS_FLAG
    ) {
      wantsBypass = true;
      continue;
    }
    ...
    if (arg === CONFIG_FLAG || arg === LONG_CONFIG_FLAG) {
      ...
      if (typeof next === "string") {
        ...
        if (translated?.kind === "system-prompt-file") {
          normalized.push(CODEBUDDY_SYSTEM_PROMPT_FILE_FLAG, translated.value);
          index += 1;
          continue;
        }
        if (translated?.kind === "reasoning") {
    ...
    if (arg.startsWith(`${LONG_CONFIG_FLAG}=`)) {
      const translated = parseLegacyConfigOverride(
  ...
  if (wantsBypass && !hasBypass && !hasExplicitBypassPermissionMode(normalized)) {
    normalized.push(CODEBUDDY_BYPASS_FLAG);
  }

  if (reasoningMode) {
    normalized.push(CODEBUDDY_EFFORT_FLAG, reasoningMode);
  }
...
function resolveModelInstructionsFilePath(
  ...
  return (
    env[OMB_MODEL_INSTRUCTIONS_FILE_ENV] ||
...
export function injectModelInstructionsBypassArgs(
  ...
  return [
    ...args,
    CODEBUDDY_SYSTEM_PROMPT_FILE_FLAG,
    resolveModelInstructionsFilePath(cwd, env, defaultFilePath),
  ];
}
...
export function buildDetachedSessionBootstrapSteps(
  ...
  const newSessionArgs: string[] = [
    ...
    ...(workerLaunchArgs
      ...
      : []),
    ...(sessionId
      ? ["-e", `OMB_SESSION_ID=${sessionId}`, "-e", `OMX_SESSION_ID=${sessionId}`]
      : []),
    ...(codexHomeOverride
      ? [
          "-e",
      ...
      : []),
    ...(notifyTempContractRaw
      ? ["-e", `${OMB_NOTIFY_TEMP_CONTRACT_ENV}=${notifyTempContractRaw}`]
      : []),
    detachedLeaderCmd,
  ];
...
export function buildNotifyFallbackWatcherEnv(
  ...
  return {
    ...
    ...(options.codexHomeOverride
      ? {
          CODEBUDDY_HOME: options.codexHomeOverride,
          CODEX_HOME: options.codexHomeOverride,
        }
      : {}),
    ...(options.sessionId
      ? { OMB_SESSION_ID: options.sessionId, OMX_SESSION_ID: options.sessionId }
      : {}),
  ...
  for (const stateDir of scopedDirs) {
    ...
    for (const file of files) {
      ...
      if (result.kind !== "ok") {
        if (result.kind === "recoverable") {
          try {
            ...
            writeWarn(
              `[omb] postLaunch: failed to recover mode state ${path}: ${err instanceof Error ? err.message : err}`,
            );
          }
        } else if (result.kind === "malformed") {
          writeWarn(
            `[omb] postLaunch: skipped malformed mode state ${path}: ${result.message}`,
          );
        }
        continue;
      }
      ...
      try {
        ...
        writeWarn(
          `[omb] postLaunch: failed to update mode state ${path}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
...
export async function reapPostLaunchOrphanedMcpProcesses(
  ...
  try {
    ...
    if (result.terminatedCount > 0) {
      writeInfo(
        `[omb] postLaunch: reaped ${result.terminatedCount} orphaned OMX MCP process(es).`,
      );
    }
    if (result.failedPids.length > 0) {
      writeWarn(
        `[omb] postLaunch: failed to reap ${result.failedPids.length} orphaned OMX MCP process(es); continuing cleanup.`,
      );
    }
  } catch (err) {
...
async function preLaunch(
  ...
  try {
    ...
    if (cleanup.terminatedCount > 0) {
      console.log(
        `[omb] Reaped ${cleanup.terminatedCount} orphaned OMX MCP process(es) before launch.`,
      );
    }
    if (cleanup.failedPids.length > 0) {
      console.warn(
        `[omb] Failed to reap ${cleanup.failedPids.length} orphaned OMX MCP process(es); continuing launch.`,
      );
    }
  } catch (err) {
    process.stderr.write(`[cli/index] operation failed: ${err}\n`);
...
${launchAppendix}`
  ...
  try {
    if (notifyTempContract?.active) {
      process.env[OMB_NOTIFY_TEMP_CONTRACT_ENV] =
      ...
      for (const info of startup.infoLines) {
        console.log(`[omb] ${info}`);
      }
      for (const warning of startup.warningLines) {
        console.warn(`[omb] ${warning}`);
      }
    } else {
      delete process.env[OMB_NOTIFY_TEMP_CONTRACT_ENV];
    }
...
function runCodex(
  ...
  const codexEnvWithNotify = notifyTempContractRaw
    ? { ...codexEnv, [OMB_NOTIFY_TEMP_CONTRACT_ENV]: notifyTempContractRaw }
    : codexEnv;
  ...
  if (launchPolicy === "inside-tmux") {
    // Already in tmux: launch CodeBuddy in current pane, HUD in bottom split
    ...
    // Not in tmux: create a new tmux session with CodeBuddy + HUD pane
    const codexCmd = buildTmuxPaneCommand(launchBinary, launchArgs);
    const detachedWindowsCodexCmd = nativeWindows
      ? buildWindowsPromptCommand(launchBinary, launchArgs)
      : null;
    ...
    try {
      ...
      // tmux not available or failed, just run CodeBuddy directly
      runCodexBlocking(cwd, launchBinary, launchArgs, codexEnvWithNotify);
    }
  }
}
...
async function postLaunch(
  ...
  // 0. Flush fallback watcher once to reduce race with fast CodeBuddy exit.
  try {
  ...
  try {
    ...
    console.error(
      `[omb] postLaunch: model instructions cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omb] postLaunch: session archive failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omb] postLaunch: mode cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    const errorSummary =
      normalizedEvent === "failed"
        ? `codebuddy exited with code ${process.exitCode}`
        : undefined;
...
export async function reapStaleNotifyFallbackWatcher(
  ...
  try {
    ...
    if (!hasErrnoCodeImpl(error, "ESRCH")) {
      warn(
        "[omb] warning: failed to stop stale notify fallback watcher",
        {
          path: pidPath,
...
async function startNotifyFallbackWatcher(
  ...
  await mkdir(join(cwd, ".omx", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omb] warning: failed to create notify fallback watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  try {
    ...
    console.warn("[omb] warning: failed to launch notify fallback watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omb] warning: failed to write notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function startHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  if (existsSync(pidPath)) {
    try {
      ...
      console.warn("[omb] warning: failed to stop stale hook-derived watcher", {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  await mkdir(join(cwd, ".omx", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omb] warning: failed to create hook-derived watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  try {
    ...
    console.warn("[omb] warning: failed to launch hook-derived watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omb] warning: failed to write hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopNotifyFallbackWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    if (!hasErrnoCode(error, "ESRCH")) {
      console.warn(
        "[omb] warning: failed to stop notify fallback watcher process",
        {
          path: pidPath,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omb] warning: failed to remove notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[omb] warning: failed to stop hook-derived watcher process", {
      path: pidPath,
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omb] warning: failed to remove hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
