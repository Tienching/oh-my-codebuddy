...
const TEAM_HELP = `
Usage: omb team [N:agent-type] "<task description>"
       omb team status <team-name> [--json] [--tail-lines <100-1000>]
       omb team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]
...
const TEAM_API_HELP = `
Usage: omb team api <operation> [--input <json>] [--json]
       omb team api <operation> --help

Supported operations:
  ${TEAM_API_OPERATIONS.join('\n  ')}

Examples:
  omb team api list-tasks --input '{"team_name":"my-team"}' --json
  omb team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1","expected_version":1}' --json
`;
...
${required}${optional}${note}Example:
  omb team api ${operation} --input '${sampleInputJson}' --json
`.trim();
}
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (token === '--tail-lines') {
      ...
      if (!Number.isFinite(parsed) || parsed < MIN_SPARKSHELL_TAIL_LINES || parsed > MAX_SPARKSHELL_TAIL_LINES) {
        throw new Error(`Usage: omb team status <team-name> [--json] [--tail-lines <${MIN_SPARKSHELL_TAIL_LINES}-${MAX_SPARKSHELL_TAIL_LINES}>]`);
      }
      return parsed;
    }
...
function parseTeamApiArgs(args: string[]): {
  ...
  if (!operation) {
    throw new Error(`Usage: omb team api <operation> [--input <json>] [--json]\nSupported operations: ${TEAM_API_OPERATIONS.join(', ')}`);
  }
...
function renderTeamPaneStatus(
  ...
  if (paneStatus.sparkshell_hint) {
    console.log('sparkshell_hint: omb sparkshell --tmux-pane <pane-id> --tail-lines 400');
  }
...
function parseTeamArgs(args: string[], cwd: string = process.cwd()): ParsedTeamArgs {
  ...
  if (!task) {
    throw new Error('Usage: omb team [N:agent-type] "<task description>"');
  }
...
export interface TeamExecutionTask {
  subject: string;
  description: string;
  owner: string;
  role?: string;
...
export interface TeamExecutionPlanMetadata {
  strategy: DecompositionStrategy;
...
export interface TeamExecutionPlan {
  ...
  metadata: TeamExecutionPlanMetadata;
}
  ...
  const tasksWithRoles = subtasks.map((st) => {
    if (explicitAgentType) {
      return {
        ...st,
        role: agentType,
        route_confidence: 'high' as const,
        route_reason: 'explicit agent type override preserved for backward compatibility',
      };
    }
...
export function buildLeaderMonitoringHints(teamName: string): string[] {
  ...
  return [
    `leader_check: omb team status ${sanitized}`,
    `leader_loop_hint: while ON, keep checking state (example: sleep 30 && omb team status ${sanitized})`,
  ];
}
...
export async function teamCommand(args: string[], _options: TeamCliOptions = {}): Promise<void> {
  ...
  if (subcommand === 'api') {
    ...
    const interopCommandBrand = resolveInteropCommandBrand();
    ...
    try {
      ...
      if (wantsJson) {
        console.log(JSON.stringify({
          ...jsonBase,
          ok: false,
          command: `${interopCommandBrand} team api`,
          operation: 'unknown',
          error: {
            code: 'invalid_input',
    ...
    if (parsedApi.json) {
      console.log(JSON.stringify({
        ...jsonBase,
        command: `${interopCommandBrand} team api ${parsedApi.operation}`,
        ...envelope,
      }));
  ...
  if (subcommand === 'status') {
    ...
    const wantsJson = teamArgs.includes('--json');
    if (!name) throw new Error('Usage: omb team status <team-name> [--json]');
    ...
    if (!snapshot) {
      if (wantsJson) {
        console.log(JSON.stringify({
          ...buildJsonBase(),
          command: resolveTeamStatusCommandLabel(),
          team_name: name,
          status: 'missing',
        }));
        return;
  ...
  if (subcommand === 'await') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omb team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]');
    const wantsJson = teamArgs.includes('--json');
  ...
  if (subcommand === 'resume') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omb team resume <team-name>');
  ...
  if (subcommand === 'shutdown') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omb team shutdown <team-name> [--force] [--confirm-issues]');
    ...
    await updateModeState('team', {
      ...
      console.warn('[omb] warning: failed to persist team mode shutdown state', {
        team: name,
