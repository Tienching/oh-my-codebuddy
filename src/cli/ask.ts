...
function resolveAskPromptsDir(cwd: string, env: NodeJS.ProcessEnv = process.env): string {
  const codebuddyHomeOverride = env.CODEBUDDY_HOME?.trim();
  if (codebuddyHomeOverride) {
    return join(codebuddyHomeOverride, 'prompts');
  }
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "${formatCliText('{cmd} setup')}" to install prompts.`);
  }
