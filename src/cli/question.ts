...
export const QUESTION_HELP = formatCliText(`{cmd} question - OMB-owned blocking user question entrypoint

Usage:
  {cmd} question --input '<json>' [--json]
  {cmd} question --ui --state-path <absolute-or-relative-record-path>
...
Options:
  ...
  --ui                 Internal renderer mode; renders the OMB question UI for an existing state record
  --state-path <path>  Question record path used by --ui mode

Input schema:
  {
    "header": "Optional short heading",
    "question": "What should OMB do next?",
    "options": [
...
export async function questionCommand(args: string[]): Promise<void> {
  ...
  if (!parsed.input) throw new Error(formatCliText('{cmd} question requires --input in normal mode'));
