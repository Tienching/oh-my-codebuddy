...
export type NativeProduct = 'omb-explore-harness' | 'omb-sparkshell';
...
const NATIVE_AUTO_FETCH_ENV = 'OMB_NATIVE_AUTO_FETCH';
const NATIVE_MANIFEST_URL_ENV = 'OMB_NATIVE_MANIFEST_URL';
const NATIVE_RELEASE_BASE_URL_ENV = 'OMB_NATIVE_RELEASE_BASE_URL';
const NATIVE_CACHE_DIR_ENV = 'OMB_NATIVE_CACHE_DIR';
export const EXPLORE_BIN_ENV = 'OMB_EXPLORE_BIN';
export const SPARKSHELL_BIN_ENV = 'OMB_SPARKSHELL_BIN';
...
export function resolveNativeCacheRoot(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (process.platform === 'win32') {
    return resolve(env.LOCALAPPDATA?.trim() || join(homedir(), 'AppData', 'Local'), 'oh-my-codebuddy', 'native');
  }
  return resolve(env.XDG_CACHE_HOME?.trim() || join(homedir(), '.cache'), 'oh-my-codebuddy', 'native');
}
