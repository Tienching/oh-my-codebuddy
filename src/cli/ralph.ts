...
Common patterns:
  {cmd} ralph "Fix flaky notify-hook tests"
  {cmd} ralph --prd "Ship release checklist automation"
  {cmd} ralph --model gpt-5 "Refactor state hydration"
  {cmd} ralph -- --task-with-leading-dash
`);
}
...
export function filterRalphCodexArgs(args: readonly string[]): string[] {
  ...
  for (const token of args) {
    if (RALPH_OMB_FLAGS.has(token.toLowerCase())) continue;
    filtered.push(token);
  }
  return filtered;
}
...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    options.noDeslop
      ...
      : `- Step 7.5 must run oh-my-codebuddy:ai-slop-cleaner in standard mode on changed files only, using the repo-relative paths listed in \`${options.changedFilesPath}\`.`,
    options.noDeslop
...
async function writeRalphSessionFiles(
  ...
  await writeFile(
    instructionsPath,
    `${buildRalphAppendInstructions(task, { changedFilesPath: '.omb/ralph/changed-files.txt', noDeslop: options.noDeslop, approvedHint: options.approvedHint ?? null })}\n`,
  );
  return { instructionsPath, changedFilesPath: '.omb/ralph/changed-files.txt' };
}
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  const codebuddyArgsBase = filterRalphCodexArgs(normalizedArgs);
  const codebuddyArgs = explicitTask === 'ralph-cli-launch' && approvedHint?.task
    ? [...codebuddyArgsBase, approvedHint.task]
    : codebuddyArgsBase;
  ...
  try {
    await launchWithHud(codebuddyArgs);
  } finally {
