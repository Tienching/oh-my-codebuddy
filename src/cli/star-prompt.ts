/**
 * One-time GitHub star prompt shown at OMB startup.
 * Skipped when no TTY or when gh CLI is not installed.
 * State stored globally (~/.omb/state/star-prompt.json) so it shows once per user.
 */
...
const REPO = 'Yeachan-Heo/oh-my-codebuddy';
...
export function starPromptStatePath(): string {
  return join(homedir(), '.omb', 'state', 'star-prompt.json');
}
...
export async function markPrompted(): Promise<void> {
  const stateDir = join(homedir(), '.omb', 'state');
...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[omb] Enjoying oh-my-codebuddy? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[omb] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[omb] Could not star repository automatically: ${star.error}`);
}
