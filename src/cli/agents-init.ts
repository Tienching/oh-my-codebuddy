...
export const AGENTS_INIT_USAGE = [
  "Usage: omb agents-init [path] [--dry-run] [--force] [--verbose]",
  "       omb deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
const MANAGED_MARKER = "<!-- OMB:AGENTS-INIT:MANAGED -->";
const MANUAL_START = "<!-- OMB:AGENTS-INIT:MANUAL:START -->";
const MANUAL_END = "<!-- OMB:AGENTS-INIT:MANUAL:END -->";
...
export async function agentsInit(
  ...
  console.log("oh-my-codebuddy AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active omb session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by omb agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active omb session is running.\n",
  );
