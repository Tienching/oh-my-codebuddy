...
const HELP = formatCliText([
	"Usage: {cmd} adapt <target> <probe|status|init|envelope|doctor> [--json] [--write]",
	"",
	"Targets:",
	"  openclaw  Foundation seam for OMB-owned OpenClaw adapter artifacts and reporting",
	"  hermes    Foundation seam for OMB-owned Hermes adapter artifacts and reporting",
	"",
	"Subcommands:",
	"  probe     Report shared foundation probe metadata (target-specific runtime probing is deferred)",
	"  status    Report OMB-owned adapter initialization status plus deferred target-runtime status",
	"  init      Preview or write OMB-owned adapter artifacts under .omb/adapters/<target>/...",
	"  envelope  Print the normalized OMB-owned adapter envelope for the target",
	"  doctor    Explain blocked foundation steps and follow-on integration gaps",
	"",
	"Options:",
	"  --json    Emit compact machine-readable JSON",
	"  --write   Only valid with init; write adapter artifacts under .omb/adapters/<target>/...",
	"",
	"Examples:",
	"  {cmd} adapt openclaw probe",
	"  {cmd} adapt hermes status --json",
	"  {cmd} adapt openclaw init --write",
	"  {cmd} adapt hermes envelope --json",
].join("\n"));
...
function targetHelp(target: AdaptTarget): string {
	...
	return [
		`Usage: ${formatCliText("{cmd}")} adapt ${target} <${ADAPT_SUBCOMMANDS.join("|")}> [--json] [--write]`,
		"",
		descriptor.summary,
		"",
		"Notes:",
		...
		target === "openclaw"
			...
			: target === "hermes"
				? "  OMB remains authoritative for orchestration state; Hermes internals are observed, not controlled."
...
export async function adaptCommand(
	...
	if (write && subcommand !== "init") {
		throw new Error(formatCliText("--write is only supported with {cmd} adapt <target> init"));
	}
