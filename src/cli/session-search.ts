...
const HELP = `omb session - Search prior local session history

Usage:
  omb session search <query> [options]
...
Examples:
  omb session search "worker inbox path"
  omb session search all_workers_idle --since 7d --limit 5
  omb session search "team api" --project current --json
`;
