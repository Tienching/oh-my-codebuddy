...
describe("adapt foundation", () => {
	it("resolves OMB-owned adapter paths under .omb/adapters/<target>", () => {
		const paths = resolveAdaptPaths(tempDir, "openclaw");
		assert.equal(
			paths.adapterRoot,
			join(tempDir, ".omb", "adapters", "openclaw"),
		);
		assert.equal(
			paths.configPath,
			join(tempDir, ".omb", "adapters", "openclaw", "adapter.json"),
		);
		assert.equal(
			paths.envelopePath,
			join(tempDir, ".omb", "adapters", "openclaw", "envelope.json"),
		);
		assert.equal(
			paths.probeReportPath,
			join(tempDir, ".omb", "adapters", "openclaw", "reports", "probe.json"),
		);
		assert.equal(
			paths.statusReportPath,
			join(tempDir, ".omb", "adapters", "openclaw", "reports", "status.json"),
		);
	});
