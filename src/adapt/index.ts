...
function readLatestPlanningArtifacts(cwd: string): { prdPath: string | null; testSpecPaths: string[]; deepInterviewSpecPaths: string[] } {
	const artifacts = readPlanningArtifacts(cwd);
	const prdPath = artifacts.prdPaths.at(-1) ?? null;
	const slug = prdPath ? artifactSlug(prdPath, /^prd-(?<slug>.*)\.md$/i) : null;
	return {
		prdPath,
