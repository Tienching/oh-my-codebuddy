/**
 * Config.toml generator/merger for oh-my-codebuddy
 * Merges OMB MCP server entries and feature flags into existing config.toml
 *
...
function getOmbTopLevelLines(
  ...
  const lines = [
    ...
    `developer_instructions = "You have oh-my-codebuddy installed. AGENTS.md is your orchestration brain and the main orchestration surface. Use skill/keyword routing like $name plus spawned role-specialized subagents for specialized work. Native subagents are available via the configured provider agents directory (.codebuddy/agents by default, .codex/agents for Codex provider installs) and may be used for independent parallel subtasks within a single session or team pane. Skills are loaded from installed SKILL.md files under the configured provider skills directory, not from native agent TOMLs. Use workflow skills via $name when explicitly invoked or clearly routed by AGENTS.md. Treat installed prompts as narrower internal execution surfaces under AGENTS.md authority, even when user-facing docs prefer $name keywords."`,
  ];
