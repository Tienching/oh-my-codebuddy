...
describe('config generator', () => {
  it('places top-level keys before [features]', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-config-gen-'));
    try {
  ...
  it('writes model_reasoning_effort and strengthened developer_instructions', async () => {
    ...
    try {
      ...
      assert.match(toml, /^developer_instructions = "You have oh-my-codebuddy installed/m);
      ...
      assert.match(toml, /Native subagents are available via the configured provider agents directory/);
      assert.match(toml, /\.codebuddy\/agents by default, \.codex\/agents for Codex provider installs/);
  ...
  it('handles paths with spaces in notify array', async () => {
    const base = await mkdtemp(join(tmpdir(), 'omb config gen space-'));
  ...
  it('re-runs setup replacing OMB config cleanly', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
      ...
      // OMB block appears exactly once
      assert.equal(
        (rerun.match(/# oh-my-codebuddy \(OMB\) Configuration/g) ?? []).length,
        1
      );
      assert.equal((rerun.match(/# End oh-my-codebuddy/g) ?? []).length, 1);
  ...
  it('writes a global [agents] section with OMB defaults', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
  ...
  it('migrates a legacy OMB block and preserves user settings', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      const legacy = [
      ...
      assert.equal(
        (toml.match(/oh-my-codebuddy \(OMB\) Configuration/g) ?? []).length,
        1
      );
  ...
  it('escapes Windows-style backslashes for MCP server args', async () => {
    ...
    try {
      const configPath = join(wd, 'config.toml');
      const windowsPkgRoot = 'C:\\Users\\alice\\oh-my-codebuddy';
      await mergeConfig(configPath, windowsPkgRoot);
      const toml = await readFile(configPath, 'utf-8');

      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-codebuddy\/dist\/mcp\/state-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-codebuddy\/dist\/mcp\/memory-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-codebuddy\/dist\/mcp\/code-intel-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-codebuddy\/dist\/mcp\/trace-server\.js"\]/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
