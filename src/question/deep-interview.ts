...
import { getStateDir, readCurrentSessionId } from '../mcp/state-paths.js';
import {
  runOmbQuestion,
  type OmbQuestionClientOptions,
  type OmbQuestionSuccessPayload,
} from './client.js';
...
function getDeepInterviewStatePath(cwd: string, sessionId?: string): string {
  return join(getStateDir(cwd, sessionId), DEEP_INTERVIEW_STATE_FILE);
}
...
async function readDeepInterviewStateIfExists(
  ...
  const statePath = getDeepInterviewStatePath(cwd, sessionId);
  try {
