...
const AUTORESEARCH_WORKTREE_EXCLUDES = ['results.tsv', 'run.log', 'node_modules', '.omb/'];
...
function activeRunStateFile(projectRoot: string): string {
  return join(projectRoot, '.omb', 'state', 'autoresearch-state.json');
}
...
export function buildAutoresearchInstructions(
  ...
  return [
    '# OMB Autoresearch Supervisor Instructions',
    '',
...
export async function loadAutoresearchRunManifest(projectRoot: string, runId: string): Promise<AutoresearchRunManifest> {
  const manifestFile = join(projectRoot, '.omb', 'logs', 'autoresearch', runId, 'manifest.json');
...
export async function prepareAutoresearchRuntime(
  ...
  const runDir = join(projectRoot, '.omb', 'logs', 'autoresearch', runId);
