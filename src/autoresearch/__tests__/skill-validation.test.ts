...
describe('autoresearch skill validation', () => {
  ...
  it('treats missing validation mode as incomplete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-skill-validation-'));
    try {
  ...
  it('requires a validator pass artifact for mission-validator-script mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-validator-script-'));
    try {
      const artifactPath = join(cwd, '.omb', 'specs', 'autoresearch-demo', 'completion.json');
      await mkdir(join(cwd, '.omb', 'specs', 'autoresearch-demo'), { recursive: true });
  ...
  it('requires prompt, architect approval, and output artifact for prompt-architect-artifact mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-autoresearch-architect-validation-'));
    try {
      const artifactDir = join(cwd, '.omb', 'specs', 'autoresearch-demo');
