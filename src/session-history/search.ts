    ...
    for await (const line of reader) {
      ...
      for (const candidate of extractSearchableTexts(parsed, line)) {
        ...
        results.push({
          ...
          transcript_path_relative: relative(options.codebuddyHomeDir, filePath),
...
export async function searchSessionHistory(options: SessionSearchOptions): Promise<SessionSearchReport> {
  ...
  const codebuddyHomeDir = options.codebuddyHomeDir ?? codebuddyHome();
  ...
  const rolloutRoot = join(codebuddyHomeDir, 'sessions');
