...
function renderTodos(ctx: HudRenderContext): string | null {
  if (!ctx.todos || !ctx.todos.active) return null;
  const { completed, total } = ctx.todos;
  const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
  return dim(`todos:${completed}/${total}(${pct}%)`);
}

function renderThinking(ctx: HudRenderContext): string | null {
  if (!ctx.thinking || !ctx.thinking.active) return null;
  const level = ctx.thinking.level;
  if (level === 'ultrathink') return yellow('ultrathink');
  if (level === 'think') return yellow('think');
  return null;
}
