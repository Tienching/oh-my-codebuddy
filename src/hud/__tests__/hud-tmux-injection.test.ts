...
describe('VULNERABILITY – old string-interpolation approach', () => {
   ...
   *   const cmd  = `node ${ombBin} hud --watch${presetArg}`;
   *   execSync(`tmux split-window -v -l 4 -c "${cwd}" '${cmd}'`);
   */
  function buildOldCommand(cwd: string, ombBin: string, preset?: string): string {
  ...
  it('cwd containing $() is injectable via double-quote context', () => {
    ...
    const shellCmd = buildOldCommand(maliciousCwd, '/usr/bin/omb.js');
  ...
  it("ombBin containing single quote breaks out of the '-quoted command", () => {
    ...
    assert.ok(
      shellCmd.includes("'node /tmp/it';touch /tmp/pwned;echo '/omb.js"),
...
describe('buildTmuxSplitArgs – shell injection hardening', () => {
  it('produces correct argv for normal inputs', () => {
    const args = buildTmuxSplitArgs('/home/user/project', '/usr/local/bin/omb.js');
    assert.deepEqual(args, [
      'split-window', '-v', '-l', String(HUD_TMUX_HEIGHT_LINES),
      // split height should come from shared HUD constants
      '-c', '/home/user/project',
      "node '/usr/local/bin/omb.js' hud --watch",
    ]);
  });

  it('cwd is an isolated array element – never shell-interpreted', () => {
    const maliciousCwd = '/tmp/foo"$(touch /tmp/pwned)"bar';
    const args = buildTmuxSplitArgs(maliciousCwd, '/usr/bin/omb.js');
  ...
  it("ombBin with single quote is properly escaped in command string", () => {
    ...
    assert.equal(cmd, "node '/tmp/it'\\''s/omb.js' hud --watch");
  });
  ...
  it('ombBin with $() is neutralised by single-quote wrapping', () => {
    ...
    assert.equal(cmd, "node '/tmp/$(id)/omb.js' hud --watch");
  });
  ...
  it("ombBin with ';command' breakout attempt is neutralised", () => {
    ...
    //
    // Raw expected value: node '/tmp/x'\'';touch /tmp/pwned;echo '\''/omb.js' hud --watch
    assert.equal(
      cmd,
      "node '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/omb.js' hud --watch",
    );
  ...
  it('preset is appended safely', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omb.js', 'minimal');
    const cmd = args[6];
    assert.ok(cmd.endsWith('--preset=minimal'));
  });

  it('absent preset produces no --preset flag', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omb.js');
    const cmd = args[6];
  ...
  it('invalid preset is dropped (defense in depth)', () => {
    ...
    const cmd = args[6];
    assert.equal(cmd, "node '/usr/bin/omb.js' hud --watch");
    assert.ok(!cmd.includes('--preset='));
  });
});
