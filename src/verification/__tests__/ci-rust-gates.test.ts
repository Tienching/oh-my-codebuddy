...
describe('CI Rust gates', () => {
  ...
  it('uses the current crates/omb-sparkshell manifest for Rust coverage', () => {
    const workflow = readCiWorkflow();

    assert.match(workflow, /crates\/omb-sparkshell\/Cargo\.toml/);
    assert.doesNotMatch(workflow, /native\/omb-sparkshell\/Cargo\.toml/);
  });
  ...
  it('uses the current sparkshell crate manifest in the Rust coverage lane', () => {
    ...
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omb-sparkshell\/Cargo\.toml --summary-only/);
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omb-sparkshell\/Cargo\.toml --lcov --output-path coverage\/rust\/omb-sparkshell\.lcov/);
    assert.doesNotMatch(workflow, /native\/omb-sparkshell\/Cargo\.toml/);
  });
});
