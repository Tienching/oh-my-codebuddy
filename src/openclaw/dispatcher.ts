 ...
 *
 * SECURITY: Command gateway requires OMB_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > OMB_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.OMB_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires OMB_OPENCLAW_COMMAND=1 opt-in (separate gate from OMB_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or OMB_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.OMB_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set OMB_OPENCLAW_COMMAND=1 to enable)",
    };
  }
