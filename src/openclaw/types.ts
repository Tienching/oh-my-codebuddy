 ...
 *
 * NOTE: CodeBuddy CLI only supports a limited set of hook events.
 * pre-tool-use, post-tool-use, and keyword-detector are OMC-specific
 * and are NOT available in CodeBuddy CLI — excluded here intentionally.
 */
...
export interface OpenClawCommandGatewayConfig {
   ...
   * Precedence: gateway timeout > OMB_OPENCLAW_COMMAND_TIMEOUT_MS > default (5000ms).
...
/** Top-level config schema for notifications.openclaw key in .omb-config.json */
