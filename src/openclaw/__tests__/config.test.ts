...
describe("getOpenClawConfig", () => {
  ...
  it("returns null when OMB_OPENCLAW is not set", async () => {
  ...
  it("returns null when OMB_OPENCLAW !== '1'", async () => {
    process.env.OMB_OPENCLAW = "0";
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
    assert.equal(result, null);
  ...
  it("returns null when OMB_OPENCLAW_CONFIG file does not exist", async () => {
    process.env.OMB_OPENCLAW = "1";
    process.env.OMB_OPENCLAW_CONFIG = join(tmpDir, "nonexistent.json");
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
    assert.equal(result, null);
  ...
  it("reads config from OMB_OPENCLAW_CONFIG override file", async () => {
    process.env.OMB_OPENCLAW = "1";
    ...
    process.env.OMB_OPENCLAW_CONFIG = configPath;
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
...
describe("getOpenClawConfig generic alias normalization", () => {
  ...
  beforeEach(() => {
    originalEnv = { ...process.env };
    tmpDir = join(tmpdir(), `omb-openclaw-alias-test-${Date.now()}`);
    mkdirSync(tmpDir, { recursive: true });
    process.env.OMB_OPENCLAW = "1";
  ...
  it("normalizes custom_webhook_command alias to openclaw runtime config", async () => {
    const omxConfigPath = join(tmpDir, ".codex", ".omb-config.json");
