...
describe('resolveCommandTimeoutMs', () => {
  afterEach(() => {
    delete process.env.OMB_OPENCLAW_COMMAND_TIMEOUT_MS;
  });
...
describe('wakeCommandGateway - command gate', () => {
  ...
  it('returns error when OMB_OPENCLAW_COMMAND is not set', async () => {
    ...
    assert.ok(result.error?.includes('OMB_OPENCLAW_COMMAND'));
  });

  it('succeeds when OMB_OPENCLAW_COMMAND=1 and command exits 0', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMB_OPENCLAW_COMMAND = '1';
  ...
  it('uses env timeout when gateway timeout is not set', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMB_OPENCLAW_COMMAND = '1';
    process.env.OMB_OPENCLAW_COMMAND_TIMEOUT_MS = '100';
