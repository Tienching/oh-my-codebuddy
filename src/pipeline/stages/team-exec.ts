 ...
 *
 * Wraps the existing team mode (tmux-based CodeBuddy CLI workers) into a
 * PipelineStage. The execution backend is always teams — this is the
 * canonical OMB execution surface.
 */
...
export interface TeamExecStageOptions {
  /** Number of CodeBuddy CLI workers to launch. Defaults to 2. */
  workerCount?: number;
 ...
 *
 * This stage delegates to the existing `omb team` infrastructure, which
 * starts real CodeBuddy CLI workers in tmux panes. The stage collects the
...
/**
 * Build the `omb team` CLI instruction from a descriptor.
 */
export function buildTeamInstruction(descriptor: TeamExecDescriptor): string {
  const launchCommand = `omb team ${descriptor.workerCount}:${descriptor.agentType} ${JSON.stringify(descriptor.task)}`;
