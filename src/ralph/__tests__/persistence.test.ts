...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.omb', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.omb', 'state', 'ralph-progress.json');
      ...
      await writeFile(join(cwd, '.omb', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.omb', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.omb', 'prd.json');
      const legacyProgressPath = join(cwd, '.omb', 'progress.txt');
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.omb\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.omb/progress.txt');
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-ralph-visual-feedback-'));
    try {
