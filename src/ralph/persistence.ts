...
const LEGACY_PRD_PATH = '.omb/prd.json';
const LEGACY_PROGRESS_PATH = '.omb/progress.txt';
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.omb', 'plans', 'ralph-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.omb/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',
