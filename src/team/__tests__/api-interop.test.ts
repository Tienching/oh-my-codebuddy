...
import {
  TEAM_INTEROP_COMPATIBILITY_FIELDS,
  TEAM_INTEROP_CONTRACT_NAME,
  TEAM_INTEROP_ERROR_CODES,
  TEAM_INTEROP_PAYLOAD_VERSION,
  normalizeTeamInteropErrorCode,
} from '../contracts.js';
import {
  initTeamState,
  createTask,
...
async function setupTeam(name: string): Promise<{ cwd: string; cleanup: () => Promise<void> }> {
  const cwd = await mkdtemp(join(tmpdir(), `omb-interop-${name}-`));
  const previousOmbTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
  ...
  return {
    cwd,
    cleanup: async () => {
      if (typeof previousOmbTeamStateRoot === 'string') process.env.OMB_TEAM_STATE_ROOT = previousOmbTeamStateRoot;
...
async function withVersionedInteropPayload<T>(fn: () => Promise<T>): Promise<T> {
  const previous = process.env.OMB_INTEROP_VERSIONED_PAYLOAD;
  process.env.OMB_INTEROP_VERSIONED_PAYLOAD = '1';
  try {
    return await fn();
  } finally {
    if (typeof previous === 'string') process.env.OMB_INTEROP_VERSIONED_PAYLOAD = previous;
    else delete process.env.OMB_INTEROP_VERSIONED_PAYLOAD;
  }
}

async function readJsonFixture(name: string): Promise<unknown> {
  return JSON.parse(await readFile(join(API_INTEROP_FIXTURES_DIR, name), 'utf8'));
}

async function withMailboxCompatHoleRuntime<T>(cwd: string, fn: () => Promise<T>): Promise<T> {
  const fakeBin = join(cwd, 'runtime-bin');
  const runtimePath = join(fakeBin, 'omb-runtime');
  const previousPath = process.env.PATH;
  const previousBinary = process.env.OMB_RUNTIME_BINARY;
  const previousBridge = process.env.OMB_RUNTIME_BRIDGE;
...
`,
  ...
  process.env.OMB_RUNTIME_BINARY = runtimePath;
  process.env.OMB_RUNTIME_BRIDGE = '1';
  try {
    return await fn();
  } finally {
    if (typeof previousPath === 'string') process.env.PATH = previousPath;
    else delete process.env.PATH;
    if (typeof previousBinary === 'string') process.env.OMB_RUNTIME_BINARY = previousBinary;
    else delete process.env.OMB_RUNTIME_BINARY;
    if (typeof previousBridge === 'string') process.env.OMB_RUNTIME_BRIDGE = previousBridge;
...
describe('buildLegacyTeamDeprecationHint', () => {
  it('produces CLI hint with resolved operation name', () => {
    ...
    assert.match(hint, /omb team api send-message/);
  ...
  it('falls back to generic hint for unresolvable legacy name', () => {
    ...
    assert.match(hint, /omb team api <operation>/);
  });
...
describe('team interop payload contract', () => {
  it('keeps the versioned contract constants aligned', () => {
    assert.equal(TEAM_INTEROP_CONTRACT_NAME, 'team-api-interop');
    assert.equal(TEAM_INTEROP_PAYLOAD_VERSION, 2);
    assert.deepEqual([...TEAM_INTEROP_COMPATIBILITY_FIELDS], ['ok', 'operation', 'data', 'error']);
    assert.deepEqual([...TEAM_INTEROP_ERROR_CODES], [
      'invalid_input',
      'manifest_not_found',
      'operation_failed',
      'task_not_found',
  ...
  it('normalizes compatibility error aliases into the standard contract set', () => {
    assert.equal(normalizeTeamInteropErrorCode('invalid_input'), 'invalid_input');
    assert.equal(normalizeTeamInteropErrorCode('team_missing'), 'team_not_found');
    assert.equal(normalizeTeamInteropErrorCode('runtime_bridge_failed'), 'operation_failed');
    assert.equal(normalizeTeamInteropErrorCode('unknown_future_code'), 'operation_failed');
  });
});
...
describe('executeTeamApiOperation: versioned interop payloads', () => {
  it('keeps the legacy envelope when the versioned payload flag is disabled', async () => {
    const { cwd, cleanup } = await setupTeam('legacy-interop');
    try {
      const result = await executeTeamApiOperation('list-tasks', { team_name: 'legacy-interop' }, cwd);
      ...
      assert.deepEqual(result.data, { count: 0, tasks: [] });
  ...
  it('matches the golden success fixture when the versioned payload flag is enabled', async () => {
    const { cwd, cleanup } = await setupTeam('versioned-interop-success');
    try {
      const result = await withVersionedInteropPayload(() =>
        executeTeamApiOperation('list-tasks', { team_name: 'versioned-interop-success' }, cwd));
      assert.deepEqual(result, await readJsonFixture('versioned-list-tasks-success.json'));
    } finally {
      await cleanup();
    }
  });

  it('matches the golden failure fixture and preserves legacy error aliases when versioned payloads are enabled', async () => {
    const result = await withVersionedInteropPayload(() => executeTeamApiOperation('list-tasks', {}, '/tmp'));
    assert.deepEqual(result, await readJsonFixture('versioned-list-tasks-error.json'));
    assert.equal(result.ok, false);
    if (result.ok) throw new Error('expected list-tasks validation to fail');
...
describe('executeTeamApiOperation: send-message', () => {
  ...
  it('returns the persisted leader mailbox message when hook-targeted sends are deduped from a worker worktree', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'omb-interop-send-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'omb-interop-send-worktree-'));
    ...
    try {
      ...
      const configPath = join(repoCwd, '.omb', 'state', 'team', teamName, 'config.json');
      ...
      const manifestPath = join(repoCwd, '.omb', 'state', 'team', teamName, 'manifest.v2.json');
      ...
      process.env.OMB_TEAM_STATE_ROOT = join(repoCwd, '.omb', 'state');
      ...
      const mailbox = JSON.parse(await readFile(
        join(repoCwd, '.omb', 'state', 'team', teamName, 'mailbox', 'leader-fixed.json'),
        'utf-8',
  ...
  it('returns the persisted mailbox message when bridge compat omits the mailbox row under an explicit shared state root', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'omb-interop-shared-root-'));
    ...
    try {
      ...
      process.env.OMB_TEAM_STATE_ROOT = sharedStateRoot;
...
describe('executeTeamApiOperation: mailbox-mark-delivered', () => {
  it('marks a message delivered after sending and promotes matching dispatch receipt', async () => {
    ...
    try {
      ...
      await mkdir(join(cwd, '.omb', 'state', 'team', 'mark-dlv', 'mailbox', 'worker-2'), { recursive: true });
  ...
  it('marks leader-fixed mailbox delivery from a worker worktree and resolves the matching dispatch receipt', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'omb-interop-mark-dlv-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'omb-interop-mark-dlv-worktree-'));
    const prevTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    delete process.env.OMB_TEAM_STATE_ROOT;
...
describe('executeTeamApiOperation: mailbox-mark-notified', () => {
  it('marks a message notified after sending', async () => {
    ...
    try {
      // Ensure the worker-2 mailbox directory exists so sendDirectMessage can write
      await mkdir(join(cwd, '.omb', 'state', 'team', 'mark-ntf', 'mailbox', 'worker-2'), { recursive: true });
      const sendResult = await executeTeamApiOperation('send-message', {
...
describe('executeTeamApiOperation: list-tasks', () => {
  ...
  it('resolves team working directory from manifest metadata over worker identity/config fallbacks', async () => {
    ...
    const cwdA = await mkdtemp(join(tmpdir(), 'omb-interop-meta-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'omb-interop-meta-b-'));
    const prevTeamWorker = process.env.OMB_TEAM_WORKER;
    const prevTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    delete process.env.OMB_TEAM_STATE_ROOT;
    process.env.OMB_TEAM_WORKER = `${teamName}/worker-1`;
    ...
    try {
      ...
      const teamRootA = join(cwdA, '.omb', 'state', 'team', teamName);
      ...
      config.team_state_root = join(cwdA, '.omb', 'state');
      manifest.team_state_root = join(cwdB, '.omb', 'state');
  ...
  it('prefers OMB_TEAM_STATE_ROOT over manifest metadata when resolving the team working directory', async () => {
    const teamName = 'list-tsk-env-root';
    const cwdA = await mkdtemp(join(tmpdir(), 'omb-interop-env-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'omb-interop-env-b-'));
    const prevTeamWorker = process.env.OMB_TEAM_WORKER;
    const prevTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    delete process.env.OMB_TEAM_STATE_ROOT;
    process.env.OMB_TEAM_WORKER = `${teamName}/worker-1`;
    ...
    try {
      ...
      process.env.OMB_TEAM_STATE_ROOT = join(cwdA, '.omb', 'state');
...
describe('executeTeamApiOperation: read-config', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-interop-cfg-nf-'));
    try {
      const result = await executeTeamApiOperation('read-config', {
...
describe('executeTeamApiOperation: read-stall-state', () => {
  ...
  it('suppresses stale native-stop attention after newer leader activity', async () => {
    ...
    try {
      ...
      await writeFile(join(cwd, '.omb', 'state', 'leader-runtime-activity.json'), JSON.stringify({
...
describe('executeTeamApiOperation: get-summary', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-interop-sum-nf-'));
    try {
      const result = await executeTeamApiOperation('get-summary', {
...
describe('executeTeamApiOperation: error handling', () => {
  it('wraps thrown errors in an error envelope', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-interop-err-'));
    try {
      await mkdir(join(cwd, '.omb', 'state', 'team', 'err-team'), { recursive: true });
      await writeFile(join(cwd, '.omb', 'state', 'team', 'err-team', 'config.json'), '{}', 'utf8');
