...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omb-team-hardening-e2e-repo-'));
...
describe('team hardening e2e', () => {
  it('reopens an expired in-progress task and allows the next worker to complete the flow', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-team-hardening-e2e-'));
    try {
      ...
      const taskPath = join(cwd, '.omb', 'state', 'team', 'team-hardening-e2e', 'tasks', `task-${task.id}.json`);
