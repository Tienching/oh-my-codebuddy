...
function setMockCodebuddyHome(codebuddyHomePath: string): () => void {
  const previous = process.env.CODEBUDDY_HOME;
  process.env.CODEBUDDY_HOME = codebuddyHomePath;
  return () => {
    if (typeof previous === "string") process.env.CODEBUDDY_HOME = previous;
...
describe("worker bootstrap", () => {
  it("worker skill lifecycle instructions are claim-safe (issue #448)", async () => {
    ...
    assert.match(workerSkill, /omb team api claim-task/);
    assert.match(workerSkill, /omb team api transition-task-status/);
    assert.match(workerSkill, /omb team api release-task-claim/);
    assert.match(
      workerSkill,
  ...
  it("generateWorkerOverlay produces markdown with correct start/end markers", () => {
    ...
    assert.match(overlay, /<!-- OMB:TEAM:WORKER:START -->/);
    assert.match(overlay, /<!-- OMB:TEAM:WORKER:END -->/);
  });
  ...
  it("generateWorkerOverlay includes the team name", () => {
    ...
    assert.match(overlay, /omb team api claim-task/);
    assert.match(overlay, /omb team api transition-task-status/);
    assert.match(overlay, /omb team api release-task-claim/);
    assert.doesNotMatch(
      overlay,
    ...
    assert.match(
      overlay,
      /You may spawn CodeBuddy native subagents when parallel execution improves throughput/,
    );
    assert.match(
      overlay,
  ...
  it("applyWorkerOverlay appends to existing AGENTS.md content", async () => {
    ...
    try {
      ...
      assert.match(content, /<!-- OMB:TEAM:WORKER:START -->/);
      assert.match(content, /<!-- OMB:TEAM:WORKER:END -->/);
    } finally {
  ...
  it("applyWorkerOverlay is idempotent (calling twice doesn't duplicate)", async () => {
    ...
    try {
      ...
      const content = await readFile(agentsMdPath, "utf8");
      const starts = content.match(/<!-- OMB:TEAM:WORKER:START -->/g) ?? [];
      const ends = content.match(/<!-- OMB:TEAM:WORKER:END -->/g) ?? [];
  ...
  it("stripWorkerOverlay removes the overlay section", async () => {
    ...
    try {
      ...
      assert.doesNotMatch(content, /<!-- OMB:TEAM:WORKER:START -->/);
      assert.doesNotMatch(content, /<!-- OMB:TEAM:WORKER:END -->/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("applyWorkerOverlay reaps stale AGENTS lock directory", async () => {
    ...
    try {
      const agentsMdPath = join(cwd, "AGENTS.md");
      const lockPath = join(cwd, ".omb", "state", "agents-md.lock");
  ...
  it("generateInitialInbox includes worker name, team name, and all tasks", () => {
    ...
    assert.match(inbox, /omb team api claim-task/);
    assert.match(inbox, /omb team api transition-task-status/);
    assert.match(inbox, /omb team api release-task-claim/);
    assert.match(
      inbox,
      /\$\{CODEBUDDY_HOME:-~\/\.codebuddy\}\/skills\/worker\/SKILL\.md/,
    );
  ...
  it("generateTriggerMessage does not contain [OMB_TMUX_INJECT]", () => {
    const message = generateTriggerMessage("worker-1", "team-safe");
    assert.equal(message.includes("[OMB_TMUX_INJECT]"), false);
  });
  ...
  it("generateTriggerMessage contains the inbox path", () => {
    ...
    assert.match(
      message,
      /\.omb\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("buildTriggerDirective keeps human text separate from orchestration intent", () => {
    ...
    assert.match(directive.text, /\.omb\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/);
    assert.doesNotMatch(directive.text, /OMB_INTENT/);
  });
  ...
  it("generateTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /\$OMB_TEAM_STATE_ROOT\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("generateMailboxTriggerMessage contains mailbox path and count", () => {
    ...
    assert.match(
      message,
      /Read .*\.omb\/state\/team\/team-mail\/mailbox\/worker-2\.json/,
    );
  ...
  it("generateMailboxTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /read .*\$OMB_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/worker-2\.json/i,
    );
  ...
  it("generateLeaderMailboxTriggerMessage tells the leader to read the mailbox and decide the next step", () => {
    ...
    assert.match(
      message,
      /Read .*\.omb\/state\/team\/team-mail\/mailbox\/leader-fixed\.json/,
    );
  ...
  it("generateLeaderMailboxTriggerMessage uses provided state-root reference for worktree leaders", () => {
    ...
    assert.match(
      message,
      /read .*\$OMB_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/leader-fixed\.json/i,
    );
  ...
  it("writeTeamWorkerInstructionsFile composes user + project AGENTS.md with overlay", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omb-worker-bootstrap-"));
    const restoreCodexHome = setMockCodebuddyHome(join(cwd, "home", ".codebuddy"));
    try {
  ...
  it("writeTeamWorkerInstructionsFile deduplicates duplicate skill references in favor of project scope", async () => {
    ...
    const restoreCodexHome = setMockCodebuddyHome(join(cwd, "home", ".codebuddy"));
    try {
      const userAgentsPath = join(cwd, "home", ".codebuddy", "AGENTS.md");
      const projectAgentsPath = join(cwd, "AGENTS.md");
      const userSkillDir = join(cwd, "home", ".codebuddy", "skills", "help");
      const projectSkillDir = join(cwd, ".codebuddy", "skills", "help");
      ...
      await writeFile(
        userAgentsPath,
        "- help user (file: /tmp/home/.codebuddy/skills/help/SKILL.md)\n",
        "utf8",
      );
      await writeFile(
        projectAgentsPath,
        "- help project (file: /tmp/project/.codebuddy/skills/help/SKILL.md)\n",
        "utf8",
      );
  ...
  it("generateWorkerRootAgentsContent includes hardcoded paths and role prompt without base AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      leaderCwd: "/repo",
      worktreePath: "/repo/.omb/team/root-team/worktrees/worker-3",
    });
  ...
  it("writeWorkerRoleInstructionsFile layers role prompt on top of team worker instructions", async () => {
    ...
    try {
      ...
      assert.match(content, /<!-- OMB:TEAM:ROLE:START -->/);
  ...
  it("generateWorkerRootAgentsContent hardcodes runtime paths and role prompt without inherited AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      workerRole: "writer",
      rolePromptContent: "<identity>You are Writer.</identity>",
      teamStateRoot: "/tmp/project/.omb/state",
      leaderCwd: "/tmp/project",
      worktreePath: "/tmp/project/.omb/team/root-team/worktrees/worker-2",
    });

    assert.match(content, /# Team Worker Runtime Instructions/);
    assert.match(content, /Inbox path: \/tmp\/project\/.omb\/state\/team\/root-team\/workers\/worker-2\/inbox\.md/);
    assert.match(content, /Mailbox path: \/tmp\/project\/.omb\/state\/team\/root-team\/mailbox\/worker-2\.json/);
    assert.match(content, /Leader mailbox path: \/tmp\/project\/.omb\/state\/team\/root-team\/mailbox\/leader-fixed\.json/);
  ...
  it("writeWorkerWorktreeRootAgentsFile writes disposable root AGENTS and remove restores tracked content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omb-worker-root-agents-"));
    const worktree = join(cwd, "worktree");
    try {
      await mkdir(join(cwd, ".omb", "state", "team", "restore-team", "workers", "worker-1"), { recursive: true });
      ...
      const outPath = await writeWorkerWorktreeRootAgentsFile({
        ...
        teamStateRoot: join(cwd, ".omb", "state"),
        leaderCwd: cwd,
      ...
      await removeWorkerWorktreeRootAgentsFile("restore-team", "worker-1", join(cwd, ".omb", "state"), worktree);
