...
const fs = require('fs');
const path = require('path');
const worker = String(process.env.OMB_TEAM_WORKER || process.env.OMX_TEAM_WORKER || 'unknown').replace(/[^a-zA-Z0-9_-]+/g, '__');
...
`,
    ...
    const prevLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
    const prevLegacyLaunchArgs = process.env.OMX_TEAM_WORKER_LAUNCH_ARGS;
    ...
    try {
      ...
      if (typeof prevLegacyLaunchArgs === 'string') process.env.OMX_TEAM_WORKER_LAUNCH_ARGS = prevLegacyLaunchArgs;
      else delete process.env.OMX_TEAM_WORKER_LAUNCH_ARGS;
...
fs.writeFileSync(path.join(logDir, 'env.json'), JSON.stringify({
  cwd: process.cwd(),
  teamStateRoot: process.env.OMB_TEAM_STATE_ROOT || process.env.OMX_TEAM_STATE_ROOT || '',
  worker: process.env.OMB_TEAM_WORKER || process.env.OMX_TEAM_WORKER || '',
}));
