...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-worktree-repo-'));
...
async function readTeamDeliveryLog(cwd: string): Promise<Array<Record<string, unknown>>> {
  const path = join(cwd, '.omb', 'logs', `team-delivery-${new Date().toISOString().slice(0, 10)}.jsonl`);
...
function withoutTeamWorkerEnv<T>(fn: () => T): T {
  const prev = process.env.OMB_TEAM_WORKER;
  ...
  try {
    ...
    if (result instanceof Promise) {
      restoreImmediately = false;
      return result.finally(() => {
        if (typeof prev === 'string') process.env.OMB_TEAM_WORKER = prev;
...
const stateRoot = process.env.OMB_TEAM_STATE_ROOT;
const worker = String(process.env.OMB_TEAM_WORKER || '');
...
async function withPromptModeCodexEnv<T>(
  ...
  const nextEnv: Record<string, string | undefined> = {
    ...
    TMUX: undefined,
    OMB_TEAM_WORKER_LAUNCH_MODE: 'prompt',
    OMB_TEAM_WORKER_CLI: 'codex',
    ...extraEnv,
  };
...
function teamStateTestPath(cwd: string, ...parts: string[]): string {
  const stateRoot = process.env.OMB_TEAM_STATE_ROOT ?? join(cwd, '.omb', 'state');
...
const ORIGINAL_OMB_TEAM_STATE_ROOT = process.env.OMB_TEAM_STATE_ROOT;
...
afterEach(() => {
  if (typeof ORIGINAL_OMB_TEAM_STATE_ROOT === 'string') process.env.OMB_TEAM_STATE_ROOT = ORIGINAL_OMB_TEAM_STATE_ROOT;
...
describe('runtime', () => {
  ...
  it('resolveWorkerLaunchArgsFromEnv preserves explicit model in either syntax', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMB_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore'),
      ['--model', 'gpt-5'],
    );
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMB_TEAM_WORKER_LAUNCH_ARGS: '--model=gpt-5.3' }, 'explore'),
      ['--model', 'gpt-5.3'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv prefers explicit env model over inherited leader model', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMB_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore', 'gpt-4.1'),
      ['--model', 'gpt-5'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv preserves explicit reasoning and logs source=explicit', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        { OMB_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort=\"high\" --no-alt-screen' },
        'explore',
      );
      assert.deepEqual(
        args,
  ...
  it('resolveWorkerLaunchArgsFromEnv logs model=gemini without thinking_level for gemini CLI', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMB_TEAM_WORKER_CLI: 'gemini',
          OMB_TEAM_WORKER_LAUNCH_ARGS: '--model gemini-2.0-pro',
        },
        'executor',
      );
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps codex thinking_level logging for mixed CLI maps', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMB_TEAM_WORKER_CLI_MAP: 'codex,claude',
          OMB_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort="high" --model claude-3-7-sonnet',
        },
        'executor',
      );
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps claude and gemini startup logs free of thinking_level during teammate allocation', () => {
    ...
    try {
      ...
      const claudeArgs = resolveWorkerLaunchArgsFromEnv(
        { OMB_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen --model claude-3-7-sonnet' },
        'executor',
        undefined,
        'low',
        'claude',
  ...
  it('waitForClaudeStartupEvidence requires first-start ACK/task progress before startup dispatch is treated as settled', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-claude-startup-'));
    try {
      ...
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'claude-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
  ...
  it('waitForWorkerStartupEvidence ignores Codex ACK-only startup replies until work is claimed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-codex-startup-'));
    try {
      ...
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'codex-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
          current_task_id: 'task-1',
          updated_at: new Date().toISOString(),
  ...
  it('startTeam rejects nested team invocation inside worker context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-'));
    const prev = process.env.OMB_TEAM_WORKER;
    process.env.OMB_TEAM_WORKER = 'alpha/worker-1';
    try {
  ...
  it('startTeam allows nested team invocation when parent governance enables it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-nested-allow-'));
...
`,
    ...
    const parentManifestPath = join(cwd, '.omb', 'state', 'team', 'parent-team', 'manifest.v2.json');
    ...
    const prevWorker = process.env.OMB_TEAM_WORKER;
    const prevStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    const prevLeaderCwd = process.env.OMB_TEAM_LEADER_CWD;
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMB_TEAM_WORKER = 'parent-team/worker-1';
    process.env.OMB_TEAM_STATE_ROOT = join(cwd, '.omb', 'state');
    process.env.OMB_TEAM_LEADER_CWD = cwd;
    process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    ...
    try {
      ...
      if (typeof prevWorker === 'string') process.env.OMB_TEAM_WORKER = prevWorker;
      else delete process.env.OMB_TEAM_WORKER;
      if (typeof prevStateRoot === 'string') process.env.OMB_TEAM_STATE_ROOT = prevStateRoot;
      else delete process.env.OMB_TEAM_STATE_ROOT;
      if (typeof prevLeaderCwd === 'string') process.env.OMB_TEAM_LEADER_CWD = prevLeaderCwd;
      else delete process.env.OMB_TEAM_LEADER_CWD;
      if (typeof prevLaunchMode === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_MODE = prevLaunchMode;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
  ...
  it('startTeam throws when tmux is not available', async () => {
    ...
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    try {
      process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'interactive';
      await assert.rejects(
  ...
  it('startTeam rejects duplicate active same-name team state without mutating existing files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-duplicate-team-'));
    const prevSessionId = process.env.OMB_SESSION_ID;
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    try {
      process.env.OMB_SESSION_ID = 'sess-existing-team';
      await initTeamState(
        'dup-team',
        'existing task',
        'executor',
      ...
      process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
      process.env.OMB_SESSION_ID = 'sess-second-team';
      ...
      if (typeof prevSessionId === 'string') process.env.OMB_SESSION_ID = prevSessionId;
  ...
  it('skips interactive worker process-tree prekill on native Windows split-pane sessions', async () => {
    await withNativeWindowsPlatform(async () => {
      ...
      assert.equal(shouldPrekillInteractiveShutdownProcessTrees('omb-team-alpha'), true);
    });
  ...
  it('startTeam accepts native Windows tmux clients even when TMUX env vars are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-win32-no-env-'));
    const prevTmux = process.env.TMUX;
    const prevTmuxPane = process.env.TMUX_PANE;
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevSkipReadyWait = process.env.OMB_TEAM_SKIP_READY_WAIT;
    const prevMsystem = process.env.MSYSTEM;
    const prevOstype = process.env.OSTYPE;
    const prevWsl = process.env.WSL_DISTRO_NAME;
    const prevWslInterop = process.env.WSL_INTEROP;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-win32-no-env-',
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'interactive';
          process.env.OMB_TEAM_WORKER_CLI = 'gemini';
          process.env.OMB_TEAM_SKIP_READY_WAIT = '1';
          delete process.env.MSYSTEM;
          delete process.env.OSTYPE;
          delete process.env.WSL_DISTRO_NAME;
          delete process.env.WSL_INTEROP;
      ...
      if (typeof prevSkipReadyWait === 'string') process.env.OMB_TEAM_SKIP_READY_WAIT = prevSkipReadyWait;
      else delete process.env.OMB_TEAM_SKIP_READY_WAIT;
      if (typeof prevMsystem === 'string') process.env.MSYSTEM = prevMsystem;
      else delete process.env.MSYSTEM;
      if (typeof prevOstype === 'string') process.env.OSTYPE = prevOstype;
      else delete process.env.OSTYPE;
  ...
  it('applyCreatedInteractiveSessionToConfig persists worker pane ids before readiness waits', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-pane-persist-race-'));
    try {
  ...
  it('startTeam captures interactive worker pid from the resolved pane id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-pane-pid-'));
    const prevTmux = process.env.TMUX;
    const prevTmuxPane = process.env.TMUX_PANE;
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-pane-pid-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
        ...
        async () => {
          ...
          process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'interactive';
          process.env.OMB_TEAM_WORKER_CLI = 'codex';
          process.env.OMB_TEAM_SKIP_READY_WAIT = '1';
          ...
          const identityPath = join(cwd, '.omb', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'identity.json');
  ...
  it('startTeam rejects dirty leader workspace before provisioning worker worktrees', async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        /leader_workspace_dirty_for_worktrees:.*M README\.md.*\?\? notes\.txt.*commit_or_stash_before_omb_team/s,
      );
      ...
      assert.equal(existsSync(join(repo, '.omb', 'state', 'team', 'team-dirty-preflight')), false);
    } finally {
      if (typeof prevLaunchMode === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_MODE = prevLaunchMode;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
  ...
  it('startTeam launches gemini workers with startup prompt and no default model passthrough', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-gemini-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
...
`,
    ...
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevLaunchArgs = process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
    const prevCapture = process.env.OMB_GEMINI_ARGV_CAPTURE_PATH;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMB_TEAM_WORKER_CLI = 'gemini';
    process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.3-codex-spark';
    process.env.OMB_GEMINI_ARGV_CAPTURE_PATH = capturePath;
    ...
    try {
      ...
      const expectedArgv = [
        ...
        'yolo',
        '-i',
        'Read .omb/state/team/team-gemini-prompt/workers/worker-1/inbox.md, start work now, report concrete progress, then continue assigned work or next feasible task.',
      ];
      ...
      if (typeof prevLaunchArgs === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = prevLaunchArgs;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
      if (typeof prevCapture === 'string') process.env.OMB_GEMINI_ARGV_CAPTURE_PATH = prevCapture;
      else delete process.env.OMB_GEMINI_ARGV_CAPTURE_PATH;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam preserves explicit codex launch args while forcing bypass in prompt mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-codex-explicit-launch-'));
    const binDir = join(cwd, 'bin');
    ...
    await writeFile(
      ...
      `#!/usr/bin/env node
const fs = require('fs');
fs.writeFileSync(process.env.OMB_CODEX_ARGV_CAPTURE_PATH, JSON.stringify(process.argv.slice(2), null, 2));
...
`,
    ...
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevLaunchArgs = process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
    const prevCapture = process.env.OMB_CODEX_ARGV_CAPTURE_PATH;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMB_TEAM_WORKER_CLI = 'codex';
    process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.3-codex-spark -c model_reasoning_effort="low"';
    process.env.OMB_CODEX_ARGV_CAPTURE_PATH = capturePath;
    ...
    try {
      ...
      if (typeof prevCapture === 'string') process.env.OMB_CODEX_ARGV_CAPTURE_PATH = prevCapture;
      else delete process.env.OMB_CODEX_ARGV_CAPTURE_PATH;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam preserves routed task roles into team state and worker launch args', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-role-routing-'));
    const binDir = join(cwd, 'bin');
    const fakeCodexPath = join(binDir, 'codex');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.codebuddy', 'prompts');
    await mkdir(binDir, { recursive: true });
...
const fs = require('fs');
const path = require('path');
const worker = String(process.env.OMB_TEAM_WORKER || process.env.OMB_TEAM_WORKER || 'unknown').replace(/[^a-zA-Z0-9_-]+/g, '__');
const out = path.join(process.env.OMB_ARGV_CAPTURE_DIR, worker + '.json');
...
`,
    ...
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevLegacyLaunchArgs = process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
    const prevOmbLaunchArgs = process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
    const prevCaptureDir = process.env.OMB_ARGV_CAPTURE_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMB_TEAM_WORKER_CLI = 'codex';
    delete process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
    process.env.OMB_ARGV_CAPTURE_DIR = captureDir;
    ...
    try {
      ...
      const worker1Instructions = await readFile(join(cwd, '.omb', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'AGENTS.md'), 'utf-8');
      const worker2Instructions = await readFile(join(cwd, '.omb', 'state', 'team', runtime.teamName, 'workers', 'worker-2', 'AGENTS.md'), 'utf-8');
      ...
      if (typeof prevLegacyLaunchArgs === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = prevLegacyLaunchArgs;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
      if (typeof prevOmbLaunchArgs === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = prevOmbLaunchArgs;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_ARGS;
      if (typeof prevCaptureDir === 'string') process.env.OMB_ARGV_CAPTURE_DIR = prevCaptureDir;
      else delete process.env.OMB_ARGV_CAPTURE_DIR;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam does not apply mini guidance for exact-match negatives like gpt-5.4-mini-tuned', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-mini-tuned-'));
    const binDir = join(cwd, 'bin');
    const fakeCodexPath = join(binDir, 'codex');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.codebuddy', 'prompts');
...
`,
    ...
    process.env.OMB_TEAM_WORKER_CLI = 'codex';
    process.env.OMB_ARGV_CAPTURE_DIR = captureDir;
    process.env.OMB_TEAM_WORKER_LAUNCH_ARGS = '--model gpt-5.4-mini-tuned';
    ...
    try {
      ...
      const workerInstructions = await readFile(join(cwd, '.omb', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'AGENTS.md'), 'utf-8');
  ...
  it('startTeam supports prompt launch mode without tmux and pipes trigger text via stdin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-'));
    const binDir = join(cwd, 'bin');
    const fakeCodexPath = join(binDir, 'codex');
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
`,
  ...
  it('startTeam relaunch re-creates HUD pane and re-registers reconcile hooks after shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-relaunch-hud-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    let runtime: TeamRuntime | null = null;
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-relaunch-hud-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
      ...
      if (typeof previousLaunchMode === 'string') process.env.OMB_TEAM_WORKER_LAUNCH_MODE = previousLaunchMode;
      else delete process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
      if (typeof previousWorkerCli === 'string') process.env.OMB_TEAM_WORKER_CLI = previousWorkerCli;
      else delete process.env.OMB_TEAM_WORKER_CLI;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam routes detached worktree worker inbox and mailbox triggers through leader-root state references', async () => {
    const repo = await initRepo();
    const toolingDir = await mkdtemp(join(tmpdir(), 'omb-runtime-worktree-tools-'));
...
const fs = require('fs');
const path = require('path');
const logDir = process.env.OMB_TEST_LOG_DIR;
fs.mkdirSync(logDir, { recursive: true });
fs.writeFileSync(path.join(logDir, 'env.json'), JSON.stringify({
  cwd: process.cwd(),
  teamStateRoot: process.env.OMB_TEAM_STATE_ROOT || process.env.OMB_TEAM_STATE_ROOT || '',
  worker: process.env.OMB_TEAM_WORKER || process.env.OMB_TEAM_WORKER || '',
}));
...
`,
    ...
    const prevLaunchMode = process.env.OMB_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevLogDir = process.env.OMB_TEST_LOG_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMB_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMB_TEAM_WORKER_CLI = 'codex';
    process.env.OMB_TEST_LOG_DIR = logDir;
    ...
    try {
      ...
      assert.match(
        startupLog,
        /\$OMB_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      assert.doesNotMatch(
        startupLog,
        /Read \.omb\/state\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      ...
      assert.equal(envLog.teamStateRoot, join(repo, '.omb', 'state'));
      ...
      assert.match(
        mailboxLog,
        /\$OMB_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/mailbox\/worker-1\.json/,
      );
      ...
      if (typeof prevLogDir === 'string') process.env.OMB_TEST_LOG_DIR = prevLogDir;
      else delete process.env.OMB_TEST_LOG_DIR;
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(repo, '.omb', 'state', 'team', 'team-detached-worktree-shutdown')), false);
    } finally {
      if (runtime) {
        await shutdownTeam(runtime.teamName, repo, { force: true }).catch(() => {});
      }
  ...
  it('resumeTeam preserves detached worktree metadata for live prompt workers', async () => {
    const repo = await initRepo();
    const binDir = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-bin-'));
    const fakeCodexPath = join(binDir, 'codex');
    const logDir = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-logs-'));
    const envLogPath = join(logDir, 'env.json');
    await writeFile(
      fakeCodexPath,
      `#!/usr/bin/env node
...
`,
    ...
    try {
      ...
      assert.equal(resumed?.config.workers[0]?.team_state_root, join(repo, '.omb', 'state'));
      ...
      assert.equal(identity.team_state_root, join(repo, '.omb', 'state'));
  ...
  it('shutdownTeam force-kills prompt workers that ignore SIGTERM', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-stubborn-'));
    const binDir = join(cwd, 'bin');
    const fakeCodexPath = join(binDir, 'codex');
    await mkdir(binDir, { recursive: true });
...
`,
  ...
  it('shutdownTeam reaps detached prompt-worker descendants', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-descendants-'));
    const binDir = join(cwd, 'bin');
    const fakeCodexPath = join(binDir, 'codex');
...
helper.unref();
writeFileSync(process.env.OMB_HELPER_PID_PATH, String(helper.pid));
process.stdin.resume();
setInterval(() => {}, 1000);
process.on('SIGTERM', () => process.exit(0));
`,
    ...
    let helperPid = 0;
    try {
      runtime = await withPromptModeCodexEnv(binDir, { OMB_HELPER_PID_PATH: helperPidPath }, () =>
        withoutTeamWorkerEnv(() =>
          startTeam(
  ...
  it('monitorTeam surfaces reclaimed work pickup attempts when an idle worker is available', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-reassign-reclaimed-'));
    ...
    try {
      ...
      const taskPath = join(cwd, '.omb', 'state', 'team', 'team-runtime-reassign', 'tasks', `task-${task.id}.json`);
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-runtime-reassign', 'manifest.v2.json');
      ...
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'working', current_task_id: task.id, updated_at: new Date().toISOString() }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-2', 'status.json'),
  ...
  it('monitorTeam reclaims expired task claims and surfaces the recovery in recommendations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-reclaim-'));
    const prevTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    delete process.env.OMB_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omb', 'state', 'team', 'team-runtime-reclaim', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeAtomic(taskPath, JSON.stringify(current, null, 2));
  ...
  it('monitorTeam keeps phase in team-verify when completed code tasks lack verification evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-verify-gate-'));
    const prevTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
    delete process.env.OMB_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omb', 'state', 'team', 'team-verify-gate', 'tasks', `task-${task.id}.json`);
  ...
  it('monitorTeam deactivates root team-state.json when the local phase becomes terminal', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-root-team-state-'));
    try {
      ...
      const rootStatePath = join(cwd, '.omb', 'state', 'team-state.json');
  ...
  it('monitorTeam emits worker_state_changed, worker_idle, and task_completed events based on transitions', async () => {
    ...
    try {
      ...
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'team-events', 'tasks', `task-${t.id}.json`),
        JSON.stringify({ ...t, status: 'completed', owner: 'worker-1' }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omb', 'state', 'team', 'team-events', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'idle', updated_at: new Date().toISOString() }, null, 2),
      );

      await monitorTeam('team-events', cwd);

      const eventsPath = join(cwd, '.omb', 'state', 'team', 'team-events', 'events', 'events.ndjson');
  ...
  it('monitorTeam persists integration ledger and cherry-picks unseen worker HEADs once', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'worker-1-branch', 'omb-runtime-worker-1-wt-');
  ...
  it('monitorTeam auto-commits dirty worker worktree before integration', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-ac-branch', 'omb-runtime-wk1-auto-commit-');
      ...
      assert.match(log, /omb\(team\): auto-checkpoint worker-1 \[1\]/, 'commit message should match auto-checkpoint pattern');
      ...
      const ledgerPath = join(repo, '.omb', 'reports', 'team-commit-hygiene', 'team-auto-commit.ledger.json');
  ...
  it('monitorTeam uses merge for worker cleanly ahead of leader (hybrid merge path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-merge-branch', 'omb-runtime-wk1-merge-clean-');
  ...
  it('monitorTeam integrates detached worker worktree by commit sha instead of merging leader HEAD into itself', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-detached-merge-branch', 'omb-runtime-wk1-detached-merge-');
      ...
      const ledgerPath = join(repo, '.omb', 'reports', 'team-commit-hygiene', 'team-merge-detached.ledger.json');
      const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8')) as {
        entries: Array<{
          operation: string;
          status: string;
  ...
  it('monitorTeam does not emit INTEGRATED when merge reports success but leader HEAD never advances', async () => {
    const repo = await initRepo();
    let workerPath = '';
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omb-runtime-fake-git-'));
    const previousPath = process.env.PATH;
    const previousFakeMode = process.env.OMB_FAKE_GIT_SUCCESS_NOOP;
    try {
      workerPath = await addWorktree(repo, 'wk1-merge-noadvance-branch', 'omb-runtime-wk1-merge-noadvance-');
      await writeFile(join(workerPath, 'feature.txt'), 'new feature\n', 'utf-8');
      execFileSync('git', ['add', 'feature.txt'], { cwd: workerPath, stdio: 'ignore' });
      execFileSync('git', ['commit', '-m', 'worker feature'], { cwd: workerPath, stdio: 'ignore' });
      ...
      await writeFile(
        ...
        `#!/usr/bin/env bash
set -euo pipefail
if [[ "\${OMB_FAKE_GIT_SUCCESS_NOOP:-}" == "merge" && "\${1:-}" == "merge" ]]; then
  exit 0
fi
exec "${realGit}" "$@"
`,
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath ?? ''}`;
      process.env.OMB_FAKE_GIT_SUCCESS_NOOP = 'merge';
      ...
      if (typeof previousFakeMode === 'string') process.env.OMB_FAKE_GIT_SUCCESS_NOOP = previousFakeMode;
      else delete process.env.OMB_FAKE_GIT_SUCCESS_NOOP;
      await rm(fakeBinDir, { recursive: true, force: true });
      if (workerPath) {
        await rm(workerPath, { recursive: true, force: true });
      }
  ...
  it('monitorTeam uses cherry-pick for diverged worker (hybrid cherry-pick path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-div-branch', 'omb-runtime-wk1-diverged-');
  ...
  it('monitorTeam rebases idle workers after integration lands on leader (cross-worker rebase)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-xr-branch', 'omb-runtime-wk1-cross-rebase-');
      worker2Path = await addWorktree(repo, 'wk2-xr-branch', 'omb-runtime-wk2-cross-rebase-');
      ...
      const ledgerPath = join(repo, '.omb', 'reports', 'team-commit-hygiene', 'team-cross-rebase.ledger.json');
      const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8')) as {
  ...
  it('monitorTeam auto-resolves conflicts with -X theirs (worker wins on leader)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-cr-branch', 'omb-runtime-wk1-conflict-resolve-');
  ...
  it('monitorTeam skips rebase for workers with "working" status (idle-only gating)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-gate-branch', 'omb-runtime-wk1-rebase-gate-');
      worker2Path = await addWorktree(repo, 'wk2-gate-branch', 'omb-runtime-wk2-rebase-gate-');
  ...
  it('monitorTeam aborts failed rebase and leaves worktree in clean state', async () => {
    ...
    try {
      ...
      worker1Path = await addWorktree(repo, 'wk1-rf-branch', 'omb-runtime-wk1-rebase-fail-');
      worker2Path = await addWorktree(repo, 'wk2-rf-branch', 'omb-runtime-wk2-rebase-fail-');
      ...
      const reportPath = join(repo, '.omb', 'state', 'team', 'team-rebase-fail', 'integration-report.md');
  ...
  it('shutdownTeam cleans up state even when tmux session doesn\'t exist', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown');
  ...
  it('shutdownTeam clean fast path ignores worker shutdown ack files', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-clean-fast-'));
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-clean-fast');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam blocks when pending tasks remain (shutdown gate)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-pending');
  ...
  it('shutdownTeam honors governance cleanup override when active tasks remain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-override-'));
    try {
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-override', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.governance = {
        ...(manifest.governance || {}),
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-override');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam honors legacy policy cleanup override after governance hydration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-legacy-'));
    try {
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-legacy', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.policy = {
        ...(manifest.policy || {}),
        cleanup_requires_all_workers_inactive: false,
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-legacy');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam requires explicit issue confirmation when failed tasks remain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-failed-'));
    try {
      ...
      await assert.rejects(
        ...
        /shutdown_confirm_issues_required:failed=1:rerun=omb team shutdown team-shutdown-gate-failed --confirm-issues/,
      );
  ...
  it('shutdownTeam force=true bypasses shutdown gate and cleans up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-force-'));
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-gate-force');
  ...
  it('shutdownTeam force=true emits shutdown_gate_forced audit event', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-gate-forced-event-'));
    try {
      ...
      const eventsPath = join(cwd, '.omb', 'state', 'team', 'team-gate-forced-event', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-gate-forced-event');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam handles persisted resize hook metadata during cleanup', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-resize-meta-'));
    try {
      const configPath = join(cwd, '.omb', 'state', 'team', 'team-resize-meta', 'config.json');
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-resize-meta', 'manifest.v2.json');
      ...
      config.resize_hook_name = 'omb_resize_team_resize_meta_test';
      config.resize_hook_target = 'omb-team-team-resize-meta:0';
      await writeFile(configPath, JSON.stringify(config, null, 2));
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
      manifest.resize_hook_name = 'omb_resize_team_resize_meta_test';
      manifest.resize_hook_target = 'omb-team-team-resize-meta:0';
      await writeFile(manifestPath, JSON.stringify(manifest, null, 2));

      await shutdownTeam('team-resize-meta', cwd);
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-resize-meta');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam continues cleanup when resize hook unregister fails while session remains active', async () => {
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-fake-tmux-',
...
case "$1" in
  -V)
    ...
    ;;
  list-sessions)
    echo "omb-team-team-shutdown-gate-failed"
    exit 0
    ;;
  set-hook)
    if [ "\${2:-}" = "-u" ]; then
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const config = JSON.parse(await readFile(configPath, 'utf-8')) as Record<string, unknown>;
          config.tmux_session = 'omb-team-team-shutdown-gate-failed';
          config.resize_hook_name = 'omb_resize_team_shutdown_gate_failed_test';
          config.resize_hook_target = 'omb-team-team-shutdown-gate-failed:0';
          await writeFile(configPath, JSON.stringify(config, null, 2));
          const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
          manifest.tmux_session = 'omb-team-team-shutdown-gate-failed';
          manifest.resize_hook_name = 'omb_resize_team_shutdown_gate_failed_test';
          manifest.resize_hook_target = 'omb-team-team-shutdown-gate-failed:0';
          await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
          ...
          const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
          assert.match(tmuxLog, /set-hook -u -t omb-team-team-shutdown-gate-failed:0 client-resized\[\d+\]/);
          assert.match(tmuxLog, /kill-session -t omb-team-team-shutdown-gate-failed/);
        },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
  ...
  it('shutdownTeam emits shutdown_ack event when worker ack is received', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omb', 'state', 'team', 'team-ack-evt', 'events', 'events.ndjson');
  ...
  it('shutdownTeam emits shutdown_ack event with accept reason for accepted acks', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omb', 'state', 'team', 'team-ack-accept', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-ack-accept');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam force=true ignores rejection and cleans up team state', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-force');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam ignores stale rejection ack from a prior request', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-stale-ack');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam confirmIssues=true allows failed-task shutdown without worker ack handshake', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-confirm-issues-'));
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-confirm-issues');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam applies best-effort teardown even when worker pane is already dead', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-dead-pane-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-shutdown-dead-pane-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam reconciles persisted worker panes with live tmux panes before teardown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-pane-reconcile-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-shutdown-pane-reconcile-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
  ...
  list-panes)
    case "$*" in
      ...
      *"-t leader:0 -F #{pane_id}"*"#{pane_current_command}"*)
        printf "%%11\\tzsh\\tzsh\\n%%12\\tnode\\tnode /tmp/bin/omb.js hud --watch\\n%%13\\tcodex\\tcodex\\n%%14\\tcodex\\tcodex\\n"
        if [ -f "$restored_marker" ]; then
          printf "%%44\\tnode\\tnode /tmp/bin/omb.js hud --watch\\n"
        fi
        exit 0
        ;;
      *)
...
`,
  ...
  it('shutdownTeam skips prekill and keeps the leader pane alive on native Windows split-pane shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-win32-split-'));
    try {
      await withNativeWindowsPlatform(async () => {
        await withMockTmuxFixture(
          {
            dirPrefix: 'omb-runtime-shutdown-win32-split-bin-',
            tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
          ...
          async ({ tmuxLogPath }) => {
            ...
            const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-win32-split');
            assert.equal(existsSync(teamRoot), false);
  ...
  it('shutdownTeam skips prekill and keeps the leader pane alive on shared-session shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-shared-session-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-shutdown-shared-session-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-shutdown-shared-session');
          assert.equal(existsSync(teamRoot), false);
  ...
  it('shutdownTeam restores a standalone HUD pane after tearing down the team HUD', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-restore-hud-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-shutdown-restore-hud-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam preserves leader exclusion while tearing down the hud pane', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-shutdown-exclusions-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-shutdown-exclusions-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          assert.ok(config);
          if (!config) return;
          config.tmux_session = 'omb-team-team-shutdown-exclusions';
          config.leader_pane_id = '%11';
          config.hud_pane_id = '%12';
  ...
  it('shutdownTeam still requires confirm-issues on failed tasks', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-normal-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omb', 'state', 'team', 'team-normal-gate');
      assert.equal(existsSync(teamRoot), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('resumeTeam returns null for prompt teams when worker handles are missing after restart', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-prompt-resume-'));
    ...
    try {
      ...
      const configPath = join(cwd, '.omb', 'state', 'team', 'team-prompt-resume', 'config.json');
  ...
  it('assignTask enforces delegation_only policy for leader-fixed worker', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-delegation', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('assignTask rolls back claim when inbox write fails after claim', async () => {
    ...
    try {
      ...
      const workerDir = join(cwd, '.omb', 'state', 'team', 'team-inbox-fail', 'workers', 'worker-1');
  ...
  it('assignTask enforces plan approval for code-change tasks when required', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-approval', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('monitorTeam does not re-notify already-notified mailbox messages (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-no-spam-'));
    try {
      ...
      const mailboxDir = join(cwd, '.omb', 'state', 'team', 'team-no-spam', 'mailbox');
  ...
  it('monitorTeam only notifies once per new message even without notified_at (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-new-msg-'));
    try {
      await initTeamState('team-new-msg', 'new msg test', 'executor', 1, cwd);

      const mailboxDir = join(cwd, '.omb', 'state', 'team', 'team-new-msg', 'mailbox');
      await mkdir(mailboxDir, { recursive: true });
  ...
  it('monitorTeam does not emit duplicate task_completed when transitionTaskStatus completed the task first (issue #161)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-no-dup-'));
    try {
      ...
      const eventsPath = join(cwd, '.omb', 'state', 'team', 'team-no-dup', 'events', 'events.ndjson');
      const content = await readFile(eventsPath, 'utf-8');
  ...
  it('sendWorkerMessage keeps hook-preferred duplicate leader mailbox sends idempotent after notification', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-leader-dedupe-notified-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-leader-dedupe-notified-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path persists leader mailbox guidance when leader pane exists', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-leader-inject-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-leader-inject-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage keeps failed hook receipts failed when fallback mailbox persistence confirms delivery', { concurrency: false }, async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-leader-failed-receipt-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omb-runtime-leader-failed-receipt-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path for leader waits for receipt then falls back to mailbox persistence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-leader-hook-'));
    try {
  ...
  it('sendWorkerMessage transport_direct keeps leader-fixed request pending when leader_pane_id missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-runtime-leader-direct-'));
    try {
      ...
      const manifestPath = join(cwd, '.omb', 'state', 'team', 'team-leader-direct', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8'));
