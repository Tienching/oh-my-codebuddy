...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omb-shutdown-fallback-'));
...
describe('shutdown fallback worktree reports', () => {
  it('shutdownTeam checkpoints dirty detached worker worktrees, merges them, and writes a report', async () => {
    ...
    const binDir = await mkdtemp(join(tmpdir(), 'omb-shutdown-fallback-bin-'));
...
`,
    ...
    try {
      ...
      const reportPath = join(preservedWorktreePath as string, '.omb', 'diff.md');
