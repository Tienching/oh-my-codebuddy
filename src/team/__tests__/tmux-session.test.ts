...
const QUEUED_AFTER_TOOL_CALL_CAPTURE = `• Messages to be submitted after next tool call (press esc to interrupt and send immediately)
  ↳ Read $OMB_TEAM_STATE_ROOT/team/demo/workers/worker-1/inbox.md, work now, report progress
...
describe('sendToWorker validation', () => {
  it('rejects text over 200 chars', async () => {
    await assert.rejects(
      sendToWorker('omb-team-x', 1, 'a'.repeat(200)),
      /< 200/i
    );
  });
  ...
  it('rejects injection marker', async () => {
    await assert.rejects(
      sendToWorker('omb-team-x', 1, `hello [OMB_TMUX_INJECT]`),
      /marker/i
    );
  });
...
`,
      async ({ logPath }) => {
        await sendToWorker('omb-team-x', 1, 'check inbox');
        const log = await readFile(logPath, 'utf-8');
        const acceptIndex = log.indexOf('send-keys -t omb-team-x:1 -l -- 2');
        const submitIndex = log.indexOf('send-keys -t omb-team-x:1 -l -- check inbox');
  ...
  it('ignores stale queued-next-tool-call banner text that only survives in scrollback history', async () => {
    await withMockTmuxFixture(
      'omb-tmux-codex-stale-queued-scrollback-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
...
`,
      async ({ logPath }) => {
        await sendToWorker('omb-team-x', 1, 'check inbox');
        const log = await readFile(logPath, 'utf-8');
        const enterCount = (log.match(/send-keys -t omb-team-x:1 C-m/g) || []).length;
        assert.equal(
          enterCount,
          2,
          `expected only the baseline submit presses when the queued banner is stale scrollback:\n${log}`,
        );
        assert.match(log, /capture-pane -t omb-team-x:1 -p/);
        assert.match(log, /capture-pane -t omb-team-x:1 -p -S -80/);
      },
    );
  });
  ...
  it('keeps nudging Codex when the visible pane still shows a live queued-next-tool-call banner', async () => {
    await withMockTmuxFixture(
      'omb-tmux-codex-visible-queued-submit-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
text_sent_file="$state_dir/text-sent"
...
`,
  ...
  it('fails closed when the visible queued-next-tool-call banner never clears after the final submit round', async () => {
    await withMockTmuxFixture(
      'omb-tmux-codex-stuck-queued-submit-',
      (logPath) => `#!/bin/sh
set -eu
state_dir="$(dirname "${logPath}")"
text_sent_file="$state_dir/text-sent"
...
`,
      async ({ logPath }) => {
        await assert.rejects(
          () => sendToWorker('omb-team-x', 1, 'check inbox'),
...
describe('buildWorkerStartupCommand', () => {
  it('auto-selects gemini worker CLI from gemini model', () => {
    ...
    const prevCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.SHELL = '/bin/bash';
    delete process.env.OMB_TEAM_WORKER_CLI; // auto
    ...
    try {
      ...
      if (typeof prevCli === 'string') process.env.OMB_TEAM_WORKER_CLI = prevCli;
  ...
  it('respects explicit OMB_TEAM_WORKER_CLI override', () => {
    const prevShell = process.env.SHELL;
    const prevCli = process.env.OMB_TEAM_WORKER_CLI;
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.SHELL = '/bin/bash';
    ...
    try {
      ...
      process.env.OMB_TEAM_WORKER_CLI = 'claude';
  ...
  it('uses zsh with ~/.zshrc and exec codebuddy by default', () => {
    const prevShell = process.env.SHELL;
    ...
    try {
      ...
      assert.match(cmd, /OMB_TEAM_WORKER=alpha\/worker-2/);
      assert.match(cmd, /'\/bin\/zsh' -lc/);
      assert.match(cmd, /source ~\/\.zshrc/);
      assert.match(cmd, /exec .*codebuddy/);
    } finally {
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
  ...
  it('uses bash with ~/.bashrc and preserves launch args for codebuddy', () => {
    const prevShell = process.env.SHELL;
    process.env.SHELL = '/bin/bash';
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = '0';
  ...
  it('injects canonical team state env vars when provided', () => {
    ...
    try {
      const cmd = buildWorkerStartupCommand(
        ...
        '/tmp/worker-cwd',
        {
          OMB_TEAM_STATE_ROOT: '/tmp/leader/.omb/state',
          OMB_TEAM_LEADER_CWD: '/tmp/leader',
        },
      );
      assert.match(cmd, /OMB_TEAM_STATE_ROOT=\/tmp\/leader\/\.omb\/state/);
      assert.match(cmd, /OMB_TEAM_LEADER_CWD=\/tmp\/leader/);
    } finally {
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
  ...
  it('resolves POSIX leader paths before building fish worker startup commands', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omb-worker-startup-posix-'));
    const prevPath = process.env.PATH;
    const prevShell = process.env.SHELL;
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.PATH = fakeBin;
    ...
    try {
      ...
      assert.match(cmd, new RegExp(escapeRegExp(`OMB_LEADER_NODE_PATH=${nodePath}`)));
      assert.match(cmd, new RegExp(escapeRegExp(`OMB_LEADER_CLI_PATH=${codexPath}`)));
  ...
  it('translates reasoning override args in the default codebuddy worker command', () => {
    const prevShell = process.env.SHELL;
    process.env.SHELL = '/bin/bash';
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = '0';
    try {
      ...
      assert.match(cmd, /exec .*codebuddy/);
      assert.match(cmd, /--effort/);
      assert.match(cmd, /xhigh/);
    } finally {
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
  ...
  it('injects system prompt override by default for codebuddy workers', () => {
    const prevShell = process.env.SHELL;
    process.env.SHELL = '/bin/bash';
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstr = process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT; // default enabled
    delete process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    try {
      const cmd = buildWorkerStartupCommand('alpha', 1, [], '/tmp/project');
      assert.match(cmd, /--system-prompt-file/);
      ...
      if (typeof prevInstr === 'string') process.env.OMB_MODEL_INSTRUCTIONS_FILE = prevInstr;
      else delete process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    }
  });
  ...
  it('uses per-worker OMB_MODEL_INSTRUCTIONS_FILE from extraEnv when building process launch spec', () => {
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstr = process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    delete process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['-c', 'model_reasoning_effort="low"'],
        '/tmp/project',
        { OMB_MODEL_INSTRUCTIONS_FILE: '/tmp/project/.omb/state/team/alpha/workers/worker-1/AGENTS.md' },
        'codex',
      );
      const joined = spec.args.join(' ');
      assert.match(joined, /model_reasoning_effort="low"/);
      assert.match(joined, /model_instructions_file="\/tmp\/project\/.omb\/state\/team\/alpha\/workers\/worker-1\/AGENTS\.md"/);
    } finally {
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
      else delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
      if (typeof prevInstr === 'string') process.env.OMB_MODEL_INSTRUCTIONS_FILE = prevInstr;
  ...
  it('does not inject a duplicate system prompt override when already provided in launch args', () => {
    const prevShell = process.env.SHELL;
    process.env.SHELL = '/bin/bash';
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT; // default enabled
    try {
      ...
      const matches = cmd.match(/--system-prompt-file/g) || [];
      assert.equal(matches.length, 1);
  ...
  it('translates system prompt file path for MSYS2/Git Bash environments', () => {
    const prevShell = process.env.SHELL;
    const prevBypass = process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstructions = process.env.OMB_MODEL_INSTRUCTIONS_FILE;
    const prevMsystem = process.env.MSYSTEM;
    const origPlatform = Object.getOwnPropertyDescriptor(process, 'platform');
    process.env.SHELL = '/bin/bash';
    delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT; // default enabled
    process.env.OMB_MODEL_INSTRUCTIONS_FILE = 'C:\\repo\\AGENTS.md';
    process.env.MSYSTEM = 'MINGW64';
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      const cmd = buildWorkerStartupCommand('alpha', 1, [], 'C:\\repo');
      assert.match(cmd, /--system-prompt-file/);
      assert.match(cmd, /\/c\/repo\/AGENTS\.md/);
    } finally {
      if (origPlatform) Object.defineProperty(process, 'platform', origPlatform);
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      ...
      if (typeof prevInstructions === 'string') process.env.OMB_MODEL_INSTRUCTIONS_FILE = prevInstructions;
      else delete process.env.OMB_MODEL_INSTRUCTIONS_FILE;
  ...
  it('uses a native PowerShell startup command on native Windows instead of /bin/sh -lc', async () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      const codebuddyPs1Path = join(fakeBin, 'codebuddy.ps1');
      await writeFile(codebuddyPs1Path, '');
      ...
      assert.match(decoded, /\$env:OMB_TEAM_WORKER = 'alpha\/worker-1'/);
      assert.match(decoded, new RegExp(escapeRegExp(`'-File' '${codebuddyPs1Path}'`)));
      assert.match(decoded, /'--model' 'gpt-5'/);
      assert.match(decoded, /'--dangerously-skip-permissions'/);
    } finally {
      if (origPlatform) Object.defineProperty(process, 'platform', origPlatform);
      if (typeof prevPath === 'string') process.env.PATH = prevPath;
      else delete process.env.PATH;
...
describe('team worker CLI helpers', () => {
  it('resolveTeamWorkerCli auto-detects specialized models and defaults to codebuddy', () => {
    ...
    assert.equal(resolveTeamWorkerCli(['--model', 'gpt-5'], {}), 'codebuddy');
    assert.equal(resolveTeamWorkerCli([], {}), 'codebuddy');
  });

  it('resolveTeamWorkerCli accepts explicit gemini override', () => {
    assert.equal(resolveTeamWorkerCli([], { OMB_TEAM_WORKER_CLI: 'gemini' }), 'gemini');
  });

  it('resolveTeamWorkerCliPlan accepts gemini in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(3, [], { OMB_TEAM_WORKER_CLI_MAP: 'codex,gemini,claude' });
  ...
  it('resolveTeamWorkerCliPlan supports mixed per-worker CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      4,
      [],
      { OMB_TEAM_WORKER_CLI_MAP: 'codex,codex,gemini,claude' },
    );
  ...
  it('resolveTeamWorkerCliPlan accepts single-value map and expands to all workers', () => {
    const plan = resolveTeamWorkerCliPlan(
      3,
      [],
      { OMB_TEAM_WORKER_CLI_MAP: 'claude' },
    );
  ...
  it('resolveTeamWorkerCliPlan supports auto entries in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      2,
      ['--model', 'claude-3-7-sonnet'],
      { OMB_TEAM_WORKER_CLI_MAP: 'auto,codex' },
    );
    assert.deepEqual(plan, ['claude', 'codex']);
  });

  it('resolveTeamWorkerCliPlan auto entries ignore OMB_TEAM_WORKER_CLI override', () => {
    const plan = resolveTeamWorkerCliPlan(
      1,
      ['--model', 'claude-3-7-sonnet'],
      {
        OMB_TEAM_WORKER_CLI: 'codex',
        OMB_TEAM_WORKER_CLI_MAP: 'auto',
      },
    );
  ...
  it('resolveTeamWorkerCliPlan rejects map lengths that do not match workerCount', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(4, [], { OMB_TEAM_WORKER_CLI_MAP: 'codex,claude' }),
      /expected 1 or 4/i,
    );
  });
  ...
  it('resolveTeamWorkerCliPlan rejects empty entries in CLI map', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(2, [], { OMB_TEAM_WORKER_CLI_MAP: 'codex,' }),
      /empty entries are not allowed/i,
    );
  });

  it('resolveTeamWorkerCliPlan reports invalid entry errors with OMB_TEAM_WORKER_CLI_MAP', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(1, [], { OMB_TEAM_WORKER_CLI_MAP: 'claudee' }),
      /OMB_TEAM_WORKER_CLI_MAP/i,
    );
  });

  it('resolveWorkerCliForSend prioritizes explicit worker CLI over map/global', () => {
    assert.equal(
      resolveWorkerCliForSend(2, 'claude', [], { OMB_TEAM_WORKER_CLI_MAP: 'codex,codex' }),
      'claude',
    );
  });
  ...
  it('resolveWorkerCliForSend resolves per-worker map entry by index', () => {
    assert.equal(
      resolveWorkerCliForSend(2, undefined, [], { OMB_TEAM_WORKER_CLI_MAP: 'codex,claude' }),
      'claude',
    );
  });
...
describe('team worker launch mode helpers', () => {
  it('resolveTeamWorkerLaunchMode defaults to interactive and accepts prompt', () => {
    ...
    assert.equal(resolveTeamWorkerLaunchMode({ OMB_TEAM_WORKER_LAUNCH_MODE: 'interactive' }), 'interactive');
    assert.equal(resolveTeamWorkerLaunchMode({ OMB_TEAM_WORKER_LAUNCH_MODE: 'prompt' }), 'prompt');
    assert.equal(resolveTeamWorkerLaunchMode({ OMB_TEAM_WORKER_LAUNCH_MODE: ' PROMPT ' }), 'prompt');
  });

  it('resolveTeamWorkerLaunchMode rejects unsupported values', () => {
    assert.throws(
      () => resolveTeamWorkerLaunchMode({ OMB_TEAM_WORKER_LAUNCH_MODE: 'tmux' }),
  ...
  it('buildWorkerProcessLaunchSpec returns command/args/env for prompt process spawn', () => {
    ...
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['--model', 'gpt-5.3-codex'],
        '/tmp/workspace',
        { OMB_TEAM_STATE_ROOT: '/tmp/workspace/.omb/state' },
        'codex',
      );
      ...
      assert.equal(spec.env.OMB_TEAM_WORKER, 'alpha-team/worker-2');
      assert.equal(spec.env.OMB_TEAM_STATE_ROOT, '/tmp/workspace/.omb/state');
    } finally {
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
      else delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    }
  ...
  it('buildWorkerProcessLaunchSpec includes leader node and CLI path env vars', () => {
    ...
    try {
      ...
      assert.ok(
        typeof spec.env.OMB_LEADER_NODE_PATH === 'string' && spec.env.OMB_LEADER_NODE_PATH.length > 0,
        'OMB_LEADER_NODE_PATH must be set',
      );
      assert.ok(
        typeof spec.env.OMB_LEADER_CLI_PATH === 'string' && spec.env.OMB_LEADER_CLI_PATH.length > 0,
        'OMB_LEADER_CLI_PATH must be set',
      );
      // command matches the resolved CLI path stored in env
      assert.equal(spec.command, spec.env.OMB_LEADER_CLI_PATH);
    } finally {
      if (typeof prevBypass === 'string') process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
      else delete process.env.OMB_BYPASS_DEFAULT_SYSTEM_PROMPT;
    }
  ...
  it('buildWorkerProcessLaunchSpec wraps Windows PowerShell shims for prompt workers', async () => {
    ...
    try {
      ...
      assert.equal(spec.env.OMB_LEADER_CLI_PATH, codexPs1Path);
...
describe('tmux-dependent functions when tmux is unavailable', () => {
  ...
  it('waitForWorkerReady uses visible capture-pane argv without tail flags', async () => {
    await withMockTmuxFixture(
      'omb-tmux-worker-ready-visible-capture-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omb-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omb-team-x:1 -p/);
        assert.doesNotMatch(log, /capture-pane -t omb-team-x:1 -p -S/);
      },
    );
  });
...
`,
  ...
  it('waitForWorkerReady falls back to recent scrollback when a live Codex viewport pushes the prompt below the visible slice', async () => {
    await withMockTmuxFixture(
      'omb-tmux-worker-ready-scrollback-fallback-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
  ...
  it('waitForWorkerReady does not consult scrollback when the visible slice is only status text', async () => {
    await withMockTmuxFixture(
      'omb-tmux-worker-ready-no-scrollback-status-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omb-team-x', 1, 250), false);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omb-team-x:1 -p/);
        assert.doesNotMatch(log, /capture-pane -t omb-team-x:1 -p -S -80/);
      },
    );
  });
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omb-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /send-keys -t omb-team-x:1 -l -- 2/);
        assert.match(log, /send-keys -t omb-team-x:1 C-m/);
      },
    );
  });

  it('waitForWorkerReady leaves the Claude bypass prompt untouched when auto-accept is disabled', async () => {
    const previousAutoAccept = process.env.OMB_TEAM_AUTO_ACCEPT_BYPASS;
    process.env.OMB_TEAM_AUTO_ACCEPT_BYPASS = '0';
    try {
      await withMockTmuxFixture(
        'omb-tmux-claude-bypass-blocked-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      ...
      if (typeof previousAutoAccept === 'string') process.env.OMB_TEAM_AUTO_ACCEPT_BYPASS = previousAutoAccept;
      else delete process.env.OMB_TEAM_AUTO_ACCEPT_BYPASS;
    }
  });

  it('waitForWorkerReady returns false on timeout', () => {
    withEmptyPath(() => {
      assert.equal(waitForWorkerReady('omb-team-x', 1, 1), false);
    });
  });
});
...
describe('dismissTrustPromptIfPresent capture shape', () => {
  it('uses visible capture-pane argv without tail flags', async () => {
    const previousAutoTrust = process.env.OMB_TEAM_AUTO_TRUST;
    delete process.env.OMB_TEAM_AUTO_TRUST;
    try {
      await withMockTmuxFixture(
        'omb-tmux-dismiss-trust-visible-capture-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
        async ({ logPath }) => {
          assert.equal(dismissTrustPromptIfPresent('omb-team-x', 1), true);
          const log = await readFile(logPath, 'utf-8');
          assert.match(log, /capture-pane -t omb-team-x:1 -p/);
          assert.doesNotMatch(log, /capture-pane -t omb-team-x:1 -p -S/);
        },
      ...
      if (typeof previousAutoTrust === 'string') process.env.OMB_TEAM_AUTO_TRUST = previousAutoTrust;
      else delete process.env.OMB_TEAM_AUTO_TRUST;
    }
  });
});
...
describe('dismissTrustPromptIfPresent', () => {
  it('returns false when tmux is unavailable', () => {
    withEmptyPath(() => {
      assert.equal(dismissTrustPromptIfPresent('omb-team-x', 1), false);
    });
  });

  it('returns false when OMB_TEAM_AUTO_TRUST is disabled', () => {
    const prev = process.env.OMB_TEAM_AUTO_TRUST;
    process.env.OMB_TEAM_AUTO_TRUST = '0';
    try {
      assert.equal(dismissTrustPromptIfPresent('omb-team-x', 1), false);
    } finally {
      if (typeof prev === 'string') process.env.OMB_TEAM_AUTO_TRUST = prev;
      else delete process.env.OMB_TEAM_AUTO_TRUST;
    }
  });

  it('returns false when OMB_TEAM_AUTO_TRUST is unset (auto-trust enabled) but tmux unavailable', () => {
    const prev = process.env.OMB_TEAM_AUTO_TRUST;
    delete process.env.OMB_TEAM_AUTO_TRUST;
    try {
      withEmptyPath(() => {
...
describe('isWorkerAlive', () => {
  it('does not require pane_current_command to match "codex"', () => {
    ...
    withEmptyPath(() => {
      assert.equal(isWorkerAlive('omb-team-x', 1), false);
    });
  });
});
...
describe('enableMouseScrolling', () => {
  it('returns false when tmux is unavailable', () => {
    ...
    withEmptyPath(() => {
      assert.equal(enableMouseScrolling('omb-team-x'), false);
    });
  });
...
describe('enableMouseScrolling scroll and copy setup (issue #206)', () => {
  ...
  it('does not throw when WSL2 env is set and tmux is unavailable (regression + #206)', () => {
    ...
    try {
      withEmptyPath(() => {
        assert.doesNotThrow(() => enableMouseScrolling('omb-team-x'));
      });
    } finally {
      if (typeof prev === 'string') process.env.WSL_DISTRO_NAME = prev;
      else delete process.env.WSL_DISTRO_NAME;
...
`,
      async ({ logPath }) => {
        assert.equal(enableMouseScrolling('omb-team-x'), true);
        const tmuxLog = await readFile(logPath, 'utf-8');
        assert.match(tmuxLog, /set-option -t omb-team-x mouse on/);
        assert.match(tmuxLog, /set-option -t omb-team-x set-clipboard on/);
...
describe('killWorker leader pane guard', () => {
  it('returns immediately when workerPaneId matches leaderPaneId', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omb-team-x:0', 1, '%5', '%5'));
    });
  });
  ...
  it('proceeds (gracefully) when pane ids differ', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omb-team-x:0', 1, '%5', '%6'));
    });
  });

  it('proceeds when leaderPaneId is not provided', () => {
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omb-team-x:0', 1, '%5'));
    });
  });
});
