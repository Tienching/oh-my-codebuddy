...
describe('followup-planner', () => {
  it('resolves available agent types from explicit prompt directories', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'omb-followup-roster-'));
    try {
  ...
  it('builds concrete team staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omb team 3:executor "Fix flaky integration tests and update README"');
  ...
  it('builds concrete ralph staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omb ralph "Investigate auth regression and verify the fix"');
