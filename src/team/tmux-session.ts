...
function normalizeTeamWorkerCliMode(raw: string | undefined, sourceEnv: string = OMB_TEAM_WORKER_CLI_ENV): TeamWorkerCliMode {
  ...
  throw new Error(`Invalid ${sourceEnv} value "${raw}". Expected: auto, codex, claude, gemini, codebuddy`);
}
...
export function resolveTeamWorkerLaunchMode(
  ...
  throw new Error(`Invalid ${launchModeEnvUsed} value "${launchModeValueUsed}". Expected: interactive, prompt`);
}
...
export function resolveTeamWorkerCliPlan(
  ...
  if (entries.length === 0 || entries.every((part) => part.length === 0)) {
    throw new Error(
      `Invalid ${mapEnvUsed} value "${mapValueUsed}". `
        + `Expected comma-separated values: auto|codebuddy|codex|claude|gemini.`,
    );
  }
  ...
  if (entries.length !== 1 && entries.length !== workerCount) {
    throw new Error(
      `Invalid ${mapEnvUsed} length ${entries.length}; `
  ...
  return expanded.map((entry) => {
    const mode = normalizeTeamWorkerCliMode(entry, mapEnvUsed);
    return mode === 'auto' ? fallbackAutoFromArgs() : mode;
  });
}

function parseTmuxTomlStringValue(rawValue: string): string {
...
function parseLegacyConfigOverride(rawValue: string): { kind: 'system-prompt-file' | 'reasoning'; value: string } | null {
  const match = rawValue.match(/^\s*([A-Za-z0-9_.-]+)\s*=\s*(.*?)\s*$/);
  if (!match) return null;
  const [, key, encodedValue] = match;
  const value = parseTmuxTomlStringValue(encodedValue);
  if (key === MODEL_INSTRUCTIONS_FILE_KEY && value !== '') {
...
function normalizeCodebuddyWorkerLaunchArgs(args: string[]): string[] {
...
function normalizeCodexWorkerLaunchArgs(args: string[]): string[] {
  const normalized: string[] = [];
  let wantsBypass = false;
  let hasLegacyBypass = false;

  for (let index = 0; index < args.length; index += 1) {
    const arg = args[index];

    if (arg === MADMAX_FLAG || arg === CODEBUDDY_BYPASS_FLAG) {
      wantsBypass = true;
      continue;
    }

    if (arg === CODEBUDDY_LEGACY_BYPASS_FLAG) {
      wantsBypass = true;
      if (!hasLegacyBypass) {
        normalized.push(arg);
        hasLegacyBypass = true;
      }
      continue;
    }

    if (arg === CODEBUDDY_EFFORT_FLAG) {
      const next = args[index + 1];
      if (typeof next === 'string' && next.trim() !== '') {
        normalized.push(CONFIG_FLAG, `${REASONING_KEY}="${next.trim()}"`);
        index += 1;
        continue;
      }
    }
    ...
    if (arg === CODEBUDDY_SYSTEM_PROMPT_FILE_FLAG) {
      const next = args[index + 1];
      if (typeof next === 'string' && next.trim() !== '') {
        normalized.push(CONFIG_FLAG, `${MODEL_INSTRUCTIONS_FILE_KEY}="${next.trim().replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`);
        index += 1;
        continue;
      }
    }
    ...
    if (arg.startsWith(`${CODEBUDDY_SYSTEM_PROMPT_FILE_FLAG}=`)) {
      ...
      if (value !== '') {
        normalized.push(CONFIG_FLAG, `${MODEL_INSTRUCTIONS_FILE_KEY}="${parseTmuxTomlStringValue(value).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`);
        continue;
      }
    }
  ...
  if (wantsBypass && !hasLegacyBypass) {
    normalized.push(CODEBUDDY_LEGACY_BYPASS_FLAG);
  }
...
export function translateWorkerLaunchArgsForCli(workerCli: TeamWorkerCli, args: string[], initialPrompt?: string): string[] {
  if (workerCli === 'codebuddy') return normalizeCodebuddyWorkerLaunchArgs(args);
  if (workerCli === 'codex') return normalizeCodexWorkerLaunchArgs(args);
  if (workerCli === 'gemini') {
...
function resolveWorkerLaunchArgs(extraArgs: string[] = [], cwd: string = process.cwd(), env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const wantsBypass = process.argv.includes(CODEBUDDY_BYPASS_FLAG) || process.argv.includes(MADMAX_FLAG);
  if (wantsBypass && !merged.includes(CODEBUDDY_BYPASS_FLAG)) {
    merged.push(CODEBUDDY_BYPASS_FLAG);
  }
...
export function buildWorkerProcessLaunchSpec(
  ...
  const effectiveCliLaunchArgs = (() => {
    if (workerCli === 'codex') {
      return cliLaunchArgs.includes(CODEBUDDY_LEGACY_BYPASS_FLAG)
        ? cliLaunchArgs
        : [...cliLaunchArgs, CODEBUDDY_LEGACY_BYPASS_FLAG];
    }
    if (workerCli === 'codebuddy') {
      return cliLaunchArgs.includes(CODEBUDDY_BYPASS_FLAG) || hasExplicitBypassPermissionMode(cliLaunchArgs)
        ? cliLaunchArgs
        : [...cliLaunchArgs, CODEBUDDY_BYPASS_FLAG];
    }
