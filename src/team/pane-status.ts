...
export async function readTeamPaneStatus(
  ...
  const sparkshellCommands = Object.fromEntries(
    [
      leaderPaneId ? ['leader', `omb sparkshell --tmux-pane ${leaderPaneId} --tail-lines ${tailLines}`] : null,
      hudPaneId ? ['hud', `omb sparkshell --tmux-pane ${hudPaneId} --tail-lines ${tailLines}`] : null,
      ...Object.entries(workerPanes).map(([workerName, paneId]) => [
        workerName,
        `omb sparkshell --tmux-pane ${paneId} --tail-lines ${tailLines}`,
      ] as const),
    ].filter((entry): entry is [string, string] => entry !== null),
  );
  ...
  const inspectStateRootForTarget = (_target: string): string | null => (
    snapshot?.teamName ? ombStateDir(cwd) : null
  );
  ...
  const recommendedInspectTaskClaimLockPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      const stateRoot = inspectStateRootForTarget(target);
      return [target, taskId && snapshot?.teamName && stateRoot ? join(stateRoot, 'team', snapshot.teamName, 'claims', `task-${taskId}.lock`) : null];
    }),
  );
  ...
  const recommendedInspectTaskPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      const stateRoot = inspectStateRootForTarget(target);
      return [target, taskId && snapshot?.teamName && stateRoot ? join(stateRoot, 'team', snapshot.teamName, 'tasks', `task-${taskId}.json`) : null];
    }),
  );
  const recommendedInspectApprovalPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      const stateRoot = inspectStateRootForTarget(target);
      return [target, taskId && snapshot?.teamName && stateRoot ? join(stateRoot, 'team', snapshot.teamName, 'approvals', `task-${taskId}.json`) : null];
    }),
  );
  const recommendedInspectWorkerStateDirs = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target) : null,
    ]),
  );
  const recommendedInspectWorkerStatusPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'status.json') : null,
    ]),
  );
  const recommendedInspectWorkerHeartbeatPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'heartbeat.json') : null,
    ]),
  );
  const recommendedInspectWorkerIdentityPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'identity.json') : null,
    ]),
  );
  const recommendedInspectWorkerInboxPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'inbox.md') : null,
    ]),
  );
  const recommendedInspectWorkerMailboxPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'mailbox', `${target}.json`) : null,
    ]),
  );
  const recommendedInspectWorkerShutdownRequestPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'shutdown-request.json') : null,
    ]),
  );
  const recommendedInspectWorkerShutdownAckPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'workers', target, 'shutdown-ack.json') : null,
    ]),
  );
  const recommendedInspectTeamConfigPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'config.json') : null,
    ]),
  );
  const recommendedInspectTeamManifestPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'manifest.v2.json') : null,
    ]),
  );
  const recommendedInspectTeamEventsPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'events', 'events.ndjson') : null,
    ]),
  );
  const recommendedInspectTeamDispatchPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'dispatch', 'requests.json') : null,
    ]),
  );
  const recommendedInspectTeamDirPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName) : null,
    ]),
  );
  const recommendedInspectTeamPhasePaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'phase.json') : null,
    ]),
  );
  const recommendedInspectTeamMonitorSnapshotPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'monitor-snapshot.json') : null,
    ]),
  );
  const recommendedInspectTeamSummarySnapshotPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName && inspectStateRootForTarget(target) ? join(inspectStateRootForTarget(target)!, 'team', snapshot.teamName, 'summary-snapshot.json') : null,
    ]),
  );
  ...
  return {
    ...
    sparkshell_hint: Object.keys(workerPanes).length > 0
      ? 'omb sparkshell --tmux-pane <pane-id> --tail-lines 400'
      : null,
