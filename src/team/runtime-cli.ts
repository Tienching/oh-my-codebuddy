 ...
 *
 * Spawned by OMB team orchestration entrypoints when a background team run starts.
 */
...
async function promptStaleCleanup(summary: StaleTeamSummary): Promise<boolean> {
  process.stderr.write(
    `\n[omb] Stale artifacts from previous team "${summary.teamName}":\n` +
...
type TeamWorkerProvider = 'codebuddy' | 'codex' | 'claude' | 'gemini';
...
export function buildTerminalCliResult(
  ...
  return {
    ...
    notice:
      ...
      + `Inspect with "omb team status ${teamName} --json" or "omb team api read-stall-state --input '{\"team_name\":\"${teamName}\"}' --json". `
      + `Run "omb team shutdown ${teamName}" (or --force after state capture) when explicit cleanup is desired.\n`,
  };
}
...
export function normalizeAgentTypes(raw: string[], workerCount: number): TeamWorkerProvider[] {
  ...
  const invalid = providers.filter((entry) => entry !== 'codebuddy' && entry !== 'codex' && entry !== 'claude' && entry !== 'gemini');
  if (invalid.length > 0) {
    throw new Error(`Invalid agentTypes entries: ${invalid.join(', ')}. Expected codebuddy|codex|claude|gemini.`);
  }
...
async function main(): Promise<void> {
  ...
  // Start the team — OMB's startTeam takes individual parameters
