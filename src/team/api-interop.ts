...
export type TeamApiEnvelope = TeamApiEnvelopeContract<TeamApiOperation>;
...
function buildTeamInteropError(error: { code: string; message: string }): TeamInteropErrorDetail {
  const normalizedCode = normalizeTeamInteropErrorCode(error.code);
  return {
    code: normalizedCode,
    message: error.message,
    ...(error.code !== normalizedCode ? { legacy_code: error.code } : {}),
  };
}

function finalizeTeamApiEnvelope(envelope: TeamLegacyApiEnvelope<TeamApiOperation>): TeamApiEnvelope {
  if (!isVersionedTeamInteropPayloadEnabled()) {
    return envelope as TeamApiEnvelope;
  }

  if (envelope.ok) {
    return {
      contract: TEAM_INTEROP_CONTRACT_NAME,
      version: TEAM_INTEROP_PAYLOAD_VERSION,
      compatibility_fallback: TEAM_INTEROP_COMPATIBILITY_FALLBACK,
      compatibility_fields: TEAM_INTEROP_COMPATIBILITY_FIELDS,
      status: 'ok',
      ok: true,
  ...
  const normalizedError = buildTeamInteropError(envelope.error);
  return {
    contract: TEAM_INTEROP_CONTRACT_NAME,
    version: TEAM_INTEROP_PAYLOAD_VERSION,
    compatibility_fallback: TEAM_INTEROP_COMPATIBILITY_FALLBACK,
...
function readLatestLeaderRuntimeActivityMs(cwd: string): number {
  ...
  for (const path of candidates) {
    ...
    try {
      ...
      if (Number.isFinite(ms)) latestMs = Number.isFinite(latestMs) ? Math.max(latestMs, ms) : ms;
    } catch {
      // ignore malformed candidate and continue to other compatible roots
    }
  }
  return latestMs;
}
...
function teamStateExists(teamName: string, candidateCwd: string): boolean {
  ...
  const teamRoot = join(candidateCwd, '.omb', 'state', 'team', teamName);
...
function resolveTeamWorkingDirectory(teamName: string, preferredCwd: string): string {
  ...
  const envTeamStateRoot = process.env.OMB_TEAM_STATE_ROOT;
...
function readLegacyMailboxMessages(
  ...
  const mailboxPath = join(resolveActiveTeamStateRoot(cwd), 'team', teamName, 'mailbox', `${workerName}.json`);
...
export function buildLegacyTeamDeprecationHint(legacyName: string, originalArgs?: Record<string, unknown>): string {
  ...
  if (!operation) {
    return `Use CLI interop: omb team api <operation> --input '${payload}' --json`;
  }
  return `Use CLI interop: omb team api ${operation} --input '${payload}' --json`;
}
...
export async function executeTeamApiOperation(
  ...
  try {
    ...
    return finalizeTeamApiEnvelope({
      ok: false,
      operation,
      error: {
