...
import { codebuddyPromptsDir, codexPromptsDir, packageRoot } from '../utils/paths.js';
...
function defaultPromptDirs(projectRoot: string): string[] {
  return [
    ...
    join(projectRoot, '.codebuddy', 'prompts'),
...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `omb team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `omb ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `omb team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',
