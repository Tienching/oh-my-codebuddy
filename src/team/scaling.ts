 ...
 * Gated behind the OMB_TEAM_SCALING_ENABLED environment variable.
 *
...
const OMB_TEAM_SCALING_ENABLED_ENV = 'OMB_TEAM_SCALING_ENABLED';
...
export function isScalingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[OMB_TEAM_SCALING_ENABLED_ENV];
  if (!raw) return false;
...
function assertScalingEnabled(env: NodeJS.ProcessEnv = process.env): void {
  if (!isScalingEnabled(env)) {
    throw new Error(
      `Dynamic scaling is disabled. Set ${OMB_TEAM_SCALING_ENABLED_ENV}=1 to enable.`,
    );
  }
}
...
export async function scaleUp(
  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
    ...
    const teamStateRoot = config.team_state_root ?? resolveActiveTeamStateRoot(leaderCwd);
    ...
    const rollbackScaleUp = async (
      ...
      for (const taskId of createdTaskIds) {
        await rm(join(leaderCwd, '.omb', 'state', 'team', sanitized, 'tasks', `task-${taskId}.json`), { force: true }).catch(() => {});
      }
    ...
    for (let i = 0; i < count; i++) {
      ...
      const workerDirPath = join(leaderCwd, '.omb', 'state', 'team', sanitized, 'workers', workerName);
      ...
      const rolePromptContent = rawRolePromptContent
        ...
        : null;
      const teamInstructionsPath = join(leaderCwd, '.omb', 'state', 'team', sanitized, 'worker-agents.md');
      ...
      const extraEnv: Record<string, string> = {
        ...
        OMB_TEAM_LEADER_CWD: leaderCwd,
        OMB_MODEL_INSTRUCTIONS_FILE: instructionsFilePath,
      };
      if (workerWorkspace) {
        extraEnv.OMB_TEAM_WORKTREE_PATH = workerWorkspace.worktreePath;
        if (workerWorkspace.branchName) {
          extraEnv.OMB_TEAM_WORKTREE_BRANCH = workerWorkspace.branchName;
        }
        extraEnv.OMB_TEAM_WORKTREE_DETACHED = workerWorkspace.detached ? '1' : '0';
      }
      ...
      const skipReadyWait = env.OMB_TEAM_SKIP_READY_WAIT === '1';
      if (!skipReadyWait) {
...
export async function scaleDown(
  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleDownResult | ScaleError> => {
    ...
    for (const w of targetWorkers) {
      if (w.worktree_path) {
        await removeWorkerWorktreeRootAgentsFile(sanitized, w.name, w.team_state_root ?? config.team_state_root ?? resolveActiveTeamStateRoot(leaderCwd), w.worktree_path).catch(() => {});
      }
