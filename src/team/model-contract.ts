...
export function collectInheritableTeamWorkerArgs(codebuddyArgs: string[]): string[] {
...
export function resolveAgentDefaultModel(
  ...
  if (normalized.endsWith('-low')) return resolveTeamLowComplexityDefaultModel(codebuddyHomeOverride);
  if (normalized === 'executor') return getMainDefaultModel(codebuddyHomeOverride);

  switch (getAgent(normalized)?.modelClass) {
    case 'fast':
      return resolveTeamLowComplexityDefaultModel(codebuddyHomeOverride);
    case 'frontier':
      return getMainDefaultModel(codebuddyHomeOverride);
    case 'standard':
      return getStandardDefaultModel(codebuddyHomeOverride);
    default:
      return undefined;
  }
}
...
export function resolveTeamLowComplexityDefaultModel(codebuddyHomeOverride?: string): string {
  return getSparkDefaultModel(codebuddyHomeOverride);
}
