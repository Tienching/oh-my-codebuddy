...
function readBaselineFile(path: string): CurrentTaskBaselineFile {
  try {
...
export function readCurrentTaskBaseline(repoRoot: string): CurrentTaskBaselineFile {
  const { canonical, legacy } = baselinePaths(repoRoot);
  if (existsSync(canonical)) return readBaselineFile(canonical);
  if (existsSync(legacy)) return readBaselineFile(legacy);
  return emptyBaseline();
}
