/**
 * Native agent config generators for CodeBuddy CLI.
 * Writes standalone TOML files under ~/.codebuddy/agents/ or ./.codebuddy/agents/.
 */
...
import { codebuddyAgentsDir } from "../utils/paths.js";
...
function readConfigContent(
  ...
  const legacySettingsPath = join(homeDir, "settings.json");
  if (!existsSync(legacySettingsPath)) {
    return "";
  }

  try {
    const parsed = JSON.parse(readFileSync(legacySettingsPath, "utf-8")) as {
      model?: unknown;
    };
    return typeof parsed.model === "string" && parsed.model.trim() !== ""
      ? `model = "${parsed.model.trim().replace(/\\/g, "\\\\").replace(/"/g, '\\"')}"\n`
      : "";
  } catch {
    return "";
  }
...
function resolveFrontierModel(options: AgentModelResolutionOptions): string {
  const configContent = readConfigContent(
    options.codebuddyHomeOverride,
    options.configTomlContent,
  );
  return getRootModelName(configContent)
    ?? getMainDefaultModel(options.codebuddyHomeOverride);
}
...
function resolveStandardModel(options: AgentModelResolutionOptions): string {
  ...
  return getStandardDefaultModel(options.codebuddyHomeOverride);
}
...
function resolveAgentModel(
  ...
  switch (agent.modelClass) {
    case "frontier":
      return resolveFrontierModel(options);
    case "fast":
      return getSparkDefaultModel(options.codebuddyHomeOverride);
    case "standard":
    default:
...
export function composeRoleInstructions(
  ...
  if (resolvedModel) {
    if (metadataLines.length === 0) {
      metadataLines.push("## OMB Agent Metadata");
    }
...
export function generateStandaloneAgentToml(
  ...
  const lines = [
    `# oh-my-codebuddy agent: ${config.name}`,
...
/**
 * Generate TOML content for a prompt-backed OMB role agent.
 */
...
/**
 * Install prompt-backed native agent config .toml files to ~/.codebuddy/agents/
...
export async function installNativeAgentConfigs(
  ...
  const {
    ...
    dryRun = false,
    verbose = false,
    agentsDir = codebuddyAgentsDir(),
  } = options;
  const codebuddyHomeOverride = join(agentsDir, "..");
  ...
  for (const [name, agent] of Object.entries(AGENT_DEFINITIONS)) {
    ...
    const toml = generateAgentToml(agent, promptContent, { codebuddyHomeOverride });
