...
const originalStandardModel = process.env.OMB_DEFAULT_STANDARD_MODEL;

beforeEach(() => {
  process.env.OMB_DEFAULT_STANDARD_MODEL = "gpt-5.4-mini";
});

afterEach(() => {
  if (typeof originalStandardModel === "string") {
    process.env.OMB_DEFAULT_STANDARD_MODEL = originalStandardModel;
  } else {
...
describe("agents/native-config", () => {
  it("generates TOML with stripped frontmatter and escaped triple quotes", () => {
    ...
    assert.match(toml, /# oh-my-codebuddy agent: executor/);
  ...
  it("applies exact-model mini guidance only for resolved gpt-5.4-mini standard roles", () => {
    ...
    const exactMiniToml = generateAgentToml(agent, prompt, {
      env: { OMB_DEFAULT_STANDARD_MODEL: "gpt-5.4-mini" } as NodeJS.ProcessEnv,
    });
    const frontierToml = generateAgentToml(agent, prompt, {
      env: { OMB_DEFAULT_STANDARD_MODEL: "gpt-5.4" } as NodeJS.ProcessEnv,
    });
    const tunedToml = generateAgentToml(agent, prompt, {
      env: { OMB_DEFAULT_STANDARD_MODEL: "gpt-5.4-mini-tuned" } as NodeJS.ProcessEnv,
    });
  ...
  it("keeps standard agents off a custom gpt-5.2 root model", async () => {
    ...
    try {
      ...
      await writeFile(join(codebuddyHome, "settings.json"), JSON.stringify({ model: "gpt-5.2" }));
      ...
      if (typeof previousCodexHome === "string") process.env.CODEBUDDY_HOME = previousCodexHome;
