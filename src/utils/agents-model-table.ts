...
export const OMB_MODELS_START_MARKER = '<!-- OMB:MODELS:START -->';
export const OMB_MODELS_END_MARKER = '<!-- OMB:MODELS:END -->';
...
function readFrontierModelFromSettingsLike(content: string): string | undefined {
...
export function resolveAgentsModelTableContext(
  ...
  const { codebuddyHomeOverride, env = process.env } = options;
  const frontierModel =
    readFrontierModelFromSettingsLike(configTomlContent) ??
    getRootModelName(configTomlContent) ??
    getEnvConfiguredMainDefaultModel(env, codebuddyHomeOverride) ??
    DEFAULT_FRONTIER_MODEL;
  const sparkModel =
    getEnvConfiguredSparkDefaultModel(env, codebuddyHomeOverride) ??
    getSparkDefaultModel(codebuddyHomeOverride) ??
    DEFAULT_SPARK_MODEL;
  const subagentDefaultModel =
    getEnvConfiguredStandardDefaultModel(env, codebuddyHomeOverride) ??
    getStandardDefaultModel(codebuddyHomeOverride) ??
    DEFAULT_STANDARD_MODEL;
...
export function buildAgentsModelTable(
  ...
  return [
    ...
    '',
    'Auto-generated by `omb setup` from the current `settings.json` plus OMB model overrides.',
    '',
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(OMB_MODELS_START_MARKER);
  const endIndex = content.indexOf(OMB_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + OMB_MODELS_END_MARKER.length;
