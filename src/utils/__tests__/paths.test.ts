...
describe("codebuddyHome", () => {
  ...
  it("returns CODEBUDDY_HOME env var when set", () => {
    process.env.CODEBUDDY_HOME = "/tmp/custom-codex";
    assert.equal(codebuddyHome(), "/tmp/custom-codex");
  });

  it("defaults to ~/.codebuddy when CODEBUDDY_HOME is not set", () => {
    delete process.env.CODEBUDDY_HOME;
    assert.equal(codebuddyHome(), join(homedir(), ".codebuddy"));
  });
});

describe("codebuddyConfigPath", () => {
  let originalCodexHome: string | undefined;
  let originalUserProfile: string | undefined;
  ...
  beforeEach(() => {
    originalCodexHome = process.env.CODEBUDDY_HOME;
    originalUserProfile = process.env.USERPROFILE;
    process.env.CODEBUDDY_HOME = "/tmp/test-codebuddy";
  });
  ...
  it("returns config.toml under codebuddy home", () => {
    assert.equal(codebuddyConfigPath(), join("/tmp/test-codebuddy", "config.toml"));
  });
});

describe("codebuddyPromptsDir", () => {
  let originalCodexHome: string | undefined;
  let originalUserProfile: string | undefined;
  ...
  it("returns prompts/ under codebuddy home", () => {
    assert.equal(codebuddyPromptsDir(), join("/tmp/test-codebuddy", "prompts"));
  });
});
...
describe("userSkillsDir", () => {
  ...
  it("returns CODEBUDDY_HOME/skills", () => {
    assert.equal(userSkillsDir(), join("/tmp/test-codebuddy", "skills"));
  });
});

describe("projectSkillsDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(projectSkillsDir("/my/project"), join("/my/project", ".codebuddy", "skills"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(projectSkillsDir(), join(process.cwd(), ".codebuddy", "skills"));
  });
});
...
describe("listInstalledSkillDirectories", () => {
  ...
  it("deduplicates by skill name and prefers project skills over user skills", async () => {
    ...
    try {
      const projectHelpDir = join(projectRoot, ".codebuddy", "skills", "help");
      ...
      const userHelpDir = join(codebuddyHomeRoot, "skills", "help");
      const userOnlyDir = join(codebuddyHomeRoot, "skills", "user-only");
      ...
      await rm(codebuddyHomeRoot, { recursive: true, force: true });
    }
  });
  it("detects overlapping legacy and canonical user skill roots including content mismatches", async () => {
    ...
    try {
      const canonicalHelpDir = join(codebuddyHomeRoot, "skills", "help");
      const canonicalPlanDir = join(codebuddyHomeRoot, "skills", "plan");
...
describe("ombStateDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(ombStateDir("/my/project"), join("/my/project", ".omb", "state"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(ombStateDir(), join(process.cwd(), ".omb", "state"));
  });
});

describe("ombProjectMemoryPath", () => {
  it("uses provided projectRoot", () => {
    assert.equal(
      ombProjectMemoryPath("/my/project"),
      join("/my/project", ".omb", "project-memory.json"),
    );
  });
  ...
  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(
      ombProjectMemoryPath(),
      join(process.cwd(), ".omb", "project-memory.json"),
    );
  });
});

describe("ombNotepadPath", () => {
  it("uses provided projectRoot", () => {
    assert.equal(ombNotepadPath("/my/project"), join("/my/project", ".omb", "notepad.md"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(ombNotepadPath(), join(process.cwd(), ".omb", "notepad.md"));
  });
});

describe("ombPlansDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(ombPlansDir("/my/project"), join("/my/project", ".omb", "plans"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(ombPlansDir(), join(process.cwd(), ".omb", "plans"));
  });
});

describe("ombLogsDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(ombLogsDir("/my/project"), join("/my/project", ".omb", "logs"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(ombLogsDir(), join(process.cwd(), ".omb", "logs"));
  });
});
