...
import {
  ...
  OMB_MODELS_END_MARKER,
  OMB_MODELS_START_MARKER,
  resolveAgentsModelTableContext,
  upsertAgentsModelTable,
} from '../agents-model-table.js';

const originalFrontierEnv = process.env.OMB_DEFAULT_FRONTIER_MODEL;
const originalStandardEnv = process.env.OMB_DEFAULT_STANDARD_MODEL;
const originalSparkEnv = process.env.OMB_DEFAULT_SPARK_MODEL;
const originalLegacySparkEnv = process.env.OMB_SPARK_MODEL;
...
afterEach(() => {
  if (typeof originalFrontierEnv === 'string') {
    process.env.OMB_DEFAULT_FRONTIER_MODEL = originalFrontierEnv;
  } else {
    delete process.env.OMB_DEFAULT_FRONTIER_MODEL;
  }
  if (typeof originalStandardEnv === 'string') {
    process.env.OMB_DEFAULT_STANDARD_MODEL = originalStandardEnv;
  } else {
    delete process.env.OMB_DEFAULT_STANDARD_MODEL;
  }
  if (typeof originalSparkEnv === 'string') {
    process.env.OMB_DEFAULT_SPARK_MODEL = originalSparkEnv;
  } else {
    delete process.env.OMB_DEFAULT_SPARK_MODEL;
  }
  if (typeof originalLegacySparkEnv === 'string') {
    process.env.OMB_SPARK_MODEL = originalLegacySparkEnv;
  } else {
    delete process.env.OMB_SPARK_MODEL;
  }
});
...
describe('agents model table', () => {
  it('resolves frontier from settings.json, standard from environment, and spark from environment', () => {
    process.env.OMB_DEFAULT_FRONTIER_MODEL = 'frontier-env';
    process.env.OMB_DEFAULT_STANDARD_MODEL = 'standard-env';
    process.env.OMB_DEFAULT_SPARK_MODEL = 'spark-env';

    const context = resolveAgentsModelTableContext(JSON.stringify({ model: 'frontier-config' }));
  ...
  it('replaces existing marker-bounded content and inserts the block after team_model_resolution when missing', () => {
    ...
    assert.match(
      inserted,
      /<\/team_model_resolution>\n\n<!-- OMB:MODELS:START -->[\s\S]*<!-- OMB:MODELS:END -->\n\n---/,
    );
  });
});
