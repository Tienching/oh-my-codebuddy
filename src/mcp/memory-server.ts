/**
 * OMB Project Memory & Notepad MCP Server
 * Provides persistent project memory and session notepad tools
 * Storage: .omb/project-memory.json, .omb/notepad.md
 */
...
function getMemoryPath(wd: string): string {
  return join(wd, '.omb', 'project-memory.json');
}

function getNotepadPath(wd: string): string {
  return join(wd, '.omb', 'notepad.md');
}
