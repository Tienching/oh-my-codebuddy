...
const OMB_JOBS_DIR = join(homedir(), '.omb', 'team-jobs');
...
async function writeJobFiles(
  ...
  await mkdir(OMB_JOBS_DIR, { recursive: true });
  await writeFile(join(OMB_JOBS_DIR, `${jobId}.json`), JSON.stringify(job));
  await writeFile(join(OMB_JOBS_DIR, `${jobId}-panes.json`), JSON.stringify(panes));
}

async function cleanupJobFiles(jobId: string): Promise<void> {
  await rm(join(OMB_JOBS_DIR, `${jobId}.json`), { force: true });
  await rm(join(OMB_JOBS_DIR, `${jobId}-panes.json`), { force: true });
}

async function loadTeamServer() {
  process.env.OMB_TEAM_SERVER_DISABLE_AUTO_START = '1';
...
describe('team-server cleanup hardening', () => {
  it('intersects live-session candidates with team config + panes file identities before kill', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-identity-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-bin-'));
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omb-team-cleanup-team';
  ...
  it('does not broad-sweep session panes during cleanup target selection', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-sweep-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-sweep-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    const jobId = `omb-${Date.now().toString(36)}`;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omb-team-cleanup-sweep';
      config.workers[0]!.pane_id = '%2';
      await saveTeamConfig(config, cwd);
  ...
  it('returns unchanged legacy content[0].text plus additive structured JSON in content[1].text', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-legacy-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omb-team-cleanup-legacy-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    const jobId = `omb-${Date.now().toString(36)}`;
  ...
  it('returns deterministic noop summary when no killable panes remain', async () => {
    ...
    try {
      await mkdir(OMB_JOBS_DIR, { recursive: true });
      await writeFile(join(OMB_JOBS_DIR, `${jobId}.json`), JSON.stringify({
        status: 'running',
        startedAt: Date.now(),
      }));
      await writeFile(join(OMB_JOBS_DIR, `${jobId}-panes.json`), JSON.stringify({
        paneIds: [],
        leaderPaneId: '',
      }));
