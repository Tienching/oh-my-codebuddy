...
describe('state paths', () => {
  ...
  it('enforces OMB_MCP_WORKDIR_ROOTS allowlist when configured', async () => {
    const allowedRoot = await mkdtemp(join(tmpdir(), 'omb-allowed-root-'));
    const disallowedRoot = await mkdtemp(join(tmpdir(), 'omb-disallowed-root-'));
    const prev = process.env.OMB_MCP_WORKDIR_ROOTS;
    process.env.OMB_MCP_WORKDIR_ROOTS = allowedRoot;
    try {
      assert.equal(
      ...
      assert.throws(
        ...
        /outside allowed roots \(OMB_MCP_WORKDIR_ROOTS\)/,
      );
    } finally {
      if (typeof prev === 'string') process.env.OMB_MCP_WORKDIR_ROOTS = prev;
      else delete process.env.OMB_MCP_WORKDIR_ROOTS;
  ...
  it('builds global state paths', () => {
    ...
    assert.equal(base, '/repo/.omb/state');
    assert.equal(getStateDir('/repo'), '/repo/.omb/state');
    assert.equal(getStatePath('team', '/repo'), '/repo/.omb/state/team-state.json');
  });

  it('builds session state paths', () => {
    assert.equal(getStateDir('/repo', 'sess1'), '/repo/.omb/state/sessions/sess1');
    assert.equal(
      getStatePath('ralph', '/repo', 'sess1'),
      '/repo/.omb/state/sessions/sess1/ralph-state.json'
    );
  });
  ...
  it('enumerates global-only path', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-state-paths-'));
    try {
