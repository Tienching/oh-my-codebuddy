...
const SERVER_ENTRYPOINTS: Array<{ server: McpServerName; file: string }> = [
  ...
  { server: 'lsp', file: 'src/mcp/lsp-server.ts' },
  { server: 'shared_memory', file: 'src/mcp/shared-memory-server.ts' },
  { server: 'wiki', file: 'src/mcp/wiki-server.ts' },
];

describe('mcp bootstrap auto-start guard', () => {
  it('allows auto-start by default for every OMB MCP server', () => {
  ...
  it('disables all servers when global disable flag is set', () => {
    const env = { OMB_MCP_SERVER_DISABLE_AUTO_START: '1' };
...
describe('mcp duplicate sibling detection', () => {
  it('extracts same-entrypoint markers from command lines', () => {
    assert.equal(
      extractMcpEntrypointMarker('node /tmp/oh-my-codebuddy/dist/mcp/state-server.js'),
      'state-server.js',
    );
    assert.equal(
      extractMcpEntrypointMarker('node C:\\\\tmp\\\\oh-my-codebuddy\\\\dist\\\\mcp\\\\trace-server.ts'),
      'trace-server.ts',
    );
