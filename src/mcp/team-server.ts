...
const jobIdSchema = z.string().regex(/^omb-[a-z0-9]{1,12}$/);
...
async function resolveCleanupTargets(jobId: string, job: OmbTeamJob): Promise<CleanupSelection> {
...
const server = new Server(
  { name: 'omb-team-server', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
  ...
  try {
    switch (name) {
      case 'omb_run_team_start': {
        ...
        const job: OmbTeamJob = { status: 'running', startedAt: Date.now(), teamName, cwd: inputCwd };
