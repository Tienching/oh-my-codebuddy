...
export interface DiscordBotNotificationConfig {
  enabled: boolean;
  /** Discord bot token (or env var: OMB_DISCORD_NOTIFIER_BOT_TOKEN) */
  botToken?: string;
  /** Channel ID to send messages to (or env var: OMB_DISCORD_NOTIFIER_CHANNEL) */
  channelId?: string;
  /** Optional mention to prepend to messages (e.g. "<@123456>" for user, "<@&789>" for role) */
  mention?: string;
}
...
/** Top-level notification configuration (stored in .omb-config.json) */
...
export interface FullNotificationPayload {
  ...
  projectName?: string;
  /** Active OMB modes during this session */
  modesUsed?: string[];
  ...
  tmuxTail?: string;
  /** Agent name (populated by extensibility plugins, not set by core CodeBuddy CLI hooks) */
  agentName?: string;
  /** Agent type (populated by extensibility plugins, not set by core CodeBuddy CLI hooks) */
  agentType?: string;
}
