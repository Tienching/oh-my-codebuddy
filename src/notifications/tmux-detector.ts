 ...
 * CodeBuddy CLI, and inject text into panes. Used by the reply-listener daemon.
 */
...
export function analyzePaneContent(content: string): PaneAnalysis {
  ...
  const hasCodex =
    ...
    lower.includes('codebuddy') ||
    lower.includes('omx') ||
    lower.includes('omb') ||
    lower.includes('oh-my-codex') ||
    lower.includes('oh-my-codebuddy') ||
...
export function buildSendPaneArgvs(
  ...
  if (pressEnter) {
    // CodeBuddy CLI uses raw input mode; send C-m (carriage return) twice
...
export function getSubmitPresses(workerCli: string): number {
  if (process.env.OMB_RUNTIME_BRIDGE === '0') {
