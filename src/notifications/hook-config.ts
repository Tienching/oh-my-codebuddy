 ...
 *
 * Reads hookTemplates from .omb-config.json for user-customizable message templates.
 * Config is stored under the notifications.hookTemplates key in codebuddyHome()/.omb-config.json.
 * Env var OMB_HOOK_CONFIG overrides to a separate file path.
 */
 ...
 *
 * Primary source: notifications.hookTemplates key in codebuddyHome()/.omb-config.json
 * Env var override: OMB_HOOK_CONFIG points to a separate file containing the
...
export function getHookConfig(): HookNotificationConfig | null {
  ...
  const envOverridePath = process.env.OMB_HOOK_CONFIG;
  ...
  // Primary: read from notifications.hookTemplates in .omb-config.json
  const OMB_CONFIG_PATH = join(codebuddyHome(), ".omb-config.json");
  if (!existsSync(OMB_CONFIG_PATH)) {
    cachedConfig = null;
    return null;
  }

  try {
    const raw = JSON.parse(readFileSync(OMB_CONFIG_PATH, "utf-8"));
 ...
 * - hook event `enabled: false` overrides .omb-config.json event `enabled: true`
 * - Platform credentials are NOT affected (they stay in .omb-config.json)
 */
