...
const DEFAULT_STATE_DIR = join(homedir(), '.omb', 'state');
...
const REPLY_ACK_PREFIX = 'Injected into CodeBuddy CLI session.';
...
function injectReply(
  ...
  if (analysis.confidence < 0.4) {
    log(`WARN: Pane ${paneId} does not appear to be running CodeBuddy CLI (confidence: ${analysis.confidence}). Skipping injection, removing stale mapping.`);
