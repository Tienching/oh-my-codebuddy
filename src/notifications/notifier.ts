/**
 * Notification system for oh-my-codebuddy
...
/**
 * Load notification config from .omb/notifications.json
 */
export async function loadNotificationConfig(projectRoot?: string): Promise<NotificationConfig | null> {
  const configPath = join(projectRoot || process.cwd(), '.omb', 'notifications.json');
...
export function _buildDesktopArgs(
  ...
  if (platform === 'darwin') {
    ...
    const ps =
      ...
      `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('oh-my-codebuddy').Show($xml)`;
...
async function sendDiscordNotification(payload: NotificationPayload, webhookUrl: string): Promise<void> {
  ...
  const body = JSON.stringify({
    embeds: [{
      title: `[OMB] ${payload.title}`,
      description: payload.message,
      color: colorMap[payload.type],
      footer: { text: `oh-my-codebuddy | ${payload.mode || 'general'}` },
