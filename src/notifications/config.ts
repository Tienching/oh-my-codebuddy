 ...
 *
 * Reads notification config from .omb-config.json and provides
...
function readRawConfig(): Record<string, unknown> | null {
  const configFile = resolveNotificationConfigPath();
...
export function buildConfigFromEnv(): FullNotificationConfig | null {
  ...
  const discordMention = validateMention(process.env.OMB_DISCORD_MENTION);

  const discordBotToken = process.env.OMB_DISCORD_NOTIFIER_BOT_TOKEN;
  const discordChannel = process.env.OMB_DISCORD_NOTIFIER_CHANNEL;
  ...
  const discordWebhook = process.env.OMB_DISCORD_WEBHOOK_URL;
  if (discordWebhook) {
    config.discord = {
      enabled: true,
  ...
  const telegramToken =
    process.env.OMB_TELEGRAM_BOT_TOKEN ||
    process.env.OMB_TELEGRAM_NOTIFIER_BOT_TOKEN;
  const telegramChatId =
    process.env.OMB_TELEGRAM_CHAT_ID ||
    process.env.OMB_TELEGRAM_NOTIFIER_CHAT_ID ||
    process.env.OMB_TELEGRAM_NOTIFIER_UID;
  ...
  const slackWebhook = process.env.OMB_SLACK_WEBHOOK_URL;
  if (slackWebhook) {
    const slackMention = validateSlackMention(process.env.OMB_SLACK_MENTION);
    config.slack = {
      enabled: true,
 ...
 *   2. OMB_NOTIFY_PROFILE environment variable
...
export function resolveProfileConfig(
  ...
  const name =
    profileName ||
    process.env.OMB_NOTIFY_PROFILE ||
...
export function getActiveProfileName(): string | null {
  if (process.env.OMB_NOTIFY_PROFILE) {
    return process.env.OMB_NOTIFY_PROFILE;
  }
  const raw = readRawConfig();
  if (!raw) return null;
  const notifications = raw.notifications as NotificationsBlock | undefined;
...
export function getNotificationConfig(
  ...
  if (raw) {
    ...
    if (notifications) {
      ...
      const envMention = validateMention(process.env.OMB_DISCORD_MENTION);
      if (envMention) {
...
export function getVerbosity(config: FullNotificationConfig | null): VerbosityLevel {
  const envVal = process.env.OMB_NOTIFY_VERBOSITY as string | undefined;
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
  ...
  const enabled = process.env.OMB_REPLY_ENABLED === "true" || replyRaw?.enabled === true;
  if (!enabled) return null;

  const authorizedDiscordUserIds = parseDiscordUserIds(
    process.env.OMB_REPLY_DISCORD_USER_IDS,
  ...
  if (hasDiscordBot && authorizedDiscordUserIds.length === 0) {
    console.warn(
      ...
      "Set OMB_REPLY_DISCORD_USER_IDS or add to .omb-config.json notifications.reply.authorizedDiscordUserIds"
    );
  }

  const pollIntervalMs = normalizeInteger(
    parseIntegerInput(process.env.OMB_REPLY_POLL_INTERVAL_MS)
  ...
  const rateLimitPerMinute = normalizeInteger(
    parseIntegerInput(process.env.OMB_REPLY_RATE_LIMIT)
  ...
  return {
    ...
    maxMessageLength,
    rateLimitPerMinute,
    includePrefix: process.env.OMB_REPLY_INCLUDE_PREFIX !== "false" && (replyRaw?.includePrefix !== false),
