...
describe('analyzePaneContent', () => {
  ...
  it('detects "oh-my-codebuddy" keyword', () => {
    const result = analyzePaneContent('oh-my-codebuddy v1.0');
    assert.equal(result.hasCodex, true);
  });
  ...
  it('gives high confidence for codex content with prompt chars', () => {
    const result = analyzePaneContent('CodeBuddy > Running agent task...');
    assert.equal(result.hasCodex, true);
  ...
  it('caps confidence at 1.0', () => {
    const result = analyzePaneContent('CodeBuddy $ > agent task running omb');
