...
async function writeCodexConfig(contents: unknown): Promise<void> {
  ...
  await writeFile(join(tempCodexHome, '.omb-config.json'), JSON.stringify(contents, null, 2));
}
...
describe('notification temp mode', () => {
  beforeEach(async () => {
    ...
    tempCodexHome = await mkdtemp(join(tmpdir(), 'omb-notify-temp-'));
    process.env.CODEBUDDY_HOME = tempCodexHome;
  });
  ...
  it('temp contract bypasses persistent file/profile routing', async () => {
    ...
    process.env.OMB_NOTIFY_PROFILE = 'file-profile';
    process.env.OMB_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/temp-only';
    process.env.OMB_NOTIFY_TEMP_CONTRACT = JSON.stringify({
      active: true,
      selectors: ['slack'],
  ...
  it('shouldDispatchOpenClaw enforces temp-mode explicit selection and gateway matching', async () => {
    ...
    assert.equal(
      await shouldDispatchOpenClaw('session-end', activeWithOpenClaw, { OMB_OPENCLAW: '0', CODEBUDDY_HOME: tempCodexHome }),
      false,
    );
  });
});
