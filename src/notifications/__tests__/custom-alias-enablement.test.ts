...
const ENV_KEYS = ['CODEX_HOME', 'OMB_OPENCLAW'] as const;
...
describe('notification custom alias enablement defaults', () => {
  before(async () => {
    clearEnv();
    tempCodexHome = await mkdtemp(join(tmpdir(), 'omb-custom-alias-'));
  ...
  afterEach(async () => {
    resetOpenClawConfigCache();
    await rm(join(tempCodexHome, '.omb-config.json'), { force: true });
  });
