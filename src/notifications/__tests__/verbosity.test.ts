...
describe('getVerbosity', () => {
  const origEnv = process.env.OMB_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.OMB_NOTIFY_VERBOSITY;
    } else {
      process.env.OMB_NOTIFY_VERBOSITY = origEnv;
    }
  });
