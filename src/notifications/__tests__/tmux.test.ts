...
describe('getCurrentTmuxSession', () => {
  ...
  const originalPidFallback = process.env.OMB_TMUX_PID_FALLBACK;
  ...
  afterEach(() => {
    ...
    if (originalPidFallback !== undefined) {
      process.env.OMB_TMUX_PID_FALLBACK = originalPidFallback;
    } else {
      delete process.env.OMB_TMUX_PID_FALLBACK;
    }
  });
  ...
  it('skips ps-based pid fallback on native Windows', () => {
    ...
    delete process.env.TMUX;
    process.env.OMB_TMUX_PID_FALLBACK = '1';
    try {
...
describe('getTeamTmuxSessions - session matching', () => {
  ...
  function makeFakeTmux(sessions: string[]): void {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omb-tmux-team-test-'));
  ...
  it('returns canonical session name (omb-team-alpha) for team "alpha"', () => {
    makeFakeTmux(['omb-team-alpha', 'other-session']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omb-team-alpha']);
  });

  it('returns prefixed worker sessions (omb-team-alpha-worker1)', () => {
    makeFakeTmux(['omb-team-alpha-worker1', 'omb-team-alpha-worker2']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omb-team-alpha-worker1', 'omb-team-alpha-worker2']);
  });

  it('does NOT return sessions for a different team', () => {
    makeFakeTmux(['omb-team-beta', 'omb-team-beta-worker1']);
...
describe('captureTmuxPane', () => {
  ...
  it('captures output for valid pane id and sanitizes/clamps lines', () => {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omb-tmux-test-'));
    tmpDirs.push(fakeBinDir);
    const tmuxPath = join(fakeBinDir, 'tmux');
    writeFileSync(
      tmuxPath,
