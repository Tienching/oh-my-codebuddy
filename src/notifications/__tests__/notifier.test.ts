...
describe('loadNotificationConfig', () => {
  it('returns null when config file does not exist', async () => {
    const fakePath = join(tmpdir(), `omb-test-${randomUUID()}`);
    const config = await loadNotificationConfig(fakePath);
    assert.equal(config, null);
  });

  it('returns parsed config when file exists', async () => {
    const tmpDir = join(tmpdir(), `omb-test-${randomUUID()}`);
    const ombDir = join(tmpDir, '.omb');
    mkdirSync(ombDir, { recursive: true });
    ...
    writeFileSync(join(ombDir, 'notifications.json'), JSON.stringify(configData));
  ...
  it('returns null for invalid JSON', async () => {
    ...
    const ombDir = join(tmpDir, '.omb');
    mkdirSync(ombDir, { recursive: true });
    writeFileSync(join(ombDir, 'notifications.json'), 'not-json');
