...
const ENV_KEYS = [
  ...
  'OMB_DISCORD_WEBHOOK_URL',
  'OMB_DISCORD_MENTION',
  'OMB_TELEGRAM_BOT_TOKEN',
  'OMB_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'OMB_TELEGRAM_CHAT_ID',
  'OMB_TELEGRAM_NOTIFIER_CHAT_ID',
  'OMB_TELEGRAM_NOTIFIER_UID',
  'OMB_SLACK_WEBHOOK_URL',
  'OMB_REPLY_ENABLED',
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.OMB_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMB_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('includes validated mention in discord-bot config', () => {
    process.env.OMB_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMB_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMB_DISCORD_MENTION = '<@12345678901234567>';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('rejects invalid mention in env var', () => {
    process.env.OMB_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMB_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMB_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.OMB_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds telegram config from env vars', () => {
    process.env.OMB_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMB_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.OMB_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses OMB_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.OMB_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.OMB_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses OMB_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.OMB_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMB_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    ...
    process.env.OMB_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.OMB_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.OMB_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.OMB_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
