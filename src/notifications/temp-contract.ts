export const OMB_NOTIFY_TEMP_ENV = 'OMB_NOTIFY_TEMP';
export const OMB_NOTIFY_TEMP_CONTRACT_ENV = 'OMB_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[OMB_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[OMB_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[OMB_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {
