...
/** OMB UI chrome: spinner/progress indicator characters */
...
/** OMB HUD status lines: [OMB#...] or [OMB] (unversioned) */
const OMB_HUD_RE = /\[OMB[#\]]/;
...
export function parseTmuxTail(raw: string): string {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (OMB_HUD_RE.test(trimmed)) continue;
...
export function formatSessionIdle(payload: FullNotificationPayload): string {
  ...
  lines.push(`CodeBuddy has finished and is waiting for input.`);
  lines.push("");
...
export function formatAskUserQuestion(payload: FullNotificationPayload): string {
  ...
  lines.push(`CodeBuddy is waiting for your response.`);
  lines.push("");
  lines.push(buildFooter(payload, true));
