...
/**
 * OMB model type system
 */
export type OmbModelType = 'spark' | 'standard' | 'frontier' | 'inherit';

/**
 * Reasoning effort levels matching CodeBuddy CLI --reasoning-effort flag
 */
 ...
 *
 * Resolved at call time so environment/config changes are respected.
 */
export function getTierModels(): Record<ComplexityTier, string> {
  return {
    LOW: getSparkDefaultModel(),
    MEDIUM: getStandardDefaultModel(),
    HIGH: getMainDefaultModel(),
  };
}

/**
 * Model tier to OMB model type mapping
 */
export const TIER_TO_MODEL_TYPE: Record<ComplexityTier, OmbModelType> = {
  LOW: 'spark',
  MEDIUM: 'standard',
  HIGH: 'frontier',
};
...
/**
 * Model tier to default reasoning effort
 */
export const TIER_TO_REASONING: Record<ComplexityTier, ReasoningEffort> = {
  LOW: 'low',
  MEDIUM: 'medium',
  HIGH: 'high',
};
...
export interface RoutingConfig {
  ...
  /** Keywords that suggest higher tier */
