 ...
 * Wiki Storage
 *
 * File I/O layer for the OMB wiki knowledge base.
 */
...
export function getWikiDir(root: string): string {
  return getManagedRoot(root);
}

export function ensureWikiDir(root: string): string {
  const wikiDir = getWikiDir(root);
  mkdirSync(wikiDir, { recursive: true });
  const managedRoot = getManagedStateRoot(root);
  mkdirSync(managedRoot, { recursive: true });
  const gitignorePath = join(managedRoot, '.gitignore');
