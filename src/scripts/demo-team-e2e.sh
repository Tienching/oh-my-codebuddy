...
#   OMB_TEAM_WORKER_CLI             Worker CLI mode (default: auto)
#   OMB_TEAM_WORKER_CLI_MAP         Comma-separated CLI assignments per worker
#   OMB_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to worker CLIs
#
# Example:
...
if [[ -n "${OMB_TEAM_WORKER_CLI_MAP:-}" ]]; then
  IFS=',' read -ra CLI_MAP_ENTRIES <<< "$OMB_TEAM_WORKER_CLI_MAP"
  if (("${#CLI_MAP_ENTRIES[@]}" != WORKER_COUNT)); then
    echo "error: OMB_TEAM_WORKER_CLI_MAP has ${#CLI_MAP_ENTRIES[@]} entries but WORKER_COUNT is $WORKER_COUNT" >&2
    exit 1
  fi
fi
...
OMB_TEAM_WORKER_CLI_MAP="${OMB_TEAM_WORKER_CLI_MAP:-$(build_default_cli_map "$WORKER_COUNT")}"
OMB_TEAM_WORKER_LAUNCH_ARGS="${OMB_TEAM_WORKER_LAUNCH_ARGS:--c model_reasoning_effort=\"low\"}"
...
echo "OMB_TEAM_WORKER_LAUNCH_ARGS=$OMB_TEAM_WORKER_LAUNCH_ARGS"
...
omb team api mailbox-mark-notified --input "$MARK_INPUT" --json >/dev/null
omb team api mailbox-mark-delivered --input "$MARK_INPUT" --json >/dev/null
