...
import { resolveBridgeStateDir } from '../runtime/bridge.js';
import {
...
const idleMaxPollMs = Math.max(
  pollMs,
  asNumber(argValue('--idle-max-poll-ms', process.env.OMB_NOTIFY_FALLBACK_IDLE_MAX_POLL_MS || '1000'), 1000),
);
...
const maxLifetimeMs = runOnce
  ...
  : Math.max(
    ...
    asNumber(
      argValue(
        '--max-lifetime-ms',
        envValue('OMB_NOTIFY_FALLBACK_MAX_LIFETIME_MS') || String(defaultMaxLifetimeMs)
      ),
...
const teamStateDir = resolveBridgeStateDir(cwd);
...
const maxLogBytes = Math.max(
  0,
  asNumber(argValue('--log-max-bytes', process.env.OMB_NOTIFY_FALLBACK_LOG_MAX_BYTES || String(defaultMaxLogBytes)), defaultMaxLogBytes),
);
...
const FALLBACK_WATCHER_INJECT_MARKER = '[OMB_TMUX_INJECT]';
...
let lastDispatchDrain: DispatchDrainState = {
  leader_only: safeString(envValue('OMB_TEAM_WORKER') || '').trim() === '',
  last_tick_at: null,
  last_result: null,
  last_error: null,
};
...
const AUTO_NUDGE_STALL_MS = Math.max(
  pollMs,
  asNumber(envValue('OMB_NOTIFY_FALLBACK_AUTO_NUDGE_STALL_MS') || '5000', 5000),
);
...
async function emitRalphContinueSteer(paneId: string, message: string): Promise<void> {
  const markedText = `${message} ${FALLBACK_WATCHER_INJECT_MARKER}`;
...
async function runLeaderNudgeTick(): Promise<boolean> {
  ...
  const leaderOnly = safeString(envValue('OMB_TEAM_WORKER') || '').trim() === '';
  ...
  try {
    const preComputedLeaderStale = await isLeaderStale(teamStateDir, staleThresholdMs, Date.now());
    await maybeNudgeTeamLeader({
      cwd,
      stateDir: teamStateDir,
      logsDir,
...
async function runDispatchDrainTick(): Promise<boolean> {
  const startedIso = new Date().toISOString();
  try {
    const result = await drainPendingTeamDispatch({ cwd, stateDir: teamStateDir, logsDir, maxPerTick: dispatchTickMax } as any);
    dispatchDrainRuns += 1;
    lastDispatchDrain = {
      leader_only: safeString(envValue('OMB_TEAM_WORKER') || '').trim() === '',
      last_tick_at: startedIso,
