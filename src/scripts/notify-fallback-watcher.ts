...
const maxLifetimeMs = runOnce
  ...
  : Math.max(
    ...
    asNumber(
      argValue(
        '--max-lifetime-ms',
        envValue('OMX_NOTIFY_FALLBACK_MAX_LIFETIME_MS', 'OMB_NOTIFY_FALLBACK_MAX_LIFETIME_MS') || String(defaultMaxLifetimeMs)
      ),
...
const FALLBACK_WATCHER_INJECT_MARKER = '[OMX_TMUX_INJECT]';
...
let lastDispatchDrain: DispatchDrainState = {
  leader_only: safeString(envValue('OMX_TEAM_WORKER', 'OMB_TEAM_WORKER') || '').trim() === '',
  last_tick_at: null,
  last_result: null,
  last_error: null,
};
...
const AUTO_NUDGE_STALL_MS = Math.max(
  pollMs,
  asNumber(envValue('OMX_NOTIFY_FALLBACK_AUTO_NUDGE_STALL_MS', 'OMB_NOTIFY_FALLBACK_AUTO_NUDGE_STALL_MS') || '5000', 5000),
);
...
async function runLeaderNudgeTick(): Promise<boolean> {
  ...
  const leaderOnly = safeString(envValue('OMX_TEAM_WORKER', 'OMB_TEAM_WORKER') || '').trim() === '';
