...
test('resolveReplyListenerLiveEnv stays disabled until explicitly opted in', () => {
  const result = resolveReplyListenerLiveEnv({
    OMB_DISCORD_NOTIFIER_BOT_TOKEN: 'discord-token',
    OMB_DISCORD_NOTIFIER_CHANNEL: 'channel-1',
    OMB_TELEGRAM_BOT_TOKEN: 'telegram-token',
    OMB_TELEGRAM_CHAT_ID: 'chat-1',
  });
