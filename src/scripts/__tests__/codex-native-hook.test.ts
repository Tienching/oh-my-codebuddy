  ...
  it("returns Stop continuation output while Autopilot is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB autopilot is still active (phase: execution); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "autopilot_execution",
        systemMessage: "OMB autopilot is still active (phase: execution).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("returns Stop continuation output while Ultrawork is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB ultrawork is still active (phase: executing); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "ultrawork_executing",
        systemMessage: "OMB ultrawork is still active (phase: executing).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("returns Stop continuation output while UltraQA is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB ultraqa is still active (phase: diagnose); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "ultraqa_diagnose",
        systemMessage: "OMB ultraqa is still active (phase: diagnose).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("returns Stop continuation output while team phase is non-terminal", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (review-team) at phase team-verify; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
        stopReason: "team_team-verify",
        systemMessage: "OMB team pipeline is still active at phase team-verify.",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("blocks Stop for a team worker with a non-terminal assigned task via native worker context", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB team worker worker-1 is still assigned non-terminal task 1 (in_progress); continue the current assigned task or report a concrete blocker before stopping.",
  ...
  it("returns Stop continuation output from canonical team state when coarse mode state is missing", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (canonical-team) at phase team-exec; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
  ...
  it("re-fires canonical-team Stop output for a later fresh Stop reply when coarse mode state is missing", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (canonical-team-refire) at phase team-exec; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
        stopReason: "team_team-exec",
        systemMessage: "OMB team pipeline is still active at phase team-exec.",
      });
    } finally {
  ...
  it("returns Stop continuation output from canonical team state when manifest session ownership is missing", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (legacy-team) at phase team-exec; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
        stopReason: "team_team-exec",
        systemMessage: "OMB team pipeline is still active at phase team-exec.",
      });
    } finally {
  ...
  it("reads canonical Stop fallback team state from OMB_TEAM_STATE_ROOT when configured", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (canonical-root-team) at phase team-exec; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
        stopReason: "team_team-exec",
        systemMessage: "OMB team pipeline is still active at phase team-exec.",
      });
  ...
  it("returns Stop continuation output from canonical team state rooted via OMB_TEAM_STATE_ROOT", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          `OMB team pipeline is still active (env-root-team) at phase team-exec; continue coordinating until the team reaches a terminal phase.${TEAM_STOP_COMMIT_GUIDANCE}`,
        stopReason: "team_team-exec",
        systemMessage: "OMB team pipeline is still active at phase team-exec.",
      });
    } finally {
  ...
  it("returns Stop continuation output for active ralplan skill with matching active mode state and without active subagents", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB skill ralplan is still active (phase: planning); continue until the current ralplan workflow reaches a terminal state.",
        stopReason: "skill_ralplan_planning",
        systemMessage: "OMB skill ralplan is still active (phase: planning).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("returns Stop continuation output for active deep-interview skill with matching active mode state and without active subagents", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB skill deep-interview is still active (phase: planning); continue until the current deep-interview workflow reaches a terminal state.",
        stopReason: "skill_deep-interview_planning",
        systemMessage: "OMB skill deep-interview is still active (phase: planning).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("returns Stop continuation output while Ralph is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OMB Ralph is still active (phase: executing); continue the task and gather fresh verification evidence before stopping.",
