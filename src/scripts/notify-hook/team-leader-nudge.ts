...
const LEADER_NUDGE_INJECT_MARKER = '[OMB_TMUX_INJECT]';
...
export function resolveLeaderNudgeIntervalMs() {
  const raw = safeString(process.env.OMB_TEAM_LEADER_NUDGE_MS || '');
...
export function resolveLeaderAllIdleNudgeCooldownMs() {
  const raw = safeString(process.env.OMB_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS || '');
  const parsed = asNumber(raw);
  // Default: 30 seconds.
...
export function resolveLeaderStalenessThresholdMs() {
  const raw = safeString(process.env.OMB_TEAM_LEADER_STALE_MS || '');
  const parsed = asNumber(raw);
...
export function resolveFallbackProgressStallThresholdMs() {
  const raw = safeString(process.env.OMB_TEAM_PROGRESS_STALL_MS || '');
  const parsed = asNumber(raw);
...
export function resolveWorkerTurnStallThresholdMs() {
  const raw = safeString(process.env.OMB_TEAM_WORKER_TURN_STALL_MS || '');
  const parsed = asNumber(raw);
...
function buildStatusCheckReminder(teamName) {
  return `Next: check messages; keep orchestrating; if done, gracefully shut down: omb team shutdown ${teamName}.`;
}

function buildMailboxCheckReminder(teamName) {
  return `Next: read messages; keep orchestrating; if done, gracefully shut down: omb team shutdown ${teamName}.`;
}

function buildWorkerStartEvidenceReminder(teamName, workerName) {
  return `Next: check ${workerName} msg/output, confirm task in omb team status ${teamName}, then reassign/nudge.`;
}
...
function buildLeaderActionGuidance(teamName, {
  ...
  if (leaderActionState === 'done_waiting_on_leader') {
    return `Next: decide whether to reconcile/merge results or gracefully shut down: omb team shutdown ${teamName}.`;
  }
  if (leaderActionState === 'stuck_waiting_on_leader') {
    return `Next: omb team status ${teamName}; read worker messages; unblock/reassign or shutdown.`;
  }
...
export async function emitTeamNudgeEvent(cwd, teamName, reason, orchestrationIntent, nowIso) {
  const eventsDir = join(cwd, '.omb', 'state', 'team', teamName, 'events');
...
export async function maybeNudgeTeamLeader({
  ...
  for (const teamName of candidateTeamNames) {
    ...
    let workers = [];
    try {
      const manifestPath = join(ombDir, 'state', 'team', teamName, 'manifest.v2.json');
      const configPath = join(ombDir, 'state', 'team', teamName, 'config.json');
    ...
    let mailbox = null;
    try {
      const mailboxPath = join(ombDir, 'state', 'team', teamName, 'mailbox', 'leader-fixed.json');
    ...
    if (shouldSendAllIdleNudge) {
      ...
      text = `[OMB] All ${N} worker${N === 1 ? '' : 's'} idle.${waitingText} ${leaderActionGuidance}`;
    ...
    const markedText = `${capped} ${LEADER_NUDGE_INJECT_MARKER}`;
