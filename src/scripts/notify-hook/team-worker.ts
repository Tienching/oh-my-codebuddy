...
function resolveTeamRuntimeBrand(stateDir) {
  ...
  if (normalized.endsWith('/.omb/state') || normalized.includes('/.omb/state/')) {
    return { tag: 'OMB', command: 'omb' };
  }
  return { tag: 'OMB', command: 'omb' };
}
...
export async function resolveTeamStateDirForWorker(cwd, parsedTeamWorker) {
  ...
  if (explicitStateRoot) {
    ...
    if (existsSync(explicitTeamRoot) || existsSync(resolvedExplicitStateRoot)) {
    ...
    if (alternateExplicitStateRoot) {
      ...
      if (existsSync(alternateTeamRoot) || existsSync(alternateExplicitStateRoot)) {
        return alternateExplicitStateRoot;
      }
    }
    return resolvedExplicitStateRoot;
  }
...
export async function maybeNotifyLeaderAllWorkersIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const runtimeBrand = resolveTeamRuntimeBrand(stateDir);
  ...
  const nextAction = `Next: run ${runtimeBrand.command} team status ${teamName}, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down.`;
  const message = `[${runtimeBrand.tag}] All ${N} worker${N === 1 ? '' : 's'} idle. ${nextAction} ${TEAM_WORKER_INJECT_MARKER}`;
...
export async function maybeNotifyLeaderWorkerIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const parts = [`[${runtimeBrand.tag}] ${workerName} ${currentState}`];
  ...
  const message = `${parts.join('. ')}. ${TEAM_WORKER_INJECT_MARKER}`;
