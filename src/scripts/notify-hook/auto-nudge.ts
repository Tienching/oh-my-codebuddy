...
function normalizeStallDetectionText(text) {
  return stripOrchestrationIntentTags(safeString(text))
    .replace(/\r\n?/g, '\n')
    .split('\n')
    .filter((line) => !line.includes(DEFAULT_MARKER) && !line.includes(AUTO_NUDGE_INJECT_MARKER) && !line.includes(LEGACY_INJECT_MARKER))
    .join('\n')
    .toLowerCase()
    .replace(/[’‘`]/g, '\'');
}
  ...
  try {
    ...
    try {
      const sendResult = await sendPaneInput({
        paneTarget: paneId,
        prompt: `${config.response} ${AUTO_NUDGE_INJECT_MARKER}`,
        submitKeyPresses: 2,
        submitDelayMs: 100,
      });
      if (!sendResult.ok) {
