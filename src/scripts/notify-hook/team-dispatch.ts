...
async function appendEventToStateRoot(stateRoot, teamName, event) {
  if (!stateRoot) return;
  const eventsDir = join(stateRoot, 'team', teamName, 'events');
...
function resolveIssueDispatchCooldownMs(env = process.env) {
  const raw = ISSUE_DISPATCH_COOLDOWN_ENVS.map((name) => safeString(env[name]).trim()).find(Boolean) || '';
...
function resolveDispatchTriggerCooldownMs(env = process.env) {
  const raw = DISPATCH_TRIGGER_COOLDOWN_ENVS.map((name) => safeString(env[name]).trim()).find(Boolean) || '';
...
async function appendLeaderNotificationDeferredEvent({
  ...
  await appendEventToStateRoot(stateDir, teamName, event);
  const alternateRoot = alternateStateDir(stateDir);
  if (alternateRoot && alternateRoot !== stateDir) {
    await appendEventToStateRoot(alternateRoot, teamName, event);
  }
}
...
async function appendDispatchLog(logsDir, event) {
  ...
  for (const root of roots) {
    ...
    await appendFile(path, payload).catch(() => {});
  }
}
  ...
  for (const teamName of teams) {
    ...
    await withDispatchLock(teamDirPath, async () => {
      ...
      for (const request of requests) {
        ...
        const result = await injector(request, config, resolve(cwd), stateDir).catch((error) => ({
          ok: false,
          reason: 'injector_error',
