...
Available showcases:
  omb-self       -> missions/in-action-cat-shellout-demo
  ...
  all            -> omb-self ml-tabular bayesopt latent sorting

Examples:
  scripts/run-autoresearch-showcase.sh --list
  scripts/run-autoresearch-showcase.sh bayesopt
  scripts/run-autoresearch-showcase.sh omb-self ml-tabular
USAGE
}

mission_for() {
  case "$1" in
    omb-self)   printf '%s' 'missions/in-action-cat-shellout-demo' ;;
...
args=("$@")
if [[ " ${args[*]} " == *" all "* ]]; then
  args=(omb-self ml-tabular bayesopt latent sorting)
fi
