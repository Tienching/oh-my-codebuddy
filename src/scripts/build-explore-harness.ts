...
const binaryName = process.platform === 'win32' ? 'omb-explore-harness.exe' : 'omb-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'omb-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'omb-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',
