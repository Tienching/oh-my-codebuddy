...
export function pickPrimaryWorkflowMode(modes: Iterable<string>): TrackedWorkflowMode | null {
  const normalized = normalizeTrackedModes(modes);
  ...
  for (const mode of TRACKED_WORKFLOW_MODES) {
    ...
    for (const candidatePath of candidatePaths) {
      ...
      try {
        ...
        throw new Error(
          `Cannot read ${mode} workflow state at ${candidatePath}. ${clearInstructions()}`,
        );
      }
    }
  }
