...
describe('modes/base session-scoped persistence', () => {
  it('writes mode state into the current session scope when session.json exists', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-mode-session-scope-'));
    try {
      await mkdir(join(wd, '.omb', 'state'), { recursive: true });
      await writeFile(join(wd, '.omb', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-base-write' }));

      await startMode('ralplan', 'write in session scope', 5, wd);

      const scopedPath = join(wd, '.omb', 'state', 'sessions', 'sess-base-write', 'ralplan-state.json');
      assert.equal(existsSync(scopedPath), true);
      assert.equal(existsSync(join(wd, '.omb', 'state', 'ralplan-state.json')), false);
  ...
  it('persists owner_omb_session_id for Ralph when session scope is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-mode-session-ralph-owner-'));
    try {
      ...
      assert.equal(scoped.owner_omb_session_id, sessionId);
  ...
  it('prefers session-scoped reads over root fallback and writes updates back to the session scope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-mode-session-read-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
  ...
  it('allows an explicit Ralph start to overwrite an inactive current-session Ralph file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omb-mode-session-ralph-restart-'));
    try {
      const stateDir = join(wd, '.omb', 'state');
