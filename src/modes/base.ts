/**
 * Base mode lifecycle management for oh-my-codebuddy
...
function statePath(mode: string, projectRoot?: string, sessionId?: string): string {
  const dir = sessionId ? join(stateDir(projectRoot), 'sessions', sessionId) : stateDir(projectRoot);
...
async function readModeStatePaths(mode: string, projectRoot?: string): Promise<string[]> {
  ...
  const scope = await resolveStateScope(projectRoot);
  const primary = statePath(mode, projectRoot, scope.sessionId);
  if (scope.sessionId) {
    return [primary, statePath(mode, projectRoot)];
  }
  return [primary];
}
...
export async function assertModeStartAllowed(
  ...
  for (const other of EXCLUSIVE_MODES) {
    ...
    const otherPaths = await readModeStatePaths(other, projectRoot);
...
export async function readModeState(mode: string, projectRoot?: string): Promise<ModeState | null> {
  const paths = await readModeStatePaths(mode, projectRoot);
