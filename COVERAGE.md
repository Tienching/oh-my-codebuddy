...
| deepinit | DONE (lightweight CLI successor) | `omb agents-init [path]` (`omb deepinit [path]` alias) |
...
| omb-setup | DONE | ~/.codex/skills/omb-setup/SKILL.md |
...
| ~~learn-about-omb~~ | REMOVED (v0.5.0) | — |
...
| SubagentStart | CodeBuddy CLI multi_agent native | FULL |
| SubagentStop | CodeBuddy CLI multi_agent native | FULL |
| PreCompact | AGENTS.md overlay compaction protocol | PARTIAL (instructions only) |
| Stop | notify config + postLaunch cleanup | FULL |
| SessionEnd | omb postLaunch lifecycle phase | PARTIAL (post-exit cleanup) |

`*` FULL via terminal automation workaround (default-enabled in `v0.2.3` generated `.omb/tmux-hook.json`), not native hook context injection.
...
2. **Native context injection from hooks** - Not available in Codex hooks API. Workaround: tmux prompt injection (`omb tmux-hook`) plus state files + AGENTS.md instructions (default-enabled in `v0.2.3` generated config).
