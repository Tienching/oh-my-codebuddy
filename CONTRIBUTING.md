# Contributing to oh-my-codebuddy
...
```bash
unset OMB_TEAM_WORKER OMB_TEAM_STATE_ROOT OMB_TEAM_LEADER_CWD OMB_TEAM_WORKER_CLI OMB_TEAM_WORKER_CLI_MAP OMB_TEAM_WORKER_LAUNCH_ARGS
```
...
2. Run `omb setup --force` to install it to `~/.codex/prompts/`
3. Use `/prompts:my-agent` in CodeBuddy CLI
...
2. Run `omb setup --force` to install it to `~/.codex/skills/`
3. Use `$my-skill` in CodeBuddy CLI
...
```text
docs: clarify setup steps for CodeBuddy CLI users
```
