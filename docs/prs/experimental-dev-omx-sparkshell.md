# PR Draft: Rebase `feat/omb-sparkshell` onto `experimental/dev`

## Target branch
`experimental/dev`

## Summary
This PR rebases the `omb sparkshell` feature line onto `experimental/dev` and preserves the follow-on team-status inspection work built on top of it.

It adds the native Rust-backed `omb sparkshell <command> [args...]` flow, plus a long sequence of `omb team status` improvements that surface pane-aware inspection metadata and ready-to-run sparkshell commands for leader triage.

## What’s included
### `omb sparkshell`
- adds CLI dispatch/help for `omb sparkshell`
- adds `src/cli/sparkshell.ts` for native binary discovery and launch
- adds the Rust crate under `native/omb-sparkshell/`
- adds packaging/build helpers:
  - `scripts/build-sparkshell.mjs`
  - `scripts/test-sparkshell.mjs`
- stages the packaged native binary under `bin/native/linux-x64/omb-sparkshell`
- adds focused CLI + packaging tests

### `omb team status` inspection improvements
...
- excluded `native/omb-sparkshell` from the root Cargo workspace so manifest-path cargo commands work again
...
- Native Rust build artifacts under `native/omb-sparkshell/target/` were removed after verification.
