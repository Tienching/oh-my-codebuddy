...
Status: contributor-facing contract for OMB prompt and orchestration surfaces.

## Purpose

This document explains the **behavioral prompt contract** introduced by the GPT-5.4 guidance rollout in [#608](https://github.com/Yeachan-Heo/oh-my-codebuddy/issues/608) and expanded in [#611](https://github.com/Yeachan-Heo/oh-my-codebuddy/pull/611) and [#612](https://github.com/Yeachan-Heo/oh-my-codebuddy/pull/612).
...
Issue [#615](https://github.com/Yeachan-Heo/oh-my-codebuddy/issues/615) uses examples like `src/prompts/role-planner.ts`, but the current prompt sources in this repository live in **`prompts/*.md`**, then get installed to `~/.codex/prompts/`.
...
OMB also has a narrow **instruction-composition seam** for subagents/workers whose **final resolved model** is exactly `gpt-5.4-mini`.
...
This contract is about **how OMB prompts should behave**.
It is not the same thing as OMB's routing metadata.
...
## The 4 core GPT-5.4 patterns OMB should now enforce
...
Contributors should preserve the rule that prompts keep using tools when correctness depends on retrieval, diagnostics, tests, or verification. OMB should not stop at a plausible answer if proof is still missing.
...
OMB also uses **scenario-style examples** to make the contract concrete for "continue", "make a PR", and "merge if CI green" flows.
...
- Implementation issue: [#608](https://github.com/Yeachan-Heo/oh-my-codebuddy/issues/608)
- Documentation issue: [#615](https://github.com/Yeachan-Heo/oh-my-codebuddy/issues/615)
