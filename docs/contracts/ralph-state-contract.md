...
Ralph runtime state is stored at `.omb/state/{scope}/ralph-state.json` and MUST use this schema:
...
explicit follow-up, but there is no built-in `omb team ralph ...` linked launch
path anymore.
...
1. If `session_id` is present (explicit argument or current `.omb/state/session.json`), session scope (`.omb/state/sessions/{session_id}/...`) is authoritative.
2. Root scope (`.omb/state/*.json`) is compatibility fallback only.
...
- Canonical PRD: `.omb/plans/prd-{slug}.md`
- Canonical progress ledger: `.omb/state/{scope}/ralph-progress.json`
- Legacy compatibility migration:
  - `.omb/prd.json` migrates one-way to canonical PRD markdown when no canonical PRD exists.
  - `.omb/progress.txt` migrates one-way to canonical `ralph-progress.json` when no canonical ledger exists.
