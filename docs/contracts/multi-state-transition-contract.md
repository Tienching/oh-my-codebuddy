...
multi-state compatibility rollout from `.omb/plans/prd-multi-state-compat.md`.

## Canonical sources of truth

The runtime must treat workflow state as a combination of:

- mode state files under `.omb/state/{scope}/<mode>-state.json`
- canonical workflow enumeration under `.omb/state/{scope}/skill-active-state.json`
...
> incompatible state first via `omb state ...` or the `state_*` MCP tools,
...
- `omb state clear --input '{"mode":"team"}' --json`
- `omb state clear --input '{"mode":"ralph","all_sessions":true}' --json`

MCP parity surface:

- `state_clear({ mode: "team" })`
- `state_clear({ mode: "ralph", all_sessions: true })`
