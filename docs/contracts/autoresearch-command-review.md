# `omb autoresearch` Full-Parity Review Notes
...
- `.omb/plans/prd-autoresearch-full-parity.md`
- `.omb/plans/test-spec-autoresearch-full-parity.md`
...
- `omb autoresearch` exposes both fresh launch and `--resume <run-id>` flows.
...
**Pass.**
- repo-root active-run lock lives at `.omb/state/autoresearch-state.json`.
- per-run manifest, candidate, ledger, and latest evaluator files live under `.omb/logs/autoresearch/<run-id>/`.
