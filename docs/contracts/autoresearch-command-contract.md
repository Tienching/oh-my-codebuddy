...
`omb autoresearch` is a thin supervisor that drives one Codex experiment session per iteration while OMX owns the durable keep/discard/reset loop.
...
- `--resume <run-id>` loads `.omb/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.omb/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.omb-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.omb/logs/autoresearch/<run-id>/`
