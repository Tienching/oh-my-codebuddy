...
| Worker overlay | Written inline to `{cwd}/AGENTS.md` (worktree) | `worker-bootstrap.ts` | `<!-- OMX:TEAM:WORKER:START -->` / `<!-- OMX:TEAM:WORKER:END -->` |
...
| **Category 3: Worker instructions** | The `OMX:TEAM:WORKER:START/END` marker-bounded section AND all worker-specific AGENTS files | `worker-bootstrap.ts` | Only `worker-bootstrap.ts` may apply or strip the worker overlay, write the worker root AGENTS.md, or manage the composed instruction files. |
