...
This document explains how OMB tracks workflow/skill state, how transition rules are evaluated, and which transitions are commonly allowed or blocked.
...
Authoritative workflow state lives in per-mode files under `.omb/state/`:

- root scope: `.omb/state/<mode>-state.json`
- session scope: `.omb/state/sessions/<session_id>/<mode>-state.json`
...
- `.omb/state/skill-active-state.json`
- `.omb/state/sessions/<session_id>/skill-active-state.json`
...
`blocked_on_user` is also compatibility-only. When surrounding question metadata proves OMB asked a blocking question, classify it as `askuserQuestion`; otherwise treat it as a user-wait compatibility signal instead of exposing it as the canonical vocabulary directly.
