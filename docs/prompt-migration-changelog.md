...
- Preserving each skill's contract/intent while improving correctness for CodeBuddy CLI users

### Behavior Notes

- These are documentation / instruction changes; they do not directly change runtime logic.
- One meaningful correction was made: the "agent teams" enablement guidance in `skills/omb-setup/SKILL.md` was updated to enable Codex features via `~/.codex/config.toml` rather than legacy `settings.json` env vars.
...
| `skills/omb-setup/SKILL.md` | 144 | 156 | Codex path/terminology normalization; examples updated to Codex-first conventions. | No direct runtime behavior change; instruction correctness improved. |
...
- `skills/omb-setup/SKILL.md`: largest edit surface; includes team enablement guidance (`~/.codex/config.toml` `[features]` flags) and teammate display preference storage (`~/.codex/.omb-config.json`).
