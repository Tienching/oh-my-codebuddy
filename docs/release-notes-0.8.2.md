...
- `omb setup` now skips the deprecated **`[tui]`** config section for CodeBuddy CLI `>= 0.107.0` (`#572`, fixes `#564`).
...
- skips writing the deprecated `[tui]` section when CodeBuddy CLI is `>= 0.107.0`

**Why this matters:**
- cleaner installs and upgrades
- fewer stale shipped assets after upgrades
- fewer setup/config issues on newer CodeBuddy CLI versions
...
Thanks for using **oh-my-codebuddy**. If anything regresses, please open an issue with reproduction steps, logs, and your CLI/runtime details.
