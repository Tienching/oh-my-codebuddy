...
If you use `$team`/`omb team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- If you previously relied on legacy `team_*` MCP workflows, migrate to:
  - `omb team api <operation> ...`
- For notification setup, prefer:
  - `omb configure-notifications` (or skill equivalent)
...
Thanks for using **oh-my-codebuddy**. If anything regresses, please open an issue with repro steps and logs.
