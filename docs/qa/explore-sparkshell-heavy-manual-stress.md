# Heavy / Manual Stress Lane: `omb-explore` and `omb-sparkshell`
...
**Goal**
- Confirm `omb-sparkshell --tmux-pane` preserves operator-critical facts when pane tails are large and noisy.
...
**Goal**
- Stress `omb-explore` and `omb-sparkshell` summarization with outputs that bury signal under distractors.
