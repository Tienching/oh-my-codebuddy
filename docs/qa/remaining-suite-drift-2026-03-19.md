...
```bash
unset OMB_TEAM_STATE_ROOT OMB_TEAM_WORKER OMB_TEAM_LEADER_CWD
npm run build
npm run lint
npm test
...
- The test expects the `omb exec` path to always use a generated session-scoped overlay file.
...
- The full-suite evidence for this snapshot is stored in `.omb/context/task-3-npm-test-20260319.log`.
- Focused repro logs were stored in:
  - `.omb/context/task-3-exec-test.log`
  - `.omb/context/task-3-codebase-map-test.log`
