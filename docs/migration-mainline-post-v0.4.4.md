...
- use `omb setup` and need predictable install scope behavior,
- run `omb team`/tmux workflows and want the latest reliability fixes,
...
- `omb setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are auto-migrated.
...
- [ ] If using spark worker routing, verify flags are available:
  ```bash
  omb --help | rg "spark|madmax-spark"
  ```
