...
Benchmark runs showed spawned child agents being labeled with legacy model aliases such as `sonnet`, even when OMB was launched under Codex with GPT-5.x reasoning settings.
...
After rebuilding and rerunning `omb setup --force`, spawned-agent metadata should stop surfacing `haiku` / `sonnet` / `opus` as if they were active runtime models for Codex-based OMB runs.

## Re-test steps

```bash
npm run build   # TypeScript build
node bin/omb.js setup --scope project --force
```
